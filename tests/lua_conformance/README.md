...
| `_baseline.py`     | builds/loads the regression baseline                                       |
...
`generate_all()` / the report test picks up every `*.lua` automatically, and the
regression test will flag any new discrepancy it introduces.
...
3. Regenerate the baseline (below) and review the new rows.
...
# regenerate artifacts AND the regression baseline (review the diff!)
python -m tests.lua_conformance --baseline

# quick subset while iterating
python -m tests.lua_conformance --limit-per-dir 2
```

`test_no_new_protocol_discrepancies` compares the live run against
`conformance_baseline.json`. A **new** entry fails CI (a code change moved
`midealocal` further from the Lua evidence); a **removed** entry also fails, to
force you to regenerate the (smaller) baseline and commit it.
...
- `conformance_baseline.json` — the regression gate.
