...
def _error_code_from_raw(raw_value: int) -> str:  # noqa: PLR0911
    """Map Lua error bits (primary | secondary<<8) to an error_code string."""
...
class C1ErrorCodeParser(BodyParser[str]):
    """Parse Lua error_code from body[3] and body[4] (priority order)."""

    def __init__(self) -> None:
        """Initialize C1 error-code parser."""
        super().__init__(
            "error_code",
            ERROR_PRIMARY_BYTE,
            length_in_bytes=2,
    ...
    def _parse(self, raw_value: int) -> str:
        """Convert packed error bytes to a Lua error_code."""
        return _error_code_from_raw(raw_value)


class C1FaultParser(BodyParser[bool]):
    """True when Lua error_code is not normal."""

    def __init__(self) -> None:
        """Initialize C1 fault parser."""
        super().__init__(
            "fault",
            ERROR_PRIMARY_BYTE,
            length_in_bytes=2,
    ...
    def _parse(self, raw_value: int) -> bool:
        """Return whether the packed error bytes indicate a fault."""
        return _error_code_from_raw(raw_value) != "normal"


class C1HeatingModeParser(BodyParser[str]):
    """Map heating mode byte to a Lua mode name."""

    def __init__(self) -> None:
        """Initialize C1 heating-mode parser."""
        super().__init__("heating_mode", HEATING_MODE_BYTE)

    def _parse(self, raw_value: int) -> str:
        """Convert mode byte to name, or unknown."""
        return C1_HEATING_MODE_NAMES.get(raw_value, "unknown")


class C1BitLabelParser(BodyParser[str]):
    """Map a single bit to one of two labels."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize bit-to-label parser."""
...
def c1_error_code(body: bytearray) -> str:
    ...
    return C1ErrorCodeParser().get_value(body)
