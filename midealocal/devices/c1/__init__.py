...
class C1DeviceStatus(StrEnum):
    """Derived C1 operational status (power / heating / fault)."""
...
class MideaC1Device(MideaDevice):
        ...
        super().__init__(
            ...
            **kwargs,
            attributes={
                ...
                DeviceAttributes.error_code: C1ErrorCode.NONE,
                ...
                DeviceAttributes.heating_mode: C1HeatingMode.UNKNOWN,
                ...
                DeviceAttributes.three_way_mode: C1ThreeWayMode.HEATING,
                DeviceAttributes.heating_unit_type: C1HeatingUnitType.FLOOR_HEATING,
                DeviceAttributes.status: C1DeviceStatus.OFF,
            },
        )
    ...
    def heating_modes(self) -> list[str]:
        ...
        return [mode.name.lower() for mode in C1HeatingMode.protocol_codes()]
    ...
    def _derive_status(
        ...
        """Map power/heating/fault bits to derived operational status."""
        if fault:
            return C1DeviceStatus.FAULT
        if not power:
            return C1DeviceStatus.OFF
        if heating:
            return C1DeviceStatus.RUNNING
        return C1DeviceStatus.IDLE
    ...
    @staticmethod
    def _heating_mode_code(mode: C1HeatingMode | str) -> int:
        if isinstance(mode, C1HeatingMode):
            return mode.to_protocol_code()
        return C1HeatingMode.from_lua_name(mode).to_protocol_code()
    ...
    def set_heating_target_temperature(
        ...
        mode_code = (
            ...
            if heating_mode is not None
                ...
                self._attributes[DeviceAttributes.heating_mode],
            )
        )
