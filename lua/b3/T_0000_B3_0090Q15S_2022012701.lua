...
local function assembleByteFromControlJson(control, msgBytes)
    ...
    if control["upstair_temp"] ~= nil then msgBytes[14] = tonumber(control["upstair_temp"]) end
    if control["downstair_mode"] ~= nil then
        msgBytes[18] = tonumber(control["downstair_mode"])
        msgBytes[17] = 0x02
    end
    if control["downstair_temp"] ~= nil then msgBytes[19] = tonumber(control["downstair_temp"]) end
    if control["middlestair_mode"] ~= nil then
        msgBytes[24] = tonumber(control["middlestair_mode"])
        msgBytes[23] = 0x02
    end
    if control["middlestair_temp"] ~= nil then msgBytes[25] = tonumber(control["middlestair_temp"]) end
    if control["upstair_hour"] ~= nil then msgBytes[28] = tonumber(control["upstair_hour"]) end
    if control["upstair_min"] ~= nil then msgBytes[15] = tonumber(control["upstair_min"]) end
    if control["upstair_sec"] ~= nil then msgBytes[16] = tonumber(control["upstair_sec"]) end
    if control["middlestair_hour"] ~= nil then msgBytes[29] = tonumber(control["middlestair_hour"]) end
    if control["middlestair_min"] ~= nil then msgBytes[26] = tonumber(control["middlestair_min"]) end
    if control["middlestair_sec"] ~= nil then msgBytes[27] = tonumber(control["middlestair_sec"]) end
    if control["downstair_hour"] ~= nil then msgBytes[30] = tonumber(control["downstair_hour"]) end
    if control["downstair_min"] ~= nil then msgBytes[20] = tonumber(control["downstair_min"]) end
    if control["downstair_sec"] ~= nil then msgBytes[21] = tonumber(control["downstair_sec"]) end
    return msgBytes
end
...
local function assembleByteFromOrderJson(control, msgBytes)
    ...
    msgBytes[29] = 0xFF
    msgBytes[30] = 0xFF
    if control["order_hour"] ~= nil then msgBytes[13] = tonumber(control["order_hour"]) end
    if control["order_min"] ~= nil then msgBytes[14] = tonumber(control["order_min"]) end
    if control["order_sec"] ~= nil then msgBytes[15] = tonumber(control["order_sec"]) end
    if control["upstair_mode"] ~= nil then
        msgBytes[17] = tonumber(control["upstair_mode"])
        msgBytes[16] = 0x03
    end
    if control["upstair_temp"] ~= nil then msgBytes[18] = tonumber(control["upstair_temp"]) end
    if control["downstair_mode"] ~= nil then
        msgBytes[22] = tonumber(control["downstair_mode"])
        msgBytes[21] = 0x03
    end
    if control["downstair_temp"] ~= nil then msgBytes[23] = tonumber(control["downstair_temp"]) end
    if control["middlestair_mode"] ~= nil then
        msgBytes[27] = tonumber(control["middlestair_mode"])
        msgBytes[26] = 0x03
    end
    if control["middlestair_temp"] ~= nil then msgBytes[28] = tonumber(control["middlestair_temp"]) end
    return msgBytes
end
...
local function assembleByteFromServiceControlJson(control, msgBytes)
    ...
    if control["time"] ~= nil then msgBytes[13] = tonumber(control["time"]) end
    if control["temperature"] ~= nil then msgBytes[14] = tonumber(control["temperature"]) end
    return msgBytes
end
...
local function assembleByteFromOtaJson(control, msgBytes)
    ...
    for i = 10, #array - 1 do
        if array[i] ~= nil then msgBytes[i] = tonumber(array[i], 16) end
    end
    return msgBytes
end
...
local function parseByteToJson(status, bodyBytes)
    if (bodyBytes[10] == 0x03 and bodyBytes[11] == 0x31) or (bodyBytes[10] == 0x04 and bodyBytes[11] == 0x41) then
        ...
        if bodyBytes[13] ~= nil and bodyBytes[13] ~= 0xFF then status["upstair_mode"] = tonumber(bodyBytes[13]) end
        if tonumber(bodyBytes[14]) ~= 0xFF then status["upstair_temp"] = tonumber(bodyBytes[14]) end
        if bodyBytes[34] ~= nil and tonumber(bodyBytes[33]) ~= 0xFF then
            status["upstair_hour"] = tonumber(bodyBytes[33])
        end
        if tonumber(bodyBytes[15]) ~= 0xFF then status["upstair_min"] = tonumber(bodyBytes[15]) end
        if tonumber(bodyBytes[16]) ~= 0xFF then status["upstair_sec"] = tonumber(bodyBytes[16]) end
        ...
        if bodyBytes[18] ~= nil and bodyBytes[18] ~= 0xFF then status["downstair_mode"] = tonumber(bodyBytes[18]) end
        if tonumber(bodyBytes[19]) ~= 0xFF then status["downstair_temp"] = tonumber(bodyBytes[19]) end
        if bodyBytes[35] ~= nil and tonumber(bodyBytes[35]) ~= 0xFF then
            status["downstair_hour"] = tonumber(bodyBytes[35])
        end
        if tonumber(bodyBytes[20]) ~= 0xFF then status["downstair_min"] = tonumber(bodyBytes[20]) end
        if tonumber(bodyBytes[21]) ~= 0xFF then status["downstair_sec"] = tonumber(bodyBytes[21]) end
        ...
        if bodyBytes[29] ~= nil and bodyBytes[29] ~= 0xFF then status["middlestair_mode"] = tonumber(bodyBytes[29]) end
        if tonumber(bodyBytes[30]) ~= 0xFF then status["middlestair_temp"] = tonumber(bodyBytes[30]) end
        if bodyBytes[35] ~= nil and tonumber(bodyBytes[34]) ~= 0xFF then
            status["middlestair_hour"] = tonumber(bodyBytes[34])
        end
        if tonumber(bodyBytes[31]) ~= 0xFF then status["middlestair_min"] = tonumber(bodyBytes[31]) end
        if tonumber(bodyBytes[32]) ~= 0xFF then status["middlestair_sec"] = tonumber(bodyBytes[32]) end
        ...
        if tonumber(bodyBytes[24]) ~= 0xFF then status["order_hour"] = tonumber(bodyBytes[24]) end
        if tonumber(bodyBytes[25]) ~= 0xFF then status["order_min"] = tonumber(bodyBytes[25]) end
        if tonumber(bodyBytes[26]) ~= 0xFF then status["order_sec"] = tonumber(bodyBytes[26]) end
    ...
    elseif bodyBytes[10] == 0x02 and bodyBytes[11] == 0x21 then
        ...
        if bodyBytes[24] ~= nil and bodyBytes[24] ~= 0xFF then status["middlestair_mode"] = tonumber(bodyBytes[24]) end
        if tonumber(bodyBytes[25]) ~= 0xFF then status["middlestair_temp"] = tonumber(bodyBytes[25]) end
        if bodyBytes[30] ~= nil and tonumber(bodyBytes[29]) ~= 0xFF then
            status["middlestair_hour"] = tonumber(bodyBytes[29])
        end
        if tonumber(bodyBytes[26]) ~= 0xFF then status["middlestair_min"] = tonumber(bodyBytes[26]) end
        if tonumber(bodyBytes[27]) ~= 0xFF then status["middlestair_sec"] = tonumber(bodyBytes[27]) end
    ...
    elseif bodyBytes[10] == 0x02 and bodyBytes[11] == 0x24 then
        if bodyBytes[13] ~= 0xFF then status["order_hour"] = tonumber(bodyBytes[13]) end
        if bodyBytes[14] ~= 0xFF then status["order_min"] = tonumber(bodyBytes[14]) end
        if bodyBytes[15] ~= 0xFF then status["order_sec"] = tonumber(bodyBytes[15]) end
        ...
        if bodyBytes[17] ~= nil and bodyBytes[17] ~= 0xFF then status["upstair_mode"] = tonumber(bodyBytes[17]) end
        if bodyBytes[18] ~= 0xFF then status["upstair_temp"] = tonumber(bodyBytes[18]) end
        if bodyBytes[19] ~= 0xFF then status["upstair_min"] = tonumber(bodyBytes[19]) end
        if bodyBytes[20] ~= 0xFF then status["upstair_sec"] = tonumber(bodyBytes[20]) end
        ...
        if bodyBytes[22] ~= nil and bodyBytes[22] ~= 0xFF then status["downstair_mode"] = tonumber(bodyBytes[22]) end
        if bodyBytes[23] ~= 0xFF then status["downstair_temp"] = tonumber(bodyBytes[23]) end
        if bodyBytes[24] ~= 0xFF then status["downstair_min"] = tonumber(bodyBytes[24]) end
        if bodyBytes[25] ~= 0xFF then status["downstair_sec"] = tonumber(bodyBytes[25]) end
        ...
        if bodyBytes[27] ~= nil and bodyBytes[27] ~= 0xFF then status["middlestair_mode"] = tonumber(bodyBytes[27]) end
        if bodyBytes[28] ~= 0xFF then status["middlestair_temp"] = tonumber(bodyBytes[28]) end
        if bodyBytes[29] ~= 0xFF then status["middlestair_min"] = tonumber(bodyBytes[29]) end
        if bodyBytes[30] ~= 0xFF then status["middlestair_sec"] = tonumber(bodyBytes[30]) end
    ...
    elseif bodyBytes[10] == 0x03 and bodyBytes[11] == 0x37 then
        if bodyBytes[12] then status["soft_version"] = tonumber(bodyBytes[12]) end
