...
local function assembleByteFromControlJson(control, msgBytes)
    ...
    if control["upstair_mode"] ~= nil then msgBytes[13] = tonumber(control["upstair_mode"]) end
    if control["upstair_temp"] ~= nil then msgBytes[14] = tonumber(control["upstair_temp"]) end
    if control["downstair_mode"] ~= nil then msgBytes[22] = tonumber(control["downstair_mode"]) end
    if control["downstair_temp"] ~= nil then msgBytes[23] = tonumber(control["downstair_temp"]) end
    if control["middlestair_mode"] ~= nil then msgBytes[32] = tonumber(control["middlestair_mode"]) end
    if control["middlestair_temp"] ~= nil then msgBytes[33] = tonumber(control["middlestair_temp"]) end
    if control["upstair_hour"] ~= nil then msgBytes[15] = tonumber(control["upstair_hour"]) end
    if control["upstair_min"] ~= nil then msgBytes[16] = tonumber(control["upstair_min"]) end
    if control["upstair_sec"] ~= nil then msgBytes[17] = tonumber(control["upstair_sec"]) end
    if control["upstair_order_hour"] ~= nil then msgBytes[18] = tonumber(control["upstair_order_hour"]) end
    if control["upstair_order_min"] ~= nil then msgBytes[19] = tonumber(control["upstair_order_min"]) end
    if control["upstair_order_sec"] ~= nil then msgBytes[20] = tonumber(control["upstair_order_sec"]) end
    if control["downstair_hour"] ~= nil then msgBytes[24] = tonumber(control["downstair_hour"]) end
    if control["downstair_min"] ~= nil then msgBytes[25] = tonumber(control["downstair_min"]) end
    if control["downstair_sec"] ~= nil then msgBytes[26] = tonumber(control["downstair_sec"]) end
    if control["downstair_order_hour"] ~= nil then msgBytes[27] = tonumber(control["downstair_order_hour"]) end
    if control["downstair_order_min"] ~= nil then msgBytes[28] = tonumber(control["downstair_order_min"]) end
    if control["downstair_order_sec"] ~= nil then msgBytes[29] = tonumber(control["downstair_order_sec"]) end
    if control["middlestair_hour"] ~= nil then msgBytes[34] = tonumber(control["middlestair_hour"]) end
    if control["middlestair_min"] ~= nil then msgBytes[35] = tonumber(control["middlestair_min"]) end
    if control["middlestair_sec"] ~= nil then msgBytes[36] = tonumber(control["middlestair_sec"]) end
    if control["middlestair_order_hour"] ~= nil then msgBytes[37] = tonumber(control["middlestair_order_hour"]) end
    if control["middlestair_order_min"] ~= nil then msgBytes[38] = tonumber(control["middlestair_order_min"]) end
    if control["middlestair_order_sec"] ~= nil then msgBytes[39] = tonumber(control["middlestair_order_sec"]) end
    return msgBytes
end
...
local function parseByteToJson(status, bodyBytes)
    ...
    if tonumber(bodyBytes[16]) ~= 0xFF then status["upstair_min"] = tonumber(bodyBytes[16]) end
    if tonumber(bodyBytes[17]) ~= 0xFF then status["upstair_sec"] = tonumber(bodyBytes[17]) end
    if bodyBytes[18] ~= nil and tonumber(bodyBytes[18]) ~= 0xFF then
        status["upstair_order_hour"] = tonumber(bodyBytes[18])
    end
    if tonumber(bodyBytes[19]) ~= 0xFF then status["upstair_order_min"] = tonumber(bodyBytes[19]) end
    if tonumber(bodyBytes[20]) ~= 0xFF then status["upstair_order_sec"] = tonumber(bodyBytes[20]) end
    ...
    if tonumber(bodyBytes[23]) ~= 0xFF then status["downstair_temp"] = tonumber(bodyBytes[23]) end
    if bodyBytes[24] ~= nil and tonumber(bodyBytes[24]) ~= 0xFF then
        status["downstair_hour"] = tonumber(bodyBytes[24])
    end
    if tonumber(bodyBytes[25]) ~= 0xFF then status["downstair_min"] = tonumber(bodyBytes[25]) end
    if tonumber(bodyBytes[26]) ~= 0xFF then status["downstair_sec"] = tonumber(bodyBytes[26]) end
    if bodyBytes[27] ~= nil and tonumber(bodyBytes[27]) ~= 0xFF then
        status["downstair_order_hour"] = tonumber(bodyBytes[27])
    end
    if tonumber(bodyBytes[28]) ~= 0xFF then status["downstair_order_min"] = tonumber(bodyBytes[28]) end
    if tonumber(bodyBytes[29]) ~= 0xFF then status["downstair_order_sec"] = tonumber(bodyBytes[29]) end
    ...
    if bodyBytes[31] ~= nil and bodyBytes[31] ~= 0xFF then status["middlestair_mode"] = tonumber(bodyBytes[31]) end
    if tonumber(bodyBytes[32]) ~= 0xFF then status["middlestair_temp"] = tonumber(bodyBytes[32]) end
    if bodyBytes[33] ~= nil and tonumber(bodyBytes[33]) ~= 0xFF then
        status["middlestair_hour"] = tonumber(bodyBytes[33])
    end
    if tonumber(bodyBytes[34]) ~= 0xFF then status["middlestair_min"] = tonumber(bodyBytes[34]) end
    if tonumber(bodyBytes[35]) ~= 0xFF then status["middlestair_sec"] = tonumber(bodyBytes[35]) end
    if bodyBytes[36] ~= nil and tonumber(bodyBytes[36]) ~= 0xFF then
        status["middlestair_order_hour"] = tonumber(bodyBytes[36])
    end
    if tonumber(bodyBytes[37]) ~= 0xFF then status["middlestair_order_min"] = tonumber(bodyBytes[37]) end
    if tonumber(bodyBytes[38]) ~= 0xFF then status["middlestair_order_sec"] = tonumber(bodyBytes[38]) end
    if tonumber(bodyBytes[39]) ~= 0xFF then status["soft_version"] = tonumber(bodyBytes[39]) end
    if tonumber(bodyBytes[40]) ~= 0xFF then status["cloud_async_result"] = tonumber(bodyBytes[40]) end
    ...
    if tonumber(bodyBytes[45]) ~= 0xFF then status["error_temp_controller"] = tonumber(bodyBytes[45]) end
