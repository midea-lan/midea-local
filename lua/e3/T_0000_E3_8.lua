...
function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable[KEY_BATHTUB_WATER_LEVEL] ~= nil then bathtubWaterLevel = luaTable[KEY_BATHTUB_WATER_LEVEL] end
    ...
    if luaTable[KEY_CAPACITY] ~= nil then capacity = luaTable[KEY_CAPACITY] end
...
function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if
        (dataType == 0x02 and messageBytes[0] == 0x01)
        or (dataType == 0x02 and messageBytes[0] == 0x02)
        or (dataType == 0x02 and messageBytes[0] == 0x04)
        or (dataType == 0x03 and messageBytes[0] == 0x01)
        or dataType == 0x04
    then
...
function assembleJsonByGlobalProperty()
    ...
    if deviceSubType == "32" then streams[KEY_CAPACITY] = "unknown" end
    return streams
end
