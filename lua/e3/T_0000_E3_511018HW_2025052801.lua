...
local function newCtrlAgreementJsonToCmd(json, cmd)
    ...
    if json["bathtub_curve"] then
        ...
        elseif json["bathtub_curve"] == 3 then
            cmd[13] = 0x03
        elseif json["bathtub_curve"] == 4 then
            cmd[13] = 0x04
        end
    end
    ...
    if json["new_filter_reset"] then
        ...
        if json["new_filter_reset"] == "on" then
            cmd[13] = 0x01
        elseif json["new_filter_reset"] == "off" then
            cmd[13] = 0x00
        end
    end
    if json["summer_washing"] then
        cmd[12] = 0x2A
        cmd[30] = 0x00
        cmd[14] = json["summer_washing_tem"]
        if json["summer_washing"] == "on" then
            cmd[13] = 0x01
        elseif json["summer_washing"] == "off" then
            cmd[13] = 0x00
        end
    end
    if json["cloud_customization"] then
        cmd[12] = 0x2B
        if json["beep"] then
            cmd[30] = 0x01
        else
        ...
        if json["cloud_customization"] == 0 then
            cmd[13] = 0x00
        elseif json["cloud_customization"] == 1 then
            cmd[13] = 0x01
        elseif json["cloud_customization"] == 2 then
            cmd[13] = 0x02
        elseif json["cloud_customization"] == 3 then
            cmd[13] = 0x03
        elseif json["cloud_customization"] == 4 then
            cmd[13] = 0x04
        elseif json["cloud_customization"] == 5 then
            cmd[13] = 0x05
        elseif json["cloud_customization"] == 6 then
            cmd[13] = 0x06
        elseif json["cloud_customization"] == 7 then
            cmd[13] = 0x07
        elseif json["cloud_customization"] == 8 then
            cmd[13] = 0x08
        end
    end
    if json["ai_water_fall_washings_mode"] then
        cmd[12] = 0x2C
        if json["beep"] then
            cmd[30] = 0x01
        else
        ...
        cmd[13] = tonumber(json["ai_water_fall_washings_mode"])
    end
    ...
    if json["silence"] then
        cmd[12] = 0x23
        if json["silence"] == 0 then
            cmd[15] = 0x00
        elseif json["silence"] == 1 then
            cmd[15] = 0x01
        elseif json["silence"] == 2 then
            cmd[15] = 0x02
        end
    end
    if json["lamp_control1_power"] and json["lamp_control2_power"] and json["lamp_control3_power"] then
        cmd[12] = 0x21
        if json["beep"] then
            cmd[30] = 0x01
        else
        ...
        if json["lamp_control1_power"] == "on" then cmd[13] = setBit(cmd[13], 0, "1") end
        if json["lamp_control2_power"] == "on" then cmd[13] = setBit(cmd[13], 1, "1") end
        if json["lamp_control3_power"] == "on" then cmd[13] = setBit(cmd[13], 2, "1") end
    end
    if json["max_targetWater_temperature"] and json["min_targetWater_temperature"] then
        cmd[11] = 0x17
        cmd[12] = 0x11
        if json["is_open_timed_zero_cold_water"] == "1" then cmd[13] = 0x01 end
        cmd[14] = json["max_targetWater_temperature"]
        cmd[15] = json["min_targetWater_temperature"]
    end
    if json["bind_water_heater"] then
        cmd[11] = 0x15
        if json["bind_water_heater"] == 1 then
            cmd[12] = setBit(cmd[12], 0, "1")
            if json["is_supported_zero_cold_water"] == 1 then cmd[12] = setBit(cmd[12], 1, "1") end
            if json["is_supported_half_temperature"] == 1 then cmd[12] = setBit(cmd[12], 2, "1") end
            if json["open_timed_zero_cold_water"] == 1 then cmd[12] = setBit(cmd[12], 3, "1") end
            if json["bind_device_online"] == 1 then cmd[12] = setBit(cmd[12], 4, "1") end
            if json["device_data"] then
                local device_data_length = #json["device_data"]
                print("device_data_length=", device_data_length)
                for i = 1, device_data_length / 2 do
                    cmd[i + 12] = tonumber(string.sub(json["device_data"], i * 2 - 1, i * 2), 16)
                end
            end
        end
    end
    ...
    if control then
        ...
        elseif control["appoint_four_beep"] then
            ...
            if isNewAgreement(deviceSubType) then
                if status then cmd = newCtrlAgreementJsonToCmd(status, cmd) end
                cmd = newCtrlAgreementJsonToCmd(control, cmd)
            else
                if status then cmd = oldCtrlAgreementJsonToCmd(status, cmd) end
...
local function cmdToJson(json, cmd)
    ...
    if
        (
            (cmd[10] == 0x04 and cmd[11] == 0x01)
            or (cmd[10] == 0x04 and cmd[11] == 0x00)
            or (cmd[10] == 0x03 and cmd[11] == 0x01)
            or (cmd[10] == 0x02 and cmd[11] == 0x01)
            or (cmd[10] == 0x02 and cmd[11] == 0x02)
            or (cmd[10] == 0x02 and cmd[11] == 0x14)
            or (cmd[10] == 0x02 and cmd[11] == 0x04)
        ) and cmd[12] == 0x01
    then
        ...
        elseif cmd[50] and bit.band(cmd[50], 0xFF) == 2 then
            json["bathtub_curve"] = 2
        elseif cmd[50] and bit.band(cmd[50], 0xFF) == 1 then
            json["bathtub_curve"] = 1
        elseif cmd[50] and bit.band(cmd[50], 0xFF) == 8 then
            json["bathtub_curve"] = 8
        elseif cmd[50] and bit.band(cmd[50], 0xFF) == 9 then
            json["bathtub_curve"] = 9
        else
        ...
        json["volume"] = cmd[55]
        if cmd[55] and getBit(cmd[56], 0) == "0" then
            json["silence"] = 0
        elseif cmd[55] and getBit(cmd[56], 0) == "1" then
            json["silence"] = 1
        else
            json["silence"] = 2
        end
        ...
        if cmd[51] and getBit(cmd[51], 3) == "1" then
            json["new_filter"] = "off"
        else
            json["new_filter"] = "on"
        end
        if cmd[51] then json["ai_water_fall_washings_mode"] = tonumber(bit.rshift(bit.band(cmd[51], 0x3F), 4)) end
        if cmd[62] and getBit(cmd[62], 7) == "1" then
            json["summer_washing"] = "on"
        else
            json["summer_washing"] = "off"
        end
        if cmd[62] then json["summer_washing_tem"] = tonumber(bit.band(cmd[62], 0x7F)) end
        if cmd[63] then json["cloud_customization"] = tonumber(bit.band(cmd[63], 0x1F)) end
        if cmd[61] and getBit(cmd[61], 4) == "1" then
            json["have_electronic_h_valve"] = "on"
        else
            json["have_electronic_h_valve"] = "off"
        end
        if cmd[61] then json["electronic_h_valve_power"] = tonumber(bit.band(cmd[61], 0x60)) end
        if cmd[61] and getBit(cmd[61], 7) == "1" then
            json["abnormal_temperature_sensation"] = "true"
        else
            json["abnormal_temperature_sensation"] = "false"
        end
        if cmd[66] then json["condensate_water_filter_life"] = tonumber(bit.band(cmd[66], 0x7F)) end
        ...
        elseif cmd[39] and getBit(cmd[39], 7) == "1" then
            json["error_code"] = "EF"
        elseif cmd[49] and getBit(cmd[49], 2) == "1" then
            json["error_code"] = "CE"
        elseif cmd[51] and getBit(cmd[51], 6) == "1" then
            json["error_code"] = "A6"
        elseif cmd[51] and getBit(cmd[51], 7) == "1" then
            json["error_code"] = "E9"
        elseif cmd[38] and getBit(cmd[52], 4) == "1" then
            json["error_code"] = "CF"
        else
    ...
    if
        ((cmd[10] == 0x04 and cmd[11] == 0x01 and cmd[12] == 0x11) or (cmd[10] == 0x03 and cmd[11] == 0x01))
        and cmd[12] == 0x11
    then
    ...
    if cmd[10] == 0x04 and cmd[11] == 0x05 and cmd[12] == 0x02 then
        if getBit(cmd[13], 0) == "1" then
            json["cold_water_master"] = "on"
        elseif getBit(cmd[13], 1) == "1" then
            json["cold_water_master"] = "off"
        elseif getBit(cmd[13], 6) == "1" then
            json["power"] = "on"
        elseif getBit(cmd[13], 7) == "1" then
            json["power"] = "off"
        end
        if cmd[14] ~= 0x00 then json["temperature"] = cmd[14] end
        if getBit(cmd[15], 0) == "1" then
            json["cold_water_master"] = "on"
        elseif getBit(cmd[15], 1) == "1" then
            json["cold_water_master"] = "off"
        end
    ...
    if
        (cmd[10] == 0x03 and cmd[11] == 0x02)
        or (
            cmd[10] == 0x02
            and (cmd[11] == 0x05 or cmd[11] == 0x06 or cmd[11] == 0x07 or cmd[11] == 0x05 or cmd[11] == 0x0A)
        )
    then
        json["appoint_one"] = {
            cmd[13],
    ...
    elseif
        (cmd[10] == 0x03 and cmd[11] == 0x03 and cmd[12] == 0x00)
        or (cmd[10] == 0x02 and cmd[11] == 0x0B and cmd[12] == 0x00)
    then
    ...
    elseif
        (cmd[10] == 0x03 and cmd[11] == 0x03 and cmd[12] == 0x01)
        or (cmd[10] == 0x02 and cmd[11] == 0x0C and cmd[12] == 0x01)
    then
    ...
    if (cmd[10] == 0x03 and cmd[11] == 0x05) or (cmd[10] == 0x02 and cmd[11] == 0x0D) then
