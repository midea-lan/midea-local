...
local function newCtrlAgreementJsonToCmd(json, cmd)
    ...
    if (json["max_targetWater_temperature"] and
        json["min_targetWater_temperature"]) then
        cmd[11] = 0x17
        cmd[12] = 0x11
        if (json["is_open_timed_zero_cold_water"] == '1') then
            cmd[13] = 0x01
        end
    ...
    if (json["bind_water_heater"]) then
        ...
        if (json["bind_water_heater"] == 1) then
            ...
            if (json["is_supported_zero_cold_water"] == 1) then
                cmd[12] = setBit(cmd[12], 1, '1')
            end
            if (json["is_supported_half_temperature"] == 1) then
                cmd[12] = setBit(cmd[12], 2, '1')
            end
            if (json["open_timed_zero_cold_water"] == 1) then
                cmd[12] = setBit(cmd[12], 3, '1')
            end
            if (json["bind_device_online"] == 1) then
                cmd[12] = setBit(cmd[12], 4, '1')
            end
            if json['device_data'] then
                ...
                for i = 1, device_data_length / 2 do
                    cmd[i + 12] = tonumber(
                                      string.sub(json['device_data'], i * 2 - 1,
                                                 i * 2), 16)
                end
            end
        end
...
local function cmdToJson(json, cmd)
    ...
    if (((cmd[10] == 0x04 and cmd[11] == 0x01) or
        ...
        if (cmd[51]) then
            json["ai_water_fall_washings_mode"] = tonumber(
                                                      bit.rshift(bit.band(
                                                                     cmd[51],
                                                                     0x3F), 4))
        end
        ...
        if (cmd[62]) then
            json["summer_washing_tem"] = tonumber(bit.band(cmd[62], 0x7F))
        end
        if (cmd[63]) then
            json["cloud_customization"] = tonumber(bit.band(cmd[63], 0x1F))
        end
        ...
        if (cmd[61]) then
            json["electronic_h_valve_power"] = tonumber(bit.band(cmd[61], 0x60))
        end
        ...
        if (cmd[66]) then
            json["condensate_water_filter_life"] =
                tonumber(bit.band(cmd[66], 0x7F))
        end
    ...
    if (((cmd[10] == 0x04 and cmd[11] == 0x01 and cmd[12] == 0x11) or
