...
function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable[KEY_STEAM_TIME] ~= nil then steamTime = checkBoundary(luaTable[KEY_STEAM_TIME], 0, 255) end
    ...
    if luaTable[KEY_MINUTES] ~= nil then minutes = checkBoundary(luaTable[KEY_MINUTES], 0, 99) end
    if luaTable[KEY_SECOND] ~= nil then second = checkBoundary(luaTable[KEY_SECOND], 0, 59) end
end
...
function assembleJsonByGlobalProperty()
    ...
    if weight ~= nil then streams[KEY_WEIGHT] = weight * 100 end
    ...
    if brittleFirePower ~= nil then streams[KEY_BRITTLE_FIRE_POWER] = brittleFirePower end
    if steamTime ~= nil then streams[KEY_STEAM_TIME] = steamTime end
    if tempBakeOrFerment ~= nil then streams[KEY_TEMP_BAKE_OR_FERMENT] = tempBakeOrFerment end
    if tempPureFerment ~= nil then streams[KEY_TEMP_PURE_FERMENT] = tempPureFerment end
    if tempFastSteam ~= nil then streams[KEY_TEMP_FAST_STEAM] = tempFastSteam end
    if cleanType ~= nil then streams[KEY_CLEAN_TYPE] = cleanType end
    if maintain ~= nil then streams[KEY_MAINTAIN] = maintain end
    return streams
end
