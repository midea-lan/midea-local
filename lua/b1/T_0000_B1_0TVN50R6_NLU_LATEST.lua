...
local function getPowerLevelValue(powerLevelString)
    ...
    if
        powerLevelString == "高火"
        or powerLevelString == "大火"
        or powerLevelString == "最大火力"
        or powerLevelString == "最大"
    then
        return 235
    end
    ...
    if
        powerLevelString == "低火"
        or powerLevelString == "小火"
        or powerLevelString == "最小火力"
        or powerLevelString == "最小"
    then
        return 100
    end
...
local function getModeDurationSecondValue(modeString, durationString)
    ...
    if modeString == "zymosis" then
        if tonumber(durationSecondValue) > 43200 then durationSecondValue = 43200 end
    else
        if tonumber(durationSecondValue) > 32400 then durationSecondValue = 32400 end
    end
...
function intentToJson(nluJson)
    ...
    local retTable = {}
    --local date,time
    if #slot ~= 0 then slotItems = getKVTable(slot, "parameter", "value") end
    ...
    if parsedIntent == deviceStatePI then
        ...
        elseif parsedIntent == deviceStartFunctionPI or parsedIntent == deviceSetFunctionPI then
            ...
            if slotItems["deviceMode"] == "preheat" then control["pre_heat"] = "on" end
            control["work_mode"] = getMappedMode(slotItems["deviceMode"])

            if
                slotItems["deviceMode"] == nil
                and slotItems["deviceAspect"] == nil
                and intent["deviceVerb"] ~= "set"
            then
            ...
            if
                control["work_mode"] == "double_tube"
                or control["work_mode"] == "core_baking"
                or control["work_mode"] == "total_baking"
            then
            ...
            elseif
                control["work_mode"] == "underside_tube_hot_wind_bake" or control["work_mode"] == "hot_wind_bake"
            then
        ...
        elseif parsedIntent == deviceVolumeControlPI then
            ...
            if status ~= nil then
                ...
                if secTime == nil or secTime == "null" then secTime = 0 end
                if temperature == nil or temperature == "null" then temperature = 0 end
            end
            if intent["deviceVerb"] == "up" then
                ...
                if slotItems["temperature"] ~= nil then
                    ...
                    if control["temp_set"] > 235 then control["temp_set"] = 235 end
                end
            elseif intent["deviceVerb"] == "down" then
                ...
                elseif slotItems["temperature"] ~= nil then
                    ...
                    if control["temp_set"] < 30 then control["temp_set"] = 30 end
                end
...
local function statusToJson(nluJson)
    ...
    if status == nil then status = "AA20B100000000000003310300000011001D3A4C00B400000000000000B40000DC" end
...
function nluToData(nluJson)
    -- do intentToJson
    if #nluJson == 0 then return nil end
