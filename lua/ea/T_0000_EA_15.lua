...
local function jsonToModel(luaTable)
    ...
    if luaTable[KEY_ORDER_TIME_HOUR] ~= nil then orderHour = string2Int(luaTable[KEY_ORDER_TIME_HOUR]) end
    if luaTable[KEY_ORDER_TIME_MIN] ~= nil then orderMin = string2Int(luaTable[KEY_ORDER_TIME_MIN]) end
    if luaTable[KEY_LEFT_TIME_HOUR] ~= nil then leftHour = string2Int(luaTable[KEY_LEFT_TIME_HOUR]) end
    if luaTable[KEY_LEFT_TIME_MIN] ~= nil then leftMin = string2Int(luaTable[KEY_LEFT_TIME_MIN]) end
...
function binToModel(messageBytes)
    ...
    elseif getSoftVersion() == 2 then
        ...
        if dataType == 0x02 and messageBytes[5] == 22 then
            ...
            if #messageBytes > 31 then voltage = messageBytes[30] + bit.lshift(messageBytes[31], 8) end
            if #messageBytes > 32 then indoorTemperature = messageBytes[32] end
        end
        ...
        if dataType == 0x04 then
            ...
            if messageBytes[5] == 69 then errorCode = messageBytes[8] end
        end
    end
end
...
function assembleJsonByGlobalProperty()
    ...
    if riceLevel ~= nil then streams[KEY_RICE_LEVEL] = riceLevel end
    if workFlag ~= nil then streams[KEY_WORK_FLAG] = workFlag end
    ...
    if stepActualTime ~= nil then streams["step_actual_time"] = stepActualTime end
    ...
    if storageTemp ~= nil then streams["storage_temp"] = storageTemp end
    if storageHumodity ~= nil then streams["storage_humodity"] = storageHumodity end
...
function assembleUart(bodyBytes, type)
    ...
    if getSoftVersion() == 1 or getSoftVersion() == 3 then msgBytes[8] = 0x01 end
    msgBytes[9] = type
    for i = 0, bodyLength - 1 do
