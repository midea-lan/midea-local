...
local keyTable = {
    ...
    KEY_KEEP_WARM_TIME_EXCEED = "keep_warm_time_exceed",
}
local proTable = {
    set_mode = 0x00,
    recipe_code = 0x0000,
...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable["cooking_settings"] == nil then
        if luaTable[keyTable.KEY_COOKING_END_TIME_H] ~= nil then
            proTable.cooking_end_time_h = string2Int(
                luaTable[keyTable.KEY_COOKING_END_TIME_H]
                    + (math.floor(luaTable[keyTable.KEY_COOKING_END_TIME_H] / 10) * 6)
            )
        end
        if luaTable[keyTable.KEY_COOKING_END_TIME_M] ~= nil then
            proTable.cooking_end_time_m = string2Int(
                luaTable[keyTable.KEY_COOKING_END_TIME_M]
                    + (math.floor(luaTable[keyTable.KEY_COOKING_END_TIME_M] / 10) * 6)
            )
        end
        ...
        if luaTable[keyTable.KEY_WATER_HARDNESS_VAL] ~= nil then
            proTable.water_hardness_val = string2Int(luaTable[keyTable.KEY_WATER_HARDNESS_VAL])
        end
    else
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H] ~= nil then
            if luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H] ~= "255" then
                proTable.cooking_end_time_h = string2Int(
                    luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H]
                        + (math.floor(luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H] / 10) * 6)
                )
            else
                proTable.cooking_end_time_h = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H])
            end
        end
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M] ~= "255" then
            if luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M] ~= nil then
                proTable.cooking_end_time_m = string2Int(
                    luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M]
                        + (math.floor(luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M] / 10) * 6)
                )
            else
                proTable.cooking_end_time_m = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M])
            end
        end
        if luaTable["cooking_settings"][keyTable.KEY_WATER_HARDNESS_SELECT] ~= nil then
            proTable.water_hardness_select =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_WATER_HARDNESS_SELECT])
        end
        if luaTable["cooking_settings"][keyTable.KEY_WATER_HARDNESS_VAL] ~= nil then
            proTable.water_hardness_val = string2Int(luaTable["cooking_settings"][keyTable.KEY_WATER_HARDNESS_VAL])
        end
        if luaTable["cooking_settings"][keyTable.KEY_RICE_BRAND] ~= nil then
            proTable.rice_brand = string2Int(luaTable["cooking_settings"][keyTable.KEY_RICE_BRAND])
        end
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_METHOD] ~= nil then
            proTable.cooking_method = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_METHOD])
        end
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_TIME] ~= nil then
            proTable.cooking_time = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_TIME])
        end
        if luaTable["cooking_settings"][keyTable.KEY_RICE_HARDNESS] ~= nil then
            proTable.rice_hardness = string2Int(luaTable["cooking_settings"][keyTable.KEY_RICE_HARDNESS])
        end
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_TEMPERATURE] ~= nil then
            proTable.cooking_temperature = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_TEMPERATURE])
        end
        if luaTable["cooking_settings"][keyTable.KEY_COOKING_COURSE] ~= nil then
            proTable.cooking_course = string2Int(luaTable["cooking_settings"][keyTable.KEY_COOKING_COURSE])
        end
        if luaTable["cooking_settings"][keyTable.KEY_RECIPE_CODE] ~= nil then
            proTable.recipe_code = string2Int(luaTable["cooking_settings"][keyTable.KEY_RECIPE_CODE])
        end
        if luaTable["cooking_settings"][keyTable.KEY_SOAK_TIME] ~= nil then
            proTable.soak_time = string2Int(luaTable["cooking_settings"][keyTable.KEY_SOAK_TIME])
        end
        if luaTable["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION1] ~= nil then
            proTable.pump_time_correction1 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION1])
        end
        if luaTable["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION2] ~= nil then
            proTable.pump_time_correction2 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION2])
        end
        if luaTable["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION1] ~= nil then
            proTable.process_time_correction1 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION1])
        end
        if luaTable["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION2] ~= nil then
            proTable.process_time_correction2 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION2])
        end
        if luaTable["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION1] ~= nil then
            proTable.additional_correction1 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION1])
        end
        if luaTable["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION2] ~= nil then
            proTable.additional_correction2 =
                string2Int(luaTable["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION2])
        end
    end
end
local function updateGlobalPropertyValueByByte(messageBytes)
    if #messageBytes == 0 then return nil end
    if (dataType == 0x0280) or (dataType == 0x0400) or (dataType == 0x0380) then
        ...
        proTable.pump_time_correction2 = bit.rshift(bit.band(messageBytes[11], 0xF0), 4)
        proTable.process_time_correction1 = bit.band(messageBytes[12], 0x0F)
        proTable.process_time_correction2 = bit.rshift(bit.band(messageBytes[12], 0xF0), 4)
        proTable.additional_correction1 = bit.band(messageBytes[13], 0x0F)
        proTable.additional_correction2 = bit.rshift(bit.band(messageBytes[13], 0xF0), 4)
        if messageBytes[15] ~= 0xFF then
            proTable.cooking_end_time_h = messageBytes[15] - (math.floor(messageBytes[15] / 0x10) * 6)
        else
            proTable.cooking_end_time_h = messageBytes[15]
        end
        if messageBytes[14] ~= 0xFF then
            proTable.cooking_end_time_m = messageBytes[14] - (math.floor(messageBytes[14] / 0x10) * 6)
        else
        ...
        proTable.remain_time_h = messageBytes[22] - (math.floor(messageBytes[22] / 0x10) * 6)
        proTable.remain_time_m = messageBytes[21] - (math.floor(messageBytes[21] / 0x10) * 6)
        proTable.remain_time_s = messageBytes[20] - (math.floor(messageBytes[20] / 0x10) * 6)
        proTable.cooking_state = messageBytes[23]
        proTable.keep_warm_time_exceed = bit.rshift(bit.band(messageBytes[24], 0x40), 6)
    end
    if dataType == 0x0A00 then
        if (messageBytes[0] == 0xFE) and (messageBytes[1] == 0x00) then
            ...
            proTable.pump_time_correction2 = bit.rshift(bit.band(messageBytes[41], 0xF0), 4)
            proTable.process_time_correction1 = bit.band(messageBytes[42], 0x0F)
            proTable.process_time_correction2 = bit.rshift(bit.band(messageBytes[42], 0xF0), 4)
            proTable.additional_correction1 = bit.band(messageBytes[43], 0x0F)
            proTable.additional_correction2 = bit.rshift(bit.band(messageBytes[43], 0xF0), 4)
            if messageBytes[45] ~= 0xFF then
                proTable.cooking_end_time_h = messageBytes[45] - (math.floor(messageBytes[45] / 0x10) * 6)
            else
                proTable.cooking_end_time_h = messageBytes[45]
            end
            if messageBytes[44] ~= 0xFF then
                proTable.cooking_end_time_m = messageBytes[44] - (math.floor(messageBytes[44] / 0x10) * 6)
            else
...
local function assembleJsonByGlobalProperty()
    ...
    if ((subDataType == 0x00) and (dataType == 0x0400)) or ((subDataType == 0x00) and (dataType == 0x0380)) then
        ...
        if proTable.keep_warm_time_exceed == 0 then
            streams[keyTable.KEY_KEEP_WARM_TIME_EXCEED] = "FALSE"
        else
            streams[keyTable.KEY_KEEP_WARM_TIME_EXCEED] = "TRUE"
        end
        streams["cooking_settings"] = {}
        streams["cooking_settings"][keyTable.KEY_RECIPE_CODE] = proTable.recipe_code
        streams["cooking_settings"][keyTable.KEY_RICE_BRAND] = proTable.rice_brand
        streams["cooking_settings"][keyTable.KEY_COOKING_TIME] = proTable.cooking_time
        streams["cooking_settings"][keyTable.KEY_COOKING_METHOD] = proTable.cooking_method
        streams["cooking_settings"][keyTable.KEY_COOKING_TEMPERATURE] = proTable.cooking_temperature
        streams["cooking_settings"][keyTable.KEY_RICE_HARDNESS] = proTable.rice_hardness
        if proTable.water_hardness_select == 0 then
            streams["cooking_settings"][keyTable.KEY_WATER_HARDNESS_SELECT] = "level"
        else
            streams["cooking_settings"][keyTable.KEY_WATER_HARDNESS_SELECT] = "value"
        end
        streams["cooking_settings"][keyTable.KEY_WATER_HARDNESS_VAL] = proTable.water_hardness_val
        streams["cooking_settings"][keyTable.KEY_SOAK_TIME] = proTable.soak_time
        streams["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION1] = proTable.pump_time_correction1
        streams["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION2] = proTable.pump_time_correction2
        streams["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION1] = proTable.process_time_correction1
        streams["cooking_settings"][keyTable.KEY_PROCESS_TIME_CORRECTION2] = proTable.process_time_correction2
        streams["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION1] = proTable.additional_correction1
        streams["cooking_settings"][keyTable.KEY_ADDITIONAL_CORRECTION2] = proTable.additional_correction2
        streams["cooking_settings"][keyTable.KEY_COOKING_END_TIME_H] = proTable.cooking_end_time_h
        streams["cooking_settings"][keyTable.KEY_COOKING_END_TIME_M] = proTable.cooking_end_time_m
        streams["cooking_settings"][keyTable.KEY_COOKING_COURSE] = proTable.cooking_course
    ...
    elseif subDataType == 0xFE then
        ...
        streams["cooking_settings"][keyTable.KEY_COOKING_TEMPERATURE] = proTable.cooking_temperature
        streams["cooking_settings"][keyTable.KEY_RICE_HARDNESS] = proTable.rice_hardness
        streams["cooking_settings"][keyTable.KEY_WATER_HARDNESS_SELECT] = proTable.water_hardness_select
        streams["cooking_settings"][keyTable.KEY_WATER_HARDNESS_VAL] = proTable.water_hardness_val
        streams["cooking_settings"][keyTable.KEY_SOAK_TIME] = proTable.soak_time
        streams["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION1] = proTable.pump_time_correction1
        streams["cooking_settings"][keyTable.KEY_PUMP_TIME_CORRECTION2] = proTable.pump_time_correction2
...
function jsonToData(jsonCmdStr)
    ...
    if control then
        ...
        if proTable.water_hardness_select == 0x01 then
            ...
            bodyBytes[8] = proTable.water_hardness_val + bit.lshift(proTable.water_hardness_select, 7)
            bodyBytes[9] = 0xFF
        end
        bodyBytes[10] = proTable.soak_time
        bodyBytes[11] = proTable.pump_time_correction1 + bit.lshift(proTable.pump_time_correction2, 4)
        bodyBytes[12] = proTable.process_time_correction1 + bit.lshift(proTable.process_time_correction2, 4)
        bodyBytes[13] = proTable.additional_correction1 + bit.lshift(proTable.additional_correction2, 4)
