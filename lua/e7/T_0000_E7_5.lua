...
local function jsonToModel(jsonCmd)
    ...
    if streams[KEY_FUNCTION_NO] ~= nil then functionNo = string2Int(streams[KEY_FUNCTION_NO]) end
    ...
    if streams[KEY_FIRE_LEVEL] ~= nil then fireLevel = checkBoundary(streams[KEY_FIRE_LEVEL], 0, 21) end
...
local function binToModel(binData)
    ...
    elseif getSoftVersion() == 3 then
        if dataType == 0x02 or dataType == 0x03 or dataType == 0x04 or dataType == 0x06 then
            ...
            responseType = messageBytes[9]
            workStatus = messageBytes[10]
            functionNo = bit.lshift(messageBytes[11], 24)
                + bit.lshift(messageBytes[12], 16)
                + bit.lshift(messageBytes[13], 8)
                + messageBytes[14]
