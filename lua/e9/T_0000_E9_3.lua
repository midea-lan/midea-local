...
function jsonToModel(jsonCmd)
    ...
    if streams[KEY_CONTROL_CMD_CODE] ~= nil then cmdControlCode = string2Int(streams[KEY_CONTROL_CMD_CODE]) end
    ...
    if streams[KEY_WORK_STATUS] ~= nil then workStatus = string2Int(streams[KEY_WORK_STATUS]) end
    if streams[KEY_VOLUME] ~= nil then volume = string2Int(streams[KEY_VOLUME]) end
    if streams[KEY_COLOR] ~= nil then color = string2Int(streams[KEY_COLOR]) end
    if streams[KEY_DOSING] ~= nil then dosing = string2Int(streams[KEY_DOSING]) end
    if streams[KEY_TIME_CUISINE_MIN] ~= nil then
        timeCuisineMin = checkBoundary(streams[KEY_TIME_CUISINE_MIN], 0, 59)
    end
    if streams[KEY_TIME_CUISINE_HR] ~= nil then timeCuisineHr = checkBoundary(streams[KEY_TIME_CUISINE_HR], 0, 23) end
    if streams[KEY_TIME_RESERVE_MIN] ~= nil then
        timeReserveMin = checkBoundary(streams[KEY_TIME_RESERVE_MIN], 0, 59)
    end
    if streams[KEY_TIME_RESERVE_HR] ~= nil then timeReserveHr = checkBoundary(streams[KEY_TIME_RESERVE_HR], 0, 23) end
