...
local keyTable = {
    ...
    KEY_DEHYDRATION_THOUSAND = "dehydration_thousand",
...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable[keyTable.KEY_DEHYDRATION_THOUSAND] == "on" then
        proTable.dehydrationThousand = 0x10
    elseif luaTable[keyTable.KEY_DEHYDRATION_THOUSAND] == "off" then
        proTable.dehydrationThousand = 0x00
    end
...
local function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if (dataType == 0x02) or (dataType == 0x03) then
        if messageBytes[0] == 0x00 then
            ...
            proTable.dehydrationThousand = bit.band(messageBytes[15], 0x10)
...
local function assembleJsonByGlobalProperty()
    ...
    if (subDataType == 0x00) then
        ...
        if proTable.dehydrationThousand == 0x10 then
            streams[keyTable.KEY_DEHYDRATION_THOUSAND] = "on"
        elseif proTable.dehydrationThousand == 0x00 then
            streams[keyTable.KEY_DEHYDRATION_THOUSAND] = "off"
        end
...
local function jsonToData(jsonCmdStr)
    ...
    if (control) then
        ...
        if (proTable.functionType == 0x10) then
            ...
            if proTable.dehydrationThousand ~= 0xff then
                byte15 = bit.bor(byte15, proTable.dehydrationThousand)
            end
...
function _M.dataToJson(jsonCmd) return dataToJson(jsonCmd) end
function _M.jsonToData(jsonCmd) return jsonToData(jsonCmd) end
return _M
