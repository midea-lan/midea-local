...
local function assembleByteFromJson(result, msgBytes)
    ...
    if control then
        ...
        elseif control["power"] and control["power"] == "off" then
            ...
            msgBytes[11] = 0x04
            if
                control["winter_mode"] and control["winter_mode"] == "on"
                or control["summer_mode"] and control["summer_mode"] == "off"
            then
                msgBytes[12] = 0x01
                msgBytes[13] = 0x01
            elseif
                control["winter_mode"] and control["winter_mode"] == "off"
                or control["summer_mode"] and control["summer_mode"] == "on"
            then
                msgBytes[12] = 0x01
                msgBytes[13] = 0x02
            ...
            elseif control["exclusive_temperature"] then
                ...
                if control["exclusive_temperature_switch"] and control["exclusive_temperature_switch"] == "on" then
                    msgBytes[14] = 0x01
                elseif control["exclusive_temperature_switch"] and control["exclusive_temperature_switch"] == "off" then
                    msgBytes[14] = 0x00
                end
            elseif control["exclusive_temperature_2"] then
                msgBytes[12] = 0x16
                msgBytes[15] = control["exclusive_temperature_2"]
                if control["exclusive_temperature_switch_2"] and control["exclusive_temperature_switch_2"] == "on" then
                    msgBytes[16] = 0x01
                elseif
                    control["exclusive_temperature_switch_2"]
                    and control["exclusive_temperature_switch_2"] == "off"
                then
                    msgBytes[16] = 0x00
                end
            elseif control["exclusive_temperature_3"] then
                ...
                if control["exclusive_temperature_switch_3"] and control["exclusive_temperature_switch_3"] == "on" then
                    msgBytes[18] = 0x01
                elseif
                    control["exclusive_temperature_switch_3"]
                    and control["exclusive_temperature_switch_3"] == "off"
                then
                    msgBytes[18] = 0x00
                end
            elseif control["temperature_sensation_switch"] and control["temperature_sensation_switch"] == "on" then
                msgBytes[12] = 0x15
                msgBytes[13] = 0x01
            elseif control["temperature_sensation_switch"] and control["temperature_sensation_switch"] == "off" then
                msgBytes[12] = 0x15
                msgBytes[13] = 0x00
            elseif control["heat_appointment_switch"] and control["heat_appointment_switch"] == "off" then
                msgBytes[11] = 0x0A
                msgBytes[12] = 0x01
                msgBytes[13] = 0x00
                msgBytes[14] = 0x00
            ...
            elseif control["bath_mode"] then
                ...
                if control["mode_temperature"] then msgBytes[14] = control["mode_temperature"] end
            elseif control["buzzing_master_switch"] and control["buzzing_master_switch"] == "no_buzzing" then
                msgBytes[12] = 0x17
                msgBytes[13] = 0x01
            elseif control["buzzing_master_switch"] and control["buzzing_master_switch"] == "buzzing" then
                msgBytes[12] = 0x17
                msgBytes[13] = 0x00
            elseif control["reset_filter"] and control["reset_filter"] == "on" then
                msgBytes[12] = 0x19
                msgBytes[13] = 0x01
            elseif control["cold_water_single"] and control["cold_water_single"] == "on" then
                msgBytes[12] = 0x1A
                msgBytes[13] = 0x01
            elseif control["cold_water_single"] and control["cold_water_single"] == "off" then
                msgBytes[12] = 0x1A
                msgBytes[13] = 0x00
            elseif control["cold_water_dot"] and control["cold_water_dot"] == "on" then
                msgBytes[12] = 0x1B
                msgBytes[13] = 0x01
            elseif control["cold_water_dot"] and control["cold_water_dot"] == "off" then
                msgBytes[12] = 0x1B
                msgBytes[13] = 0x00
            elseif control["cold_water_appoint_switch"] and control["cold_water_appoint_switch"] == "on" then
                msgBytes[12] = 0x1C
                msgBytes[13] = 0x01
            elseif control["cold_water_appoint_switch"] and control["cold_water_appoint_switch"] == "off" then
                msgBytes[12] = 0x1C
                msgBytes[13] = 0x00
            elseif control["cold_water_has_enabled_appoint"] and control["cold_water_has_enabled_appoint"] == "on" then
                msgBytes[12] = 0x1D
                msgBytes[13] = 0x01
            elseif control["cold_water_has_enabled_appoint"] and control["cold_water_has_enabled_appoint"] == "off" then
                msgBytes[12] = 0x1D
                msgBytes[13] = 0x00
            elseif control["cold_water_appoint_master"] and control["cold_water_appoint_master"] == "on" then
                msgBytes[12] = 0x1E
                msgBytes[13] = 0x01
            elseif control["cold_water_appoint_master"] and control["cold_water_appoint_master"] == "off" then
                msgBytes[12] = 0x1E
                msgBytes[13] = 0x00
            elseif control["cold_water_master"] and control["cold_water_master"] == "on" then
                msgBytes[12] = 0x1F
                msgBytes[13] = 0x01
            elseif control["cold_water_master"] and control["cold_water_master"] == "off" then
                msgBytes[12] = 0x1F
                msgBytes[13] = 0x00
            elseif control["pressure"] and control["pressure"] == "on" then
                msgBytes[12] = 0x21
                msgBytes[13] = 0x01
            elseif control["pressure"] and control["pressure"] == "off" then
                msgBytes[12] = 0x21
                msgBytes[13] = 0x00
            elseif control["half_pipe_time"] then
                msgBytes[12] = 0x20
                msgBytes[14] = control["half_pipe_time"]
                if control["half_pipe_time_switch"] and control["half_pipe_time_switch"] == "on" then
                    msgBytes[13] = 0x01
                elseif control["half_pipe_time_switch"] and control["half_pipe_time_switch"] == "off" then
                    msgBytes[13] = 0x00
                else
                    msgBytes[13] = 0x01
                end
            elseif control["cold_water_ai"] and control["cold_water_ai"] == "on" then
                msgBytes[12] = 0x21
                msgBytes[13] = 0x01
            elseif control["cold_water_ai"] and control["cold_water_ai"] == "off" then
                msgBytes[12] = 0x21
                msgBytes[13] = 0x00
            end
        end
        if control["buzzing_switch"] and control["buzzing_switch"] == "no_buzzing" then
            msgBytes[30] = 0x01
        elseif control["buzzing_switch"] and control["buzzing_switch"] == "buzzing" then
            msgBytes[30] = 0x00
        end
    elseif query then
        msgBytes[10] = 0x03
...
local function parseByteToJson(status, bodyBytes)
    ...
    if bit.band(bodyBytes[13], 0x40) == 0x40 then
        status["pressure"] = "on"
    else
        status["pressure"] = "off"
    end
