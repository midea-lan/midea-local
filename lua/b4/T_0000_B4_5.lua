...
local function jsonToModel(stateJson, controlJson)
    ...
    if getSoftVersion() == 2 then
        ...
        if controlCmd[KEY_TEMPERATURE] ~= nil then temValue = controlCmd[KEY_TEMPERATURE] end
        temperatureValue = checkBoundary(temValue, 70, 230)
        temValue = oldState[KEY_TUBE_TEMPERATURE]
        if controlCmd[KEY_TUBE_TEMPERATURE] ~= nil then temValue = controlCmd[KEY_TUBE_TEMPERATURE] end
        undersideTubeTempValue = checkBoundary(temValue, 70, 230)
        temValue = oldState[KEY_WORK_HOUR]
        if controlCmd[KEY_WORK_HOUR] ~= nil then temValue = controlCmd[KEY_WORK_HOUR] end
        workHourValue = checkBoundary(temValue, 0, 9)
        temValue = oldState[KEY_WORK_MINUTE]
        if controlCmd[KEY_WORK_MINUTE] ~= nil then temValue = controlCmd[KEY_WORK_MINUTE] end
        workMinuteValue = checkBoundary(temValue, 0, 59)
        temValue = oldState[KEY_WORK_SECOND]
        if controlCmd[KEY_WORK_SECOND] ~= nil then temValue = controlCmd[KEY_WORK_SECOND] end
        workSecond = checkBoundary(temValue, 0, 59)
        temValue = oldState[KEY_WORK_MODE]
        if controlCmd[KEY_WORK_MODE] ~= nil then temValue = controlCmd[KEY_WORK_MODE] end
        ...
        if controlCmd[KEY_WORK_STATUS] ~= nil then temValue = controlCmd[KEY_WORK_STATUS] end
        ...
        if (workHourValue == 0) and (workMinuteValue == 0) then workMinuteValue = BYTE_MINUTE_DEFAULT end
    end
end
local function binToModel(binData)
    ...
    if getSoftVersion() == 2 then
        if dataType == 0x02 and messageBytes[0] == 0x22 then
            ...
            elseif
                messageBytes[1] == 0x01
                or messageBytes[1] == 0x03
                or messageBytes[1] == 0x04
                or messageBytes[1] == 0x05
            then
...
function jsonToData(jsonCmd)
    ...
    if deviceSN ~= nil then deviceSN8 = string.sub(deviceSN, 10, 17) end
    ...
    elseif control then
        ...
        if getSoftVersion() == 2 then
            if control[KEY_WORK_STATUS] ~= nil or control[KEY_LOCK] ~= nil then
                bodyBytes[0] = 0x22
                bodyBytes[1] = 0x02
                if control[KEY_WORK_STATUS] ~= nil then bodyBytes[2] = workStatusValue end
                if control[KEY_LOCK] ~= nil then bodyBytes[3] = workModeValue end
            else
                bodyBytes[0] = 0x22
                bodyBytes[1] = 0x04
