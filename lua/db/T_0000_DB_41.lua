...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    elseif luaTable["temperature"] == "95" then
        temperature = 0x06
    elseif luaTable["temperature"] == "90" then
        temperature = 0x08
    elseif luaTable["temperature"] == "50" then
        temperature = 0x09
    else
        temperature = 0xff
    end
    ...
    if luaTable["water_plus"] == "0" then
        water_plus = 0
    elseif luaTable["water_plus"] == "1" then
        water_plus = 1
    else
        water_plus = 3
    end
    if luaTable["silent"] == "0" then
        silent = 0
    elseif luaTable["silent"] == "1" then
        silent = 1
    else
        silent = 3
    end
...
local function assembleJsonByGlobalProperty()
    ...
    elseif program == 0x6C then
        streams["program"] = "eco_40_60"
    elseif program == 0x6D then
        streams["program"] = "steam_care"
    elseif program == 0x6E then
        streams["program"] = "allergy_care"
    elseif program == 0x6F then
        streams["program"] = "greatwaves"
    elseif program == 0x71 then
        streams["program"] = "wash45"
    elseif program == 0x72 then
        streams["program"] = "quick12"
    elseif program == 0x73 then
        streams["program"] = "bulky"
    elseif program == 0x74 then
        streams["program"] = "whites"
    elseif program == 0x75 then
        streams["program"] = "towels"
    elseif program == 0x76 then
        streams["program"] = "delicates"
    elseif program == 0x77 then
        streams["program"] = "drum_clean"
    elseif program == 0x80 then
        streams["program"] = "wash_and_dry"
    elseif program == 0x81 then
        streams["program"] = "auto_dry"
    elseif program == 0x82 then
        streams["program"] = "time_dry"
    end
    ...
    elseif temperature == 0x08 then
        streams["temperature"] = "90"
    elseif temperature == 0x09 then
        streams["temperature"] = "50"
    end
    ...
    if byte31 ~= nil then
        ...
        if bit.band(byte31, 0x02) == 0x02 then
            streams["water_plus"] = "1"
        else
            streams["water_plus"] = "0"
        end
        if bit.band(byte31, 0x04) == 0x04 then
            streams["remote_control_flag"] = "1"
        else
            streams["remote_control_flag"] = "0"
        end
        if bit.band(byte31, 0x08) == 0x08 then
            streams["silent"] = "1"
        else
            streams["silent"] = "0"
        end
    ...
    if byte29 ~= nil then
        ...
        if bit.band(byte29, 0x80) == 0x80 then
            streams["hy_pro"] = "1"
        else
            streams["hy_pro"] = "0"
        end
    ...
    if appointment_time_high_byte ~= nil and appointment_time_low_byte ~= nil then
    ...
    if byte25 ~= nil then
        if bit.band(byte25, 0x01) == 0x01 then
        ...
        if bit.band(byte25, 0x08) == 0x08 then
        ...
        if bit.band(byte25, 0x40) == 0x40 then
            streams["disinfectant"] = "1"
        else
            streams["disinfectant"] = "0"
        end
        if bit.band(byte25, 0x80) == 0x80 then
            streams["active_oxygen"] = "1"
        else
            streams["active_oxygen"] = "0"
        end
    end
    if byte21 ~= nil and byte22 ~= nil then
    ...
    if control then
        if judge_if_protocol_02ca_control(control) ~= nil then
            ...
            if control["power"] ~= nil then
                ...
                bodyBytes[19] = bitBor(
                    bitBor(wind_dispel, bit.lshift(speedy, 2)),
