...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    elseif luaTable[keyTable.KEY_PROGRAM] == "tub_clean_wash_dry" then
        proTable.program = 0x32
    ...
    if luaTable[keyTable.KEY_DETERGENT_REMAIN_EXPLANATION] ~= nil then
        proTable.detergentRemainExplanation = string2Int(luaTable[keyTable.KEY_DETERGENT_REMAIN_EXPLANATION])
    end
    ...
    if luaTable[keyTable.KEY_SOFTNER_REMAIN_EXPLANATION] ~= nil then
        proTable.softnerRemainExplanation = string2Int(luaTable[keyTable.KEY_SOFTNER_REMAIN_EXPLANATION])
    end
    ...
    if luaTable["last_app_course_number"] ~= nil then
        proTable.last_app_course_number = string2Int(luaTable["last_app_course_number"])
    end
...
local function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if (dataType == 0x02) or (dataType == 0x03) then
        if messageBytes[0] == 0x00 then
            ...
            proTable.appCourseNumber = bit.band(bit.rshift(messageBytes[28], 2), 0x0f)
    ...
    if (dataType == 0x02) or (dataType == 0x04) or (dataType == 0x05) then
        if messageBytes[0] == 0x10 then
            ...
            proTable.last_app_course_number = messageBytes[2]
...
local function assembleJsonByGlobalProperty()
    ...
    if subDataType == 0x00 then
        ...
        elseif proTable.program == 0x32 then
            streams[keyTable.KEY_PROGRAM] = "tub_clean_wash_dry"
...
function jsonToData(jsonCmdStr)
    ...
    if control then
        ...
        if proTable.functionType == 0x10 then
            ...
            bodyBytes[2] = proTable.last_app_course_number
