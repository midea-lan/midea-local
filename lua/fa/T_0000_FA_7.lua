...
function jsonToModel(jsonCmd)
    ...
    if streams["gear"] ~= nil then gearValue = checkBoundary(streams["gear"], 1, 26) end
    ...
    if streams["timer_on_hour"] ~= nil then timerOnHour = checkBoundary(string2Int(streams["timer_on_hour"]), 0, 24) end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        for key, value in pairs(control) do
            ...
            if key == "swing_direction" then haveSwingDirection = true end
        ...
        if (havePower == false) or (newPower == oldPower) then bodyBytes[4] = bit.bor(0x80, bodyBytes[4]) end
