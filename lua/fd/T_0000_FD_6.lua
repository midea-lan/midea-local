...
function jsonToModel(jsonCmd)
    ...
    if streams[KEY_SET_HUMIDITY] ~= nil then setHumidityValue = checkBoundary(streams[KEY_SET_HUMIDITY], 1, 100) end
end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        bodyBytes[0] = 0x48
        if powerValue ~= nil then bodyBytes[1] = bit.bor(powerValue, BYTE_BUZZER_ON) end
        if fanspeedValue ~= nil then bodyBytes[3] = fanspeedValue end
        if setHumidityValue ~= nil then bodyBytes[7] = setHumidityValue end
