...
local function jsonToModel(jsonCmd, subType, modelResult)
    ...
    if s.wind_speed then
        modelResult.fanspeedValue = getIndexFromValueTab(ENUM_WIND_SPEED_TAB(subType), s.wind_speed, 0)
    end
    if s.humidity then modelResult.setHumidityValue = tonumber(s.humidity) end
    if s.humidity_mode then modelResult.humidityMode = getIndexFromValueTab(ENUM_MODE_TAB, s.humidity_mode, 0) end
    if s.bright_led then modelResult.brightLED = getIndexFromValueTab(ENUM_BRIGHT_TAB, s.bright_led, 0) end
    if s.display_on_off then
        modelResult.displayOnOffValue = s.display_on_off == "on" and 1 or s.display_on_off == "off" and 2 or 0
    end
    modelResult.setTimerFlag = false
    if s.power_off_timer then
        modelResult.scheduleCloseSwitcher = s.power_off_timer == "on" and true or false
    ...
    if s.power_on_timer then
        modelResult.scheduleOpenSwitcher = s.power_on_timer == "on" and true or false
        modelResult.setTimerFlag = true
    end
    if s.time_on then
    ...
    if s.disinfect_on_off then
        modelResult.disinfectOnOffValue = s.disinfect_on_off == "on" and 1 or s.disinfect_on_off == "off" and 2 or 0
    end
    if s.netIons_on_off then modelResult.netIonsOnOffValue = s.netIons_on_off == "on" and 1 or 0 end
    if s.airDry_on_off then
        modelResult.airDryOnOffValue = s.airDry_on_off == "on" and 1 or s.airDry_on_off == "off" and 2 or 0
    end
    if s.wind_gear then
        modelResult.windGearValue = getIndexFromValueTab(ENUM_WIND_SPEED_TAB(subType), s.wind_gear, 0)
    end
    if s.buzzer then modelResult.buzzerValue = s.buzzer == "on" and 1 or 0 end
    if s.light_color then modelResult.lightColorValue = getIndexFromValueTab(ENUM_LIGHT_COLOR_TAB, s.light_color, 0) end
    return modelResult
end
local function jsonToModelByStatus(jsonCmd, subType)
    ...
    if subType < 8 and subType ~= 0 then
        modelResult.fanspeedValue = getIndexFromValueTab(ENUM_WIND_SPEED_TAB(subType), s.wind_speed, 0x7f)
    end
    if s.humidity then modelResult.setHumidityValue = s.humidity end
    modelResult.humidityMode = 0
    if subType < 8 and subType ~= 0 then
        modelResult.humidityMode = getIndexFromValueTab(ENUM_MODE_TAB, s.humidity_mode, 0)
    end
    if s.bright_led then modelResult.brightLED = getIndexFromValueTab(ENUM_BRIGHT_TAB, s.bright_led, 0) end
    if s.display_on_off then
        modelResult.displayOnOffValue = s.display_on_off == "on" and 1 or s.display_on_off == "off" and 2 or 0
...
local function binToModel(binData)
    ...
    modelResult.buzzerValue = bit.band(messageBytes[19], 0x80) == 0x80 and 1 or 0
    ...
    if #messageBytes >= 33 then modelResult.runningPercentValue = messageBytes[30] end
    modelResult.humidityMode = messageBytes[13]
    if #messageBytes >= 22 then modelResult.netIonsOnOffValue = bit.rshift(bit.band(messageBytes[19], 0x40), 6) end
    if #messageBytes >= 37 then modelResult.disinfectOnOffValue = bit.band(messageBytes[34], 0x03) end
    if #messageBytes >= 41 then modelResult.windGearValue = messageBytes[39] end
    if #messageBytes >= 44 then modelResult.airDryOnOffValue = bit.rshift(bit.band(messageBytes[42], 0x30), 4) end
    if #messageBytes >= 45 then modelResult.displayOnOffValue = bit.band(messageBytes[43], 0x03) end
    return modelResult
end
...
function jsonToData(jsonCmd)
    ...
    if deviceSubType == nil then
        ...
        if type(deviceSubType) == "string" then deviceSubType = tonumber(deviceSubType) end
    end
    ...
    elseif control then
        ...
        if status then model = jsonToModelByStatus(status, deviceSubType) end
        if control then model = jsonToModel(control, deviceSubType, model) end
        bodyBytes[0] = 0x48
        if model.powerValue ~= nil then bodyBytes[1] = bit.bor(model.powerValue, 0x42) end
        if control.silence_ctrl ~= nil and control.silence_ctrl == "enable" then
            bodyBytes[1] = bit.bor(bodyBytes[1], 0x04)
        end
        if model.lightColorValue ~= nil then bodyBytes[2] = model.lightColorValue end
        if model.fanspeedValue ~= nil then bodyBytes[3] = model.fanspeedValue end
        if model.setTimerFlag == true then
            bodyBytes[3] = bit.bor(bodyBytes[3], 0x80)
            local timingOffValue = model.scheduleCloseTime ~= nil and model.scheduleCloseTime or 0
            ...
            if model.scheduleCloseSwitcher == true then
                ...
                bodyBytes[5] = bit.bor(uptable["BYTE_SCHEDULE_CLOSE_TIME_SWITCHER_OFF"], timingOffMul15)
            end
            ...
            local timingOnValue = model.scheduleOpenTime ~= nil and model.scheduleOpenTime or 0
            ...
            if model.scheduleOpenSwitcher == true then
                ...
                bodyBytes[4] = bit.bor(uptable["BYTE_SCHEDULE_OPEN_TIME_SWITCHER_OFF"], timingOnMul15)
            end
        ...
        if model.setHumidityValue ~= nil then bodyBytes[7] = model.setHumidityValue end
        if model["brightLED"] ~= nil then bodyBytes[9] = model["brightLED"] end
        if model["humidityMode"] ~= nil then bodyBytes[11] = model["humidityMode"] end
        if model["buzzerValue"] ~= nil then
            bodyBytes[10] = bit.bor(bodyBytes[10], model["buzzerValue"] == 1 and 0x40 or 0)
        end
        if model["disinfectOnOffValue"] ~= nil then bodyBytes[15] = model["disinfectOnOffValue"] end
        if model.displayOnOffValue ~= nil then
            bodyBytes[15] = bit.lshift(model.displayOnOffValue, 6) + bodyBytes[15]
        end
        if model["netIonsOnOffValue"] ~= nil then
            bodyBytes[10] = bit.bor(bodyBytes[10], bit.lshift(model["netIonsOnOffValue"], 5))
        end
        if model["airDryOnOffValue"] ~= nil then bodyBytes[21] = model["airDryOnOffValue"] end
        if model["windGearValue"] ~= nil then bodyBytes[18] = model["windGearValue"] end
...
function dataToJson(jsonCmd)
    ...
    streams.light_color = getValueFromValueTab(ENUM_LIGHT_COLOR_TAB, model.lightColorValue, "invalid")
    streams.wind_speed = getValueFromValueTab(ENUM_WIND_SPEED_TAB(deviceSubType), model.fanspeedValue, "invalid")
    ...
    streams.bright_led = getValueFromValueTab(ENUM_BRIGHT_TAB, model.brightLED, "invalid")
    streams.humidity_mode = getValueFromValueTab(ENUM_MODE_TAB, model.humidityMode, "invalid")
    streams.power_off_timer = model.scheduleCloseSwitcher == true and "on" or "off"
    streams.time_off = model.scheduleCloseTime
    streams.power_on_timer = model.scheduleOpenSwitcher == true and "on" or "off"
    streams.time_on = model.scheduleOpenTime
    streams.running_percent = model.runningPercentValue
    streams.disinfect_on_off = model.disinfectOnOffValue == 1 and "on"
        or model.disinfectOnOffValue == 2 and "off"
        or "invalid"
    streams.netIons_on_off = model.netIonsOnOffValue == 1 and "on" or "off"
    streams.airDry_on_off = model.airDryOnOffValue == 1 and "on" or model.airDryOnOffValue == 2 and "off" or "invalid"
    streams.wind_gear = getValueFromValueTab(ENUM_WIND_SPEED_TAB(deviceSubType), model.windGearValue, "invalid")
    streams.buzzer = model.buzzerValue == 1 and "on" or "off"
    if model.displayOnOffValue then
        streams.display_on_off = model.displayOnOffValue == 1 and "on"
            or model.displayOnOffValue == 2 and "off"
            or "invalid"
    end
    if #bodyBytes >= 49 then
        streams["air_dry_left_time"] = bodyBytes[40] + bodyBytes[41] * 256
        local sensorTemperature = (bit.band(bodyBytes[45], 0x7f) * 256 + bodyBytes[44]) / 10
        if bit.band(bodyBytes[45], 0x80) == 0x80 then sensorTemperature = sensorTemperature * -1 end
