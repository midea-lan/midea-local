...
local function setBits(pBytes, pIndex, pBitStartIndex, pBitEndIndex, pValue)
    pBytes[pIndex] = _setBits(pBytes[pIndex], pBitStartIndex, pBitEndIndex, pValue)
    return pBytes
end
...
local function initByteByStatus(status, byteArray)
    ...
    for key, value in pairs(status) do
        if key == "gear" then
            ...
            if numTemp >= 1 and numTemp <= 10 then byteArray[15] = numTemp end
        ...
        elseif key == "timer_off_hour" then
            ...
            if numTemp >= 1 and numTemp <= 24 then setBits(byteArray, 20, 0, 4, numTemp) end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        msgBytes[9] = 0x02
        msgBytes[28] = 0xFF
        if deviceSubType == 3 or deviceSubType == 4 then msgBytes = initByteByStatus(status, msgBytes) end
        local numTemp = 0
        for key, value in pairs(control) do
            ...
            elseif key == "gear" then
                ...
                if numTemp >= 1 and numTemp <= 10 then msgBytes[15] = numTemp end
            elseif key == "temperature" then
                if type(value) == "number" then
                    numTemp = value
                else
            ...
            elseif key == "uvMinute" then
                ...
                if uvMinuteTemp >= 1 and uvMinuteTemp <= 255 then msgBytes[36] = uvMinuteTemp end
            elseif key == "hotDryMinute" then
                ...
                if hotDryMinuteTemp >= 1 and hotDryMinuteTemp <= 255 then msgBytes[37] = hotDryMinuteTemp end
            end
        end
    end
...
function dataToJson(jsonCmd)
    ...
    if sub_cmd == 0 then
        ...
        if humidify_feedback >= 1 and humidify_feedback <= 100 then streams["cur_humidity"] = humidify_feedback end
        ...
        if #info >= 33 then streams["power_statistics"] = msgBytes[31] * 256 + msgBytes[30] end
        if #info >= 34 then streams["protocol_version"] = msgBytes[32] end
        if #info >= 36 then streams["running_time"] = msgBytes[34] * 256 + msgBytes[33] end
        if #info >= 37 then
            ...
            if uvMinute >= 1 and uvMinute <= 255 then streams["uvMinute"] = uvMinute end
        end
        if #info >= 38 then
            local hotDryMinute = msgBytes[37]
            if hotDryMinute >= 1 and hotDryMinute <= 255 then streams["hotDryMinute"] = hotDryMinute end
        end
    end
    if sub_cmd == 1 then
    end
