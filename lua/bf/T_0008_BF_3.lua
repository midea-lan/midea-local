...
local function updateGlobalPropertyValueByJson(luaTable)
    if luaTable[keyTable.KEY_RECIPE_NUMBER] ~= nil then
        proTable.recipe_number = string2Int(luaTable[keyTable.KEY_RECIPE_NUMBER])
    end
    if luaTable[keyTable.KEY_PROCESS_NUMBER] ~= nil then
        proTable.process_number = string2Int(luaTable[keyTable.KEY_PROCESS_NUMBER])
    end
    ...
    if luaTable[keyTable.KEY_COOKING_STRENGTH] ~= nil then
        proTable.cooking_strength = string2Int(luaTable[keyTable.KEY_COOKING_STRENGTH])
    end
    if luaTable[keyTable.KEY_COOKING_TIME_1] ~= nil then
        proTable.cooking_time_1 = string2Int(luaTable[keyTable.KEY_COOKING_TIME_1])
    end
    if luaTable[keyTable.KEY_COOKING_TIME_2] ~= nil then
        proTable.cooking_time_2 = string2Int(luaTable[keyTable.KEY_COOKING_TIME_2])
    end
    if luaTable[keyTable.KEY_COOKING_TEMP_OR_POWER_1] ~= nil then
        proTable.cooking_temp_or_power_1 = string2Int(luaTable[keyTable.KEY_COOKING_TEMP_OR_POWER_1])
    end
    if luaTable[keyTable.KEY_COOKING_TEMP_OR_POWER_2] ~= nil then
        proTable.cooking_temp_or_power_2 = string2Int(luaTable[keyTable.KEY_COOKING_TEMP_OR_POWER_2])
    end
end
local function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if dataType == 0x02 then
        if messageBytes[0] == 0x00 then
            ...
            if getMenuCodeType(proTable.menu_code) == 1 then
                proTable.cooking_time_1 = messageBytes[7] + messageBytes[6] * 256
                proTable.cooking_time_2 = messageBytes[9] + messageBytes[8] * 256
                proTable.cooking_temp_or_power_1 = messageBytes[11] + messageBytes[10] * 256
                proTable.cooking_temp_or_power_2 = messageBytes[13] + messageBytes[12] * 256
            else
                proTable.cooking_strength = messageBytes[7] + messageBytes[6] * 256
            end
        end
    end
    if (dataType == 0x04) or (dataType == 0x03) then
        if messageBytes[0] == 0x00 then
            ...
            proTable.cooking_state = bit.band(bit.rshift(messageBytes[19], 2), 0x07)
            ...
            proTable.ferment_thaw_bakery_high_temperature = bit.band(messageBytes[20], 0x04)
            proTable.low_temp_steam_high_temperature = bit.band(messageBytes[20], 0x08)
            ...
            proTable.during_cooking_finish_change = bit.band(messageBytes[21], 0x10)
            proTable.remaining_time_finish_change = bit.band(messageBytes[21], 0x20)
            proTable.cooking_temperature_finish_change = bit.band(messageBytes[21], 0x40)
            ...
            proTable.chamber_light_status_door_open = bit.band(messageBytes[25], 0x01)
            proTable.chamber_light_status_preheating = bit.band(messageBytes[25], 0x02)
            proTable.chamber_light_status_cooking = bit.band(messageBytes[25], 0x04)
    ...
    if (dataType == 0x03) or (dataType == 0x0A) then
        if (messageBytes[0] == 0x21) or (messageBytes[0] == 0xFE) then
            ...
            proTable.time_last_start = messageBytes[1072] * 65536 + messageBytes[1071] * 256 + messageBytes[1070]
            ...
            proTable.pc_board_operating_time = messageBytes[1081] * 65536
                + messageBytes[1080] * 256
                + messageBytes[1079]
            proTable.microwave_operating_time = messageBytes[1084] * 65536
                + messageBytes[1083] * 256
                + messageBytes[1082]
            proTable.oven_operating_time = messageBytes[1087] * 65536 + messageBytes[1086] * 256 + messageBytes[1085]
            proTable.steam_operating_time = messageBytes[1090] * 65536 + messageBytes[1089] * 256 + messageBytes[1088]
            proTable.display_operating_time = messageBytes[1093] * 65536 + messageBytes[1092] * 256 + messageBytes[1091]
            proTable.door_opening_count = messageBytes[1096] * 65536 + messageBytes[1095] * 256 + messageBytes[1094]
        end
    end
end
local function assembleJsonByGlobalProperty()
    ...
    if subDataType == 0x00 then
        ...
        streams[keyTable.KEY_COOKING_TEMP_OR_POWER_1] = proTable.cooking_temp_or_power_1
        streams[keyTable.KEY_COOKING_TEMP_OR_POWER_2] = proTable.cooking_temp_or_power_2
        ...
        streams[keyTable.KEY_E2PROM_START_ADDRESS] = proTable.e2prom_start_address
        streams[keyTable.KEY_COOKING_SETTING_DATA] = proTable.cooking_setting_data
        streams[keyTable.KEY_TIME_LAST_START] = proTable.time_last_start
        streams[keyTable.KEY_OVEN_LOW_TEMPERATURE_CIRCUIT] = proTable.oven_low_temperature_circuit
        streams[keyTable.KEY_OVEN_HIGH_TEMPERATURE_CIRCUIT] = proTable.oven_high_temperature_circuit
        streams[keyTable.KEY_STEAM_LOW_TEMPERATURE_CIRCUIT] = proTable.steam_low_temperature_circuit
        streams[keyTable.KEY_STEAM_HIGH_TEMPERATURE_CIRCUIT] = proTable.steam_high_temperature_circuit
        streams[keyTable.KEY_PANEL_THERMISTOR_DATA] = proTable.panel_thermistor_data
        streams[keyTable.KEY_POWER_SUPPLY_VOLTAGE] = proTable.power_supply_voltage
        streams[keyTable.KEY_PC_BOARD_OPERATING_TIME] = proTable.pc_board_operating_time
        streams[keyTable.KEY_MICROWAVE_OPERATING_TIME] = proTable.microwave_operating_time
        streams[keyTable.KEY_OVEN_OPERATING_TIME] = proTable.oven_operating_time
        streams[keyTable.KEY_STEAM_OPERATING_TIME] = proTable.steam_operating_time
        streams[keyTable.KEY_DISPLAY_OPERATING_TIME] = proTable.display_operating_time
        streams[keyTable.KEY_DOOR_OPENING_COUNT] = proTable.door_opening_count
    end
    return streams
end
local function jsonToData(jsonCmdStr)
    ...
    if control then
        ...
        if getMenuCodeType(proTable.menu_code) == 1 then
            bodyBytes[6] = bit.band(bit.rshift(proTable.cooking_time_1, 8), 0xff)
            bodyBytes[7] = bit.band(proTable.cooking_time_1, 0xff)
            bodyBytes[8] = bit.band(bit.rshift(proTable.cooking_time_2, 8), 0xff)
            bodyBytes[9] = bit.band(proTable.cooking_time_2, 0xff)
            bodyBytes[10] = bit.band(bit.rshift(proTable.cooking_temp_or_power_1, 8), 0xff)
            bodyBytes[11] = bit.band(proTable.cooking_temp_or_power_1, 0xff)
            bodyBytes[12] = bit.band(bit.rshift(proTable.cooking_temp_or_power_2, 8), 0xff)
            ...
            if proTable.menu_code == 0x0000 then
                ...
                bodyBytes[6] = bit.band(bit.rshift(proTable.cooking_strength, 8), 0xff)
...
local function dataToJson(jsonStr)
...
function _M.dataToJson(jsonCmd)
    return dataToJson(jsonCmd)
end
function _M.jsonToData(jsonCmd)
    return jsonToData(jsonCmd)
end
return _M
