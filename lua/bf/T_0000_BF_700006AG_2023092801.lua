...
local function setSystemTime(control, bodyBytes)
    ...
    if control["sys_second"] ~= nil or control["sys_minute"] ~= nil or control["sys_hour"] ~= nil then
...
function jsonToData(jsonCmdStr)
    ...
    if control then
        ...
        elseif
            control["work_status"] ~= nil
            or control["lock"] ~= nil
            or control["furnace_light"] ~= nil
            or control["power"] ~= nil
            or control["hot_wind"] ~= nil
            or control["door"] ~= nil
            or control["volume"] ~= nil
            or control["screen_luminance"] ~= nil
            or control["water_box"] ~= nil
        then
        ...
        elseif
            ...
            or control["steam_set"] ~= nil
            or control["fire_power_set"] ~= nil
            or control["probe_temp_set"] ~= nil
            or control["temp_above_set"] ~= nil
            or control["temp_underside_set"] ~= nil
        then
...
local function totalState(jsonTable, megBodys)
    ...
    if temperature == 0 then temperature = tonumber(megBodys[25] .. megBodys[26], 16) end
    jsonTable["temperature"] = temperature
    jsonTable["temperature_above"] = tonumber(megBodys[23] .. megBodys[24], 16)
    jsonTable["temperature_underside"] = tonumber(megBodys[25] .. megBodys[26], 16)
    ...
    if cur_temperature == 0 then cur_temperature = tonumber(megBodys[37] .. megBodys[38], 16) end
    jsonTable["cur_temperature"] = cur_temperature
    jsonTable["cur_temperature_above"] = tonumber(megBodys[35] .. megBodys[36], 16)
    jsonTable["cur_temperature_underside"] = tonumber(megBodys[37] .. megBodys[38], 16)
    jsonTable["cur_probe_temperature"] = tonumber(megBodys[39] .. megBodys[40], 16)
    ...
    if b57 ~= nil and b58 ~= nil and b59 ~= nil then
        ...
        cbsVersionInt = tonumber(b57, 16) * (16 ^ 4) + tonumber(b58, 16) * (16 ^ 2) + tonumber(b59, 16)
    end
    ...
    if b68 ~= nil then
        ...
        if clean_sink_ponding == "1" then jsonTable["clean_sink_ponding"] = 1 end
        local dissipate_heat = getByteBit(b68, 1)
        if dissipate_heat == "1" then jsonTable["dissipate_heat"] = "work" end
    end
    if cbsVersionInt < 65796 and jsonTable["work_mode"] ~= "auto_menu" then jsonTable["cloudmenuid"] = 0 end
end
