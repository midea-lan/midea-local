...
local function jsonToModel(controlJson)
    ...
    elseif myTable["controlType"] == 0x02 then
        --定时段位
        if controlCmd["timer_amount"] ~= nil then myTable["timer_amount"] = string2Int(controlCmd["timer_amount"]) end
        --定时1是否有效
    ...
    elseif myTable["controlType"] == 0x08 then
        ...
        if controlCmd["zero_cold_water"] ~= nil then myTable["zero_cold_water"] = controlCmd["zero_cold_water"] end
...
function dataToJson(jsonCmd)
    ...
    then
        --日定时查询
        --定时段数
        if myTable["timer_amount"] ~= nil then streams["timer_amount"] = myTable["timer_amount"] end
        --日定时1是否有效
    ...
    elseif msgSubType == 0xB0 or msgSubType == 0xB5 or msgSubType == 0xB1 then
        ...
        if myTable["huge_water_amount"] ~= nil then streams["huge_water_amount"] = myTable["huge_water_amount"] end
        if myTable["out_machine_clean"] ~= nil then streams["out_machine_clean"] = myTable["out_machine_clean"] end
        if myTable["mid_temp_keep_warm"] ~= nil then streams["mid_temp_keep_warm"] = myTable["mid_temp_keep_warm"] end
        if myTable["zero_cold_water"] ~= nil then streams["zero_cold_water"] = myTable["zero_cold_water"] end
        if myTable["ai_zero_cold_water"] ~= nil then streams["ai_zero_cold_water"] = myTable["ai_zero_cold_water"] end
        if myTable["mode_type"] ~= nil then streams["mode_type"] = myTable["mode_type"] end
        if myTable["sterilize_effect_enable"] ~= nil then
            streams["sterilize_effect_enable"] = myTable["sterilize_effect_enable"]
        end
        if myTable["appointment_timer"] ~= nil then streams["appointment_timer"] = myTable["appointment_timer"] end
    end
