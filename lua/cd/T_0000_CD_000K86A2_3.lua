...
local function jsonToModel(controlJson)
    ...
    myTable["controlType"] = 0x01; if (controlCmd["control_type"] ~= nil) then
        myTable["controlType"] = string2Int(
            controlCmd["control_type"])
    end
    if (myTable["controlType"] == 0x01) then
        ...
        if controlCmd[KEY_MODE] ~= nil then if controlCmd[KEY_MODE] == "energy" then myTable["modeValue"] = 0x01 elseif controlCmd[KEY_MODE] == "standard" then myTable["modeValue"] = 0x02 elseif controlCmd[KEY_MODE] == "compatibilizing" then myTable["modeValue"] = 0x03 elseif controlCmd[KEY_MODE] == "smart" then myTable["modeValue"] = 0x04 end end
        ...
        if controlCmd["open_ptc"] ~= nil then if controlCmd["open_ptc"] == "0" then myTable["openPTC"] = 0x00 elseif controlCmd["open_ptc"] == "1" then myTable["openPTC"] = 0x01 elseif controlCmd["open_ptc"] == "2" then myTable["openPTC"] = 0x02 end end
    ...
    elseif (myTable["controlType"] == 0x02) then
        if controlCmd["timer1_effect"] ~= nil then if controlCmd["timer1_effect"] == VALUE_FUNCTION_ON then myTable["timer1Effect"] = 0x01 elseif controlCmd["timer1_effect"] == VALUE_FUNCTION_OFF then myTable["timer1Effect"] = 0 end end
        if controlCmd["timer2_effect"] ~= nil then if controlCmd["timer2_effect"] == VALUE_FUNCTION_ON then myTable["timer2Effect"] = 0x02 elseif controlCmd["timer2_effect"] == VALUE_FUNCTION_OFF then myTable["timer2Effect"] = 0 end end
        if controlCmd["timer1_openHour"] ~= nil then
            myTable["timer1OpenHour"] = string2Int(controlCmd
                ["timer1_openHour"])
        end
        if controlCmd["timer1_openhour"] ~= nil then
            myTable["timer1OpenHour"] = string2Int(controlCmd
        ...
        if controlCmd["timer1_openMin"] ~= nil then myTable["timer1OpenMin"] = string2Int(controlCmd["timer1_openMin"]) end
        if controlCmd["timer1_openmin"] ~= nil then myTable["timer1OpenMin"] = string2Int(controlCmd["timer1_openmin"]) end
        if controlCmd["timer1_closeHour"] ~= nil then
            myTable["timer1CloseHour"] = string2Int(controlCmd
                ["timer1_closeHour"])
        end
        if controlCmd["timer1_closehour"] ~= nil then
            myTable["timer1CloseHour"] = string2Int(controlCmd
        ...
        if controlCmd["timer1_closeMin"] ~= nil then
            myTable["timer1CloseMin"] = string2Int(controlCmd
                ["timer1_closeMin"])
        end
        if controlCmd["timer1_closemin"] ~= nil then
            myTable["timer1CloseMin"] = string2Int(controlCmd
        ...
        if controlCmd["timer2_openHour"] ~= nil then
            myTable["timer2OpenHour"] = string2Int(controlCmd
                ["timer2_openHour"])
        end
        if controlCmd["timer2_openhour"] ~= nil then
            myTable["timer2OpenHour"] = string2Int(controlCmd
        ...
        if controlCmd["timer2_openMin"] ~= nil then myTable["timer2OpenMin"] = string2Int(controlCmd["timer2_openMin"]) end
        if controlCmd["timer2_openmin"] ~= nil then myTable["timer2OpenMin"] = string2Int(controlCmd["timer2_openmin"]) end
        if controlCmd["timer2_closeHour"] ~= nil then
            myTable["timer2CloseHour"] = string2Int(controlCmd
                ["timer2_closeHour"])
        end
        if controlCmd["timer2_closehour"] ~= nil then
            myTable["timer2CloseHour"] = string2Int(controlCmd
        ...
        if controlCmd["timer2_closeMin"] ~= nil then
            myTable["timer2CloseMin"] = string2Int(controlCmd
                ["timer2_closeMin"])
        end
        if controlCmd["timer2_closemin"] ~= nil then
            myTable["timer2CloseMin"] = string2Int(controlCmd
    ...
    elseif (myTable["controlType"] == 0x03) then
        if controlCmd["order1_effect"] ~= nil then if controlCmd["order1_effect"] == VALUE_FUNCTION_ON then myTable["order1Effect"] = 0x01 elseif controlCmd["order1_effect"] == VALUE_FUNCTION_OFF then myTable["order1Effect"] = 0 end end
        if controlCmd["order2_effect"] ~= nil then if controlCmd["order2_effect"] == VALUE_FUNCTION_ON then myTable["order2Effect"] = 0x01 elseif controlCmd["order2_effect"] == VALUE_FUNCTION_OFF then myTable["order2Effect"] = 0 end end
        if controlCmd["order1_timeHour"] ~= nil then
            myTable["order1TimeHour"] = string2Int(controlCmd
                ["order1_timeHour"])
        end
        if controlCmd["order1_timehour"] ~= nil then
            myTable["order1TimeHour"] = string2Int(controlCmd
        ...
        if controlCmd["order1_timeMin"] ~= nil then myTable["order1TimeMin"] = string2Int(controlCmd["order1_timeMin"]) end
        if controlCmd["order1_timemin"] ~= nil then myTable["order1TimeMin"] = string2Int(controlCmd["order1_timemin"]) end
        if controlCmd["order1_stoptimeHour"] ~= nil then
            myTable["order1StopTimeHour"] = string2Int(controlCmd
                ["order1_stoptimeHour"])
        end
        if controlCmd["order1_stoptimehour"] ~= nil then
            myTable["order1StopTimeHour"] = string2Int(controlCmd
        ...
        if controlCmd["order1_stoptimeMin"] ~= nil then
            myTable["order1StopTimeMin"] = string2Int(controlCmd
                ["order1_stoptimeMin"])
        end
        if controlCmd["order1_stoptimemin"] ~= nil then
            myTable["order1StopTimeMin"] = string2Int(controlCmd
        ...
        if controlCmd["order2_timeHour"] ~= nil then
            myTable["order2TimeHour"] = string2Int(controlCmd
                ["order2_timeHour"])
        end
        if controlCmd["order2_timehour"] ~= nil then
            myTable["order2TimeHour"] = string2Int(controlCmd
        ...
        if controlCmd["order2_timeMin"] ~= nil then myTable["order2TimeMin"] = string2Int(controlCmd["order2_timeMin"]) end
        if controlCmd["order2_timemin"] ~= nil then myTable["order2TimeMin"] = string2Int(controlCmd["order2_timemin"]) end
        if controlCmd["order2_stoptimeHour"] ~= nil then
            myTable["order2StopTimeHour"] = string2Int(controlCmd
                ["order2_stoptimeHour"])
        end
        if controlCmd["order2_stoptimehour"] ~= nil then
            myTable["order2StopTimeHour"] = string2Int(controlCmd
        ...
        if controlCmd["order2_stoptimeMin"] ~= nil then
            myTable["order2StopTimeMin"] = string2Int(controlCmd
                ["order2_stoptimeMin"])
        end
        if controlCmd["order2_stoptimemin"] ~= nil then
            myTable["order2StopTimeMin"] = string2Int(controlCmd
...
local function binToModel(binData)
    ...
    if (myTable["dataType"] == 0x03 or myTable["dataType"] == 0x05) then
        ...
        myTable["smartMode"]           = bit.rshift(bit.band(messageBytes[28], 0x20), 5)
...
function jsonToData(jsonCmd)
    ...
    elseif (control) then
        ...
        if (myTable["controlType"] == 0x01) then
            ...
            bodyBytes[8] = bit.bor(
                bit.bor(bit.bor(bit.bor(myTable["waterPump"], myTable["refrigerantRecycling"]), myTable["defrost"]),
                    myTable["mute"]), myTable["openPTCTemp"])
        ...
        elseif (myTable["controlType"] == 0x02) then
            ...
            bodyBytes[4] = myTable["timer1OpenHour"]; bodyBytes[5] = myTable["timer1OpenMin"]; bodyBytes[6] = myTable
                ["timer1CloseHour"]; bodyBytes[7] = myTable["timer1CloseMin"]; bodyBytes[8] = 45; bodyBytes[9] = myTable
                ["timer2OpenHour"]; bodyBytes[10] = myTable["timer2OpenMin"]; bodyBytes[11] = myTable["timer2CloseHour"]; bodyBytes[12] =
                myTable["timer2CloseMin"]; bodyBytes[13] = 45; for i = 14, 23 do bodyBytes[i] = 0 end
            infoM = getTotalMsg(bodyBytes, BYTE_CONTROL_REQUEST)
        elseif (myTable["controlType"] == 0x03) then
...
function dataToJson(jsonCmd)
    ...
    for i = 0, bodyLength do bodyBytes[i] = msgBytes[i + BYTE_PROTOCOL_LENGTH] end
