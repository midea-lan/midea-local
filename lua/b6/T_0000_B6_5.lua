            ...
            if ctrl["light"] then
                if ctrl["light"] == VALUE_ON then
                    ...
                    if ctrl["lightness"] then
                        ...
                        if lightness ~= 0x00 then cmd[13] = lightness * 10 end
                    end
...
local function queryAllCmdToJson(json, cmd)
    ...
    if cmd[13] == 0x16 and cmd[34] + cmd[35] ~= 0 then json["wind_pressure"] = cmd[34] * 256 + cmd[35] end
    return json
end
local function queryErrorCmdToJson(json, cmd)
    ...
    if cmd[15] ~= 0 or cmd[16] ~= 0 then json["error_type"] = "inverter_error" end
    return json
end
