...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    if
        luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == nil
        or luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == ""
        or luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == VALUE_FUNCTION_SETTING
        or luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == VALUE_FUNCTION_QUERY
    then
        cmdType = 0x01
    elseif
        luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == VALUE_TIMING_SETTING
        or luaTable[keytable["KEY_SUB_PACKET_TYPE"]] == VALUE_TIMING_QUERY
    then
        cmdType = 0x02
    end
    if luaTable[keytable["KEY_LIGHT_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_LIGHT_MODE"]], VALUE_FUNCTION_MAIN_LIGHT) ~= nil then
    ...
    if luaTable[keytable["KEY_MAIN_LIGHT_BRIGHTNESS"]] ~= nil then
        valuetable["MAIN_LIGHT_BRIGHTNESS"] = string2Int(luaTable[keytable["KEY_MAIN_LIGHT_BRIGHTNESS"]])
    end
    if luaTable[keytable["KEY_LIGHT_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_LIGHT_MODE"]], VALUE_FUNCTION_NIGHT_LIGHT) ~= nil then
    ...
    if luaTable[keytable["KEY_NIGHT_LIGHT_BRIGHTNESS"]] ~= nil then
        valuetable["NIGHT_LIGHT_BRIGHTNESS"] = string2Int(luaTable[keytable["KEY_NIGHT_LIGHT_BRIGHTNESS"]])
    end
    if luaTable[keytable["KEY_RADAR_INDUCTION_ENABLE"]] == VALUE_FUNCTION_ON then
        valuetable["RADAR_INDUCTION_ENABLE"] = 0x01
    elseif luaTable[keytable["KEY_RADAR_INDUCTION_ENABLE"]] == VALUE_FUNCTION_OFF then
        valuetable["RADAR_INDUCTION_ENABLE"] = 0x00
    end
    if luaTable[keytable["KEY_RADAR_INDUCTION_CLOSING_TIME"]] ~= nil then
        valuetable["RADAR_INDUCTION_CLOSING_TIME"] = string2Int(luaTable[keytable["KEY_RADAR_INDUCTION_CLOSING_TIME"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_HEATING) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_HEATING) ~= nil then
            valuetable["HEATING_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_HEATING) ~= nil then
            valuetable["HEATING_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_HEATING_TEMPERATURE"]] ~= nil then
        valuetable["HEATING_TEMPERATURE"] = string2Int(luaTable[keytable["KEY_HEATING_TEMPERATURE"]])
    end
    if luaTable[keytable["KEY_HEATING_SPEED"]] ~= nil then
        valuetable["HEATING_SPEED"] = string2Int(luaTable[keytable["KEY_HEATING_SPEED"]])
    end
    if luaTable[keytable["KEY_HEATING_DIRECTION"]] ~= nil then
        valuetable["HEATING_DIRECTION"] = string2Int(luaTable[keytable["KEY_HEATING_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_BATH) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_BATH) ~= nil then
            valuetable["BATH_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_BATH) ~= nil then
            valuetable["BATH_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_BATH_HEATING_TIME"]] ~= nil then
        valuetable["BATH_HEATING_TIME"] = string2Int(luaTable[keytable["KEY_BATH_HEATING_TIME"]])
    end
    if luaTable[keytable["KEY_BATH_TEMPERATURE"]] ~= nil then
        valuetable["BATH_TEMPERATURE"] = string2Int(luaTable[keytable["KEY_BATH_TEMPERATURE"]])
    end
    if luaTable[keytable["KEY_BATH_SPEED"]] ~= nil then
        valuetable["BATH_SPEED"] = string2Int(luaTable[keytable["KEY_BATH_SPEED"]])
    end
    if luaTable[keytable["KEY_BATH_DIRECTION"]] ~= nil then
        valuetable["BATH_DIRECTION"] = string2Int(luaTable[keytable["KEY_BATH_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_SOFT_WIND) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_SOFT_WIND) ~= nil then
            valuetable["SOFT_WIND_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_SOFT_WIND) ~= nil then
            valuetable["SOFT_WIND_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_SOFT_WIND_HEATING_TIME"]] ~= nil then
        valuetable["SOFT_WIND_HEATING_TIME"] = string2Int(luaTable[keytable["KEY_SOFT_WIND_HEATING_TIME"]])
    end
    if luaTable[keytable["KEY_SOFT_WIND_TEMPERATURE"]] ~= nil then
        valuetable["SOFT_WIND_TEMPERATURE"] = string2Int(luaTable[keytable["KEY_SOFT_WIND_TEMPERATURE"]])
    end
    if luaTable[keytable["KEY_SOFT_WIND_SPEED"]] ~= nil then
        valuetable["SOFT_WIND_SPEED"] = string2Int(luaTable[keytable["KEY_SOFT_WIND_SPEED"]])
    end
    if luaTable[keytable["KEY_SOFT_WIND_DIRECTION"]] ~= nil then
        valuetable["SOFT_WIND_DIRECTION"] = string2Int(luaTable[keytable["KEY_SOFT_WIND_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_VENTILATION) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_VENTILATION) ~= nil then
            valuetable["VENTILATION_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_VENTILATION) ~= nil then
            valuetable["VENTILATION_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_VENTILATION_SPEED"]] ~= nil then
        valuetable["VENTILATION_SPEED"] = string2Int(luaTable[keytable["KEY_VENTILATION_SPEED"]])
    end
    if luaTable[keytable["KEY_VENTILATION_DIRECTION"]] ~= nil then
        valuetable["VENTILATION_DIRECTION"] = string2Int(luaTable[keytable["KEY_VENTILATION_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_DRYING) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_DRYING) ~= nil then
            valuetable["DRYING_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_DRYING) ~= nil then
            valuetable["DRYING_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_DRYING_TIME"]] ~= nil then
        valuetable["DRYING_TIME"] = string2Int(luaTable[keytable["KEY_DRYING_TIME"]])
    end
    if luaTable[keytable["KEY_DRYING_TEMPERATURE"]] ~= nil then
        valuetable["DRYING_TEMPERATURE"] = string2Int(luaTable[keytable["KEY_DRYING_TEMPERATURE"]])
    end
    if luaTable[keytable["KEY_DRYING_SPEED"]] ~= nil then
        valuetable["DRYING_SPEED"] = string2Int(luaTable[keytable["KEY_DRYING_SPEED"]])
    end
    if luaTable[keytable["KEY_DRYING_DIRECTION"]] ~= nil then
        valuetable["DRYING_DIRECTION"] = string2Int(luaTable[keytable["KEY_DRYING_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE"]], VALUE_FUNCTION_BLOWING) ~= nil then
    ...
    if luaTable[keytable["KEY_MODE_ENABLE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_ENABLE"]], VALUE_FUNCTION_BLOWING) ~= nil then
            valuetable["BLOWING_ENABLE"] = 0x01
        end
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_BLOWING) ~= nil then
            valuetable["BLOWING_ENABLE"] = 0x00
        end
    end
    if luaTable[keytable["KEY_BLOWING_SPEED"]] ~= nil then
        valuetable["BLOWING_SPEED"] = string2Int(luaTable[keytable["KEY_BLOWING_SPEED"]])
    end
    if luaTable[keytable["KEY_BLOWING_DIRECTION"]] ~= nil then
        valuetable["BLOWING_DIRECTION"] = string2Int(luaTable[keytable["KEY_BLOWING_DIRECTION"]])
    end
    if luaTable[keytable["KEY_MODE_CLOSE"]] ~= nil then
        if string.find(luaTable[keytable["KEY_MODE_CLOSE"]], VALUE_FUNCTION_CLOSE_ALL) ~= nil then
            valuetable["HEATING_ENABLE"] = 0x00
            valuetable["BATH_ENABLE"] = 0x00
            valuetable["SOFT_WIND_ENABLE"] = 0x00
            valuetable["VENTILATION_ENABLE"] = 0x00
    ...
    if luaTable[keytable["KEY_DELAY_TIME"]] ~= nil then
        valuetable["DELAY_TIME"] = string2Int(luaTable[keytable["KEY_DELAY_TIME"]])
    end
    ...
    if luaTable[keytable["KEY_SMELLY_THRESHOLD"]] ~= nil then
        valuetable["SMELLY_THRESHOLD"] = string2Int(luaTable[keytable["KEY_SMELLY_THRESHOLD"]])
    end
    if luaTable[keytable["KEY_AUTO_DEHUMIDIFICATION"]] == VALUE_FUNCTION_ON then
        valuetable["AUTO_DEHUMIDIFICATION"] = 0x01
    elseif luaTable[keytable["KEY_AUTO_DEHUMIDIFICATION"]] == VALUE_FUNCTION_OFF then
        valuetable["AUTO_DEHUMIDIFICATION"] = 0x00
    end
    if luaTable[keytable["KEY_DEHUMIDITY_THRESHOLD"]] ~= nil then
        valuetable["DEHUMIDITY_THRESHOLD"] = string2Int(luaTable[keytable["KEY_DEHUMIDITY_THRESHOLD"]])
    end
    if luaTable[keytable["KEY_DEHUMIDITY_TIME"]] ~= nil then
        valuetable["DEHUMIDITY_TIME"] = string2Int(luaTable[keytable["KEY_DEHUMIDITY_TIME"]])
    end
    if luaTable[keytable["KEY_DEHUMIDITY_INTERVAL_TIME"]] ~= nil then
        valuetable["DEHUMIDITY_INTERVAL_TIME"] = string2Int(luaTable[keytable["KEY_DEHUMIDITY_INTERVAL_TIME"]])
    end
    if luaTable[keytable["KEY_DEHUMIDITY_DIRECTION"]] ~= nil then
        valuetable["DEHUMIDITY_DIRECTION"] = string2Int(luaTable[keytable["KEY_DEHUMIDITY_DIRECTION"]])
    end
    ...
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_NIGHT_LIGHT then
        valuetable["TIMING1_FUNCTION"] = 0x02
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_HEATING_HIGHT then
        valuetable["TIMING1_FUNCTION"] = 0x03
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_HEATING_LOW then
    ...
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_VENTILATION then
    ...
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_DRYING_SAFE_POWER then
        valuetable["TIMING1_FUNCTION"] = 0x08
    elseif luaTable[keytable["KEY_TIMING1_FUNCTION"]] == VALUE_FUNCTION_DRYING_FAST then
        valuetable["TIMING1_FUNCTION"] = 0x09
    end
    if luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]] ~= nil then
        valuetable["TIMING1_DATE_REPEAT"] = 0x00
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "sunday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x01)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "monday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x02)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "thursday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x04)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "wednesday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x08)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "tuesday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x10)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "friday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x20)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "saturday") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x40)
        end
        if string.find(luaTable[keytable["KEY_TIMING1_DATE_REPEAT"]], "once") ~= nil then
            valuetable["TIMING1_DATE_REPEAT"] = bit.bor(valuetable["TIMING1_DATE_REPEAT"], 0x80)
        end
    end
    if luaTable[keytable["KEY_TIMING1_OPENINT_TIME"]] ~= nil then
        valuetable["TIMING1_OPENINT_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING1_OPENINT_TIME"]], 1, 2))
        valuetable["TIMING1_OPENINT_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING1_OPENINT_TIME"]], 4, 5))
    end
    if luaTable[keytable["KEY_TIMING1_CLOSING_TIME"]] ~= nil then
        valuetable["TIMING1_CLOSING_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING1_CLOSING_TIME"]], 1, 2))
        valuetable["TIMING1_CLOSING_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING1_CLOSING_TIME"]], 4, 5))
    end
    ...
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_NIGHT_LIGHT then
        valuetable["TIMING2_FUNCTION"] = 0x02
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_HEATING_HIGHT then
        valuetable["TIMING2_FUNCTION"] = 0x03
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_HEATING_LOW then
    ...
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_VENTILATION then
    ...
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_DRYING_SAFE_POWER then
        valuetable["TIMING2_FUNCTION"] = 0x08
    elseif luaTable[keytable["KEY_TIMING2_FUNCTION"]] == VALUE_FUNCTION_DRYING_FAST then
        valuetable["TIMING2_FUNCTION"] = 0x09
    end
    if luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]] ~= nil then
        valuetable["TIMING2_DATE_REPEAT"] = 0x00
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "sunday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x01)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "monday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x02)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "thursday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x04)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "wednesday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x08)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "tuesday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x10)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "friday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x20)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "saturday") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x40)
        end
        if string.find(luaTable[keytable["KEY_TIMING2_DATE_REPEAT"]], "once") ~= nil then
            valuetable["TIMING2_DATE_REPEAT"] = bit.bor(valuetable["TIMING2_DATE_REPEAT"], 0x80)
        end
    end
    if luaTable[keytable["KEY_TIMING2_OPENINT_TIME"]] ~= nil then
        valuetable["TIMING2_OPENINT_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING2_OPENINT_TIME"]], 1, 2))
        valuetable["TIMING2_OPENINT_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING2_OPENINT_TIME"]], 4, 5))
    end
    if luaTable[keytable["KEY_TIMING2_CLOSING_TIME"]] ~= nil then
        valuetable["TIMING2_CLOSING_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING2_CLOSING_TIME"]], 1, 2))
        valuetable["TIMING2_CLOSING_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING2_CLOSING_TIME"]], 4, 5))
    end
    ...
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_NIGHT_LIGHT then
        valuetable["TIMING3_FUNCTION"] = 0x02
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_HEATING_HIGHT then
        valuetable["TIMING3_FUNCTION"] = 0x03
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_HEATING_LOW then
    ...
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_VENTILATION then
    ...
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_DRYING_SAFE_POWER then
        valuetable["TIMING3_FUNCTION"] = 0x08
    elseif luaTable[keytable["KEY_TIMING3_FUNCTION"]] == VALUE_FUNCTION_DRYING_FAST then
        valuetable["TIMING3_FUNCTION"] = 0x09
    end
    if luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]] ~= nil then
        valuetable["TIMING3_DATE_REPEAT"] = 0x00
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "sunday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x01)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "monday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x02)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "thursday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x04)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "wednesday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x08)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "tuesday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x10)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "friday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x20)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "saturday") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x40)
        end
        if string.find(luaTable[keytable["KEY_TIMING3_DATE_REPEAT"]], "once") ~= nil then
            valuetable["TIMING3_DATE_REPEAT"] = bit.bor(valuetable["TIMING3_DATE_REPEAT"], 0x80)
        end
    end
    if luaTable[keytable["KEY_TIMING3_OPENINT_TIME"]] ~= nil then
        valuetable["TIMING3_OPENINT_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING3_OPENINT_TIME"]], 1, 2))
        valuetable["TIMING3_OPENINT_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING3_OPENINT_TIME"]], 4, 5))
    end
    if luaTable[keytable["KEY_TIMING3_CLOSING_TIME"]] ~= nil then
        valuetable["TIMING3_CLOSING_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING3_CLOSING_TIME"]], 1, 2))
        valuetable["TIMING3_CLOSING_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING3_CLOSING_TIME"]], 4, 5))
    end
    ...
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_NIGHT_LIGHT then
        valuetable["TIMING4_FUNCTION"] = 0x02
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_HEATING_HIGHT then
        valuetable["TIMING4_FUNCTION"] = 0x03
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_HEATING_LOW then
    ...
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_VENTILATION then
    ...
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_DRYING_SAFE_POWER then
        valuetable["TIMING4_FUNCTION"] = 0x08
    elseif luaTable[keytable["KEY_TIMING4_FUNCTION"]] == VALUE_FUNCTION_DRYING_FAST then
        valuetable["TIMING4_FUNCTION"] = 0x09
    end
    if luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]] ~= nil then
        valuetable["TIMING4_DATE_REPEAT"] = 0x00
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "sunday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x01)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "monday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x02)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "thursday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x04)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "wednesday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x08)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "tuesday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x10)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "friday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x20)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "saturday") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x40)
        end
        if string.find(luaTable[keytable["KEY_TIMING4_DATE_REPEAT"]], "once") ~= nil then
            valuetable["TIMING4_DATE_REPEAT"] = bit.bor(valuetable["TIMING4_DATE_REPEAT"], 0x80)
        end
    end
    if luaTable[keytable["KEY_TIMING4_OPENINT_TIME"]] ~= nil then
        valuetable["TIMING4_OPENINT_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING4_OPENINT_TIME"]], 1, 2))
        valuetable["TIMING4_OPENINT_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING4_OPENINT_TIME"]], 4, 5))
    end
    if luaTable[keytable["KEY_TIMING4_CLOSING_TIME"]] ~= nil then
        valuetable["TIMING4_CLOSING_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING4_CLOSING_TIME"]], 1, 2))
        valuetable["TIMING4_CLOSING_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING4_CLOSING_TIME"]], 4, 5))
    end
    ...
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_NIGHT_LIGHT then
        valuetable["TIMING5_FUNCTION"] = 0x02
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_HEATING_HIGHT then
        valuetable["TIMING5_FUNCTION"] = 0x03
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_HEATING_LOW then
    ...
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_VENTILATION then
    ...
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_DRYING_SAFE_POWER then
        valuetable["TIMING5_FUNCTION"] = 0x08
    elseif luaTable[keytable["KEY_TIMING5_FUNCTION"]] == VALUE_FUNCTION_DRYING_FAST then
        valuetable["TIMING5_FUNCTION"] = 0x09
    end
    if luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]] ~= nil then
        valuetable["TIMING5_DATE_REPEAT"] = 0x00
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "sunday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x01)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "monday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x02)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "thursday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x04)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "wednesday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x08)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "tuesday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x10)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "friday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x20)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "saturday") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x40)
        end
        if string.find(luaTable[keytable["KEY_TIMING5_DATE_REPEAT"]], "once") ~= nil then
            valuetable["TIMING5_DATE_REPEAT"] = bit.bor(valuetable["TIMING5_DATE_REPEAT"], 0x80)
        end
    end
    if luaTable[keytable["KEY_TIMING5_OPENINT_TIME"]] ~= nil then
        valuetable["TIMING5_OPENINT_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING5_OPENINT_TIME"]], 1, 2))
        valuetable["TIMING5_OPENINT_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING5_OPENINT_TIME"]], 4, 5))
    end
    if luaTable[keytable["KEY_TIMING5_CLOSING_TIME"]] ~= nil then
        valuetable["TIMING5_CLOSING_TIME_HOUR"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING5_CLOSING_TIME"]], 1, 2))
        valuetable["TIMING5_CLOSING_TIME_MIN"] =
            string2Int(string.sub(luaTable[keytable["KEY_TIMING5_CLOSING_TIME"]], 4, 5))
    end
end
local function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if cmdType == 0x01 then
        if messageBytes[1] ~= nil then valuetable["MAIN_LIGHT_ENABLE"] = messageBytes[1] end
        if messageBytes[2] ~= nil then valuetable["MAIN_LIGHT_BRIGHTNESS"] = messageBytes[2] end
        if messageBytes[3] ~= nil then valuetable["NIGHT_LIGHT_ENABLE"] = messageBytes[3] end
        if messageBytes[4] ~= nil then valuetable["NIGHT_LIGHT_BRIGHTNESS"] = messageBytes[4] end
        if messageBytes[5] ~= nil then valuetable["RADAR_INDUCTION_ENABLE"] = messageBytes[5] end
        if messageBytes[6] ~= nil then valuetable["RADAR_INDUCTION_CLOSING_TIME"] = messageBytes[6] end
        if messageBytes[7] ~= nil then valuetable["LIGHT_INTENSITY_THRESHOLD"] = messageBytes[7] end
        if messageBytes[8] ~= nil then valuetable["RADAR_SENSITIVITY"] = messageBytes[8] end
        if messageBytes[9] ~= nil then valuetable["HEATING_ENABLE"] = messageBytes[9] end
        if messageBytes[10] ~= nil then valuetable["HEATING_TEMPERATURE"] = messageBytes[10] end
        if messageBytes[11] ~= nil then valuetable["HEATING_SPEED"] = messageBytes[11] end
        if messageBytes[12] ~= nil then valuetable["HEATING_DIRECTION"] = messageBytes[12] end
        if messageBytes[13] ~= nil then valuetable["BATH_ENABLE"] = messageBytes[13] end
        if messageBytes[14] ~= nil then valuetable["BATH_HEATING_TIME"] = messageBytes[14] end
        if messageBytes[15] ~= nil then valuetable["BATH_TEMPERATURE"] = messageBytes[15] end
        if messageBytes[16] ~= nil then valuetable["BATH_SPEED"] = messageBytes[16] end
        if messageBytes[17] ~= nil then valuetable["BATH_DIRECTION"] = messageBytes[17] end
        if messageBytes[18] ~= nil then valuetable["VENTILATION_ENABLE"] = messageBytes[18] end
        if messageBytes[19] ~= nil then valuetable["VENTILATION_SPEED"] = messageBytes[19] end
        if messageBytes[20] ~= nil then valuetable["VENTILATION_DIRECTION"] = messageBytes[20] end
        if messageBytes[21] ~= nil then valuetable["DRYING_ENABLE"] = messageBytes[21] end
        if messageBytes[22] ~= nil then valuetable["DRYING_TIME"] = messageBytes[22] end
        if messageBytes[23] ~= nil then valuetable["DRYING_TEMPERATURE"] = messageBytes[23] end
        if messageBytes[24] ~= nil then valuetable["DRYING_SPEED"] = messageBytes[24] end
        if messageBytes[25] ~= nil then valuetable["DRYING_DIRECTION"] = messageBytes[25] end
        if messageBytes[26] ~= nil then valuetable["BLOWING_ENABLE"] = messageBytes[26] end
        if messageBytes[27] ~= nil then valuetable["BLOWING_SPEED"] = messageBytes[27] end
        if messageBytes[28] ~= nil then valuetable["BLOWING_DIRECTION"] = messageBytes[28] end
        if messageBytes[29] ~= nil then valuetable["DELAY_ENABLE"] = messageBytes[29] end
        if messageBytes[30] ~= nil then valuetable["DELAY_TIME"] = messageBytes[30] end
        if messageBytes[31] ~= nil then valuetable["CURRENT_HUMIDITY"] = messageBytes[31] end
        if messageBytes[32] ~= nil then valuetable["CURRENT_RADAR_STATUS"] = messageBytes[32] end
        if messageBytes[33] ~= nil then valuetable["CURRENT_TEMPERATURE"] = messageBytes[33] end
        if messageBytes[34] ~= nil then valuetable["FILTER_STATUS"] = messageBytes[34] end
        if messageBytes[35] ~= nil then valuetable["REMOTE_LOW_POWER"] = messageBytes[35] end
        if messageBytes[36] ~= nil then valuetable["REMOTE_POWER"] = messageBytes[36] end
        if messageBytes[37] ~= nil then valuetable["SMELLY_LEVEL"] = messageBytes[37] end
        if messageBytes[38] ~= nil then valuetable["SOFT_WIND_ENABLE"] = messageBytes[38] end
        if messageBytes[39] ~= nil then valuetable["SOFT_WIND_HEATING_TIME"] = messageBytes[39] end
        if messageBytes[40] ~= nil then valuetable["SOFT_WIND_TEMPERATURE"] = messageBytes[40] end
        if messageBytes[41] ~= nil then valuetable["SOFT_WIND_SPEED"] = messageBytes[41] end
        if messageBytes[42] ~= nil then valuetable["SOFT_WIND_DIRECTION"] = messageBytes[42] end
        if messageBytes[43] ~= nil then valuetable["WINDLESS_ENABLE"] = messageBytes[43] end
        if messageBytes[44] ~= nil then valuetable["ANION_ENABLE"] = messageBytes[44] end
        if messageBytes[45] ~= nil then valuetable["SMELLY_ENABLE"] = messageBytes[45] end
        if messageBytes[46] ~= nil then valuetable["SMELLY_THRESHOLD"] = messageBytes[46] end
        if messageBytes[47] ~= nil then valuetable["AUTO_DEHUMIDIFICATION"] = messageBytes[47] end
        if messageBytes[48] ~= nil then valuetable["DEHUMIDITY_THRESHOLD"] = messageBytes[48] end
        if messageBytes[49] ~= nil then valuetable["DEHUMIDITY_TIME"] = messageBytes[49] end
        if messageBytes[50] ~= nil then valuetable["DEHUMIDITY_INTERVAL_TIME"] = messageBytes[50] end
        if messageBytes[51] ~= nil then valuetable["DEHUMIDITY_DIRECTION"] = messageBytes[51] end
        if messageBytes[52] ~= nil then
            valuetable["WIFI_LED_ENABLE"] = bit.band(messageBytes[52], 0x03)
            valuetable["FUNCTION_LED_ENABLE"] = bit.band(bit.rshift(messageBytes[52], 2), 0x03)
            valuetable["DIGIT_LED_ENABLE"] = bit.band(bit.rshift(messageBytes[52], 4), 0x03)
        end
    elseif cmdType == 0x02 then
        if messageBytes[1] ~= nil then valuetable["TIMING1_ENABLE"] = messageBytes[1] end
        if messageBytes[2] ~= nil then valuetable["TIMING1_FUNCTION"] = messageBytes[2] end
        if messageBytes[3] ~= nil then valuetable["TIMING1_DATE_REPEAT"] = messageBytes[3] end
        ...
        if messageBytes[8] ~= nil then valuetable["TIMING2_ENABLE"] = messageBytes[8] end
        if messageBytes[9] ~= nil then valuetable["TIMING2_FUNCTION"] = messageBytes[9] end
        if messageBytes[10] ~= nil then valuetable["TIMING2_DATE_REPEAT"] = messageBytes[10] end
        ...
        if messageBytes[15] ~= nil then valuetable["TIMING3_ENABLE"] = messageBytes[15] end
        if messageBytes[16] ~= nil then valuetable["TIMING3_FUNCTION"] = messageBytes[16] end
        if messageBytes[17] ~= nil then valuetable["TIMING3_DATE_REPEAT"] = messageBytes[17] end
        ...
        if messageBytes[22] ~= nil then valuetable["TIMING4_ENABLE"] = messageBytes[22] end
        if messageBytes[23] ~= nil then valuetable["TIMING4_FUNCTION"] = messageBytes[23] end
        if messageBytes[24] ~= nil then valuetable["TIMING4_DATE_REPEAT"] = messageBytes[24] end
        ...
        if messageBytes[29] ~= nil then valuetable["TIMING5_ENABLE"] = messageBytes[29] end
        if messageBytes[30] ~= nil then valuetable["TIMING5_FUNCTION"] = messageBytes[30] end
        if messageBytes[31] ~= nil then valuetable["TIMING5_DATE_REPEAT"] = messageBytes[31] end
    ...
    elseif cmdType == 0x03 then
        ...
        if hard_version_len ~= nil then
            for i = 0, hard_version_len - 1 do
                valuetable["HARDWARE_VERSION"] = valuetable["HARDWARE_VERSION"] .. string.char(messageBytes[i + 2])
            end
        end
        soft_version_len = messageBytes[hard_version_len + 2]
        if soft_version_len ~= nil then
            for i = 0, soft_version_len - 1 do
                valuetable["SOFTWARE_VERSION"] = valuetable["SOFTWARE_VERSION"]
...
local function assembleJsonByGlobalProperty()
    ...
    if dataType == BYTE_CONTROL_REQUEST then
        if cmdType == 0x02 then streams[keytable["KEY_SUB_PACKET_TYPE"]] = VALUE_TIMING_SETTING end
    ...
    if (valuetable["MAIN_LIGHT_ENABLE"] ~= 0xFF) or (valuetable["NIGHT_LIGHT_ENABLE"] ~= 0xFF) then
        streams[keytable["KEY_LIGHT_MODE"]] = ""
        if valuetable["MAIN_LIGHT_ENABLE"] == 0x01 then
            streams[keytable["KEY_LIGHT_MODE"]] = streams[keytable["KEY_LIGHT_MODE"]] .. VALUE_FUNCTION_MAIN_LIGHT
            streams[keytable["KEY_LIGHT_MODE"]] = streams[keytable["KEY_LIGHT_MODE"]] .. ","
        end
        if valuetable["NIGHT_LIGHT_ENABLE"] == 0x01 then
            streams[keytable["KEY_LIGHT_MODE"]] = streams[keytable["KEY_LIGHT_MODE"]] .. VALUE_FUNCTION_NIGHT_LIGHT
            streams[keytable["KEY_LIGHT_MODE"]] = streams[keytable["KEY_LIGHT_MODE"]] .. ","
        end
        if streams[keytable["KEY_LIGHT_MODE"]] == "" then
            streams[keytable["KEY_LIGHT_MODE"]] = VALUE_FUNCTION_CLOSE_ALL
        else
            streams[keytable["KEY_LIGHT_MODE"]] = string.sub(streams[keytable["KEY_LIGHT_MODE"]], 1, -2)
        end
    end
    if valuetable["MAIN_LIGHT_BRIGHTNESS"] ~= 0xFF then
        streams[keytable["KEY_MAIN_LIGHT_BRIGHTNESS"]] = int2String(valuetable["MAIN_LIGHT_BRIGHTNESS"])
    end
    if valuetable["NIGHT_LIGHT_BRIGHTNESS"] ~= 0xFF then
        streams[keytable["KEY_NIGHT_LIGHT_BRIGHTNESS"]] = int2String(valuetable["NIGHT_LIGHT_BRIGHTNESS"])
    end
    ...
    if valuetable["RADAR_INDUCTION_CLOSING_TIME"] ~= 0xFF then
        streams[keytable["KEY_RADAR_INDUCTION_CLOSING_TIME"]] = int2String(valuetable["RADAR_INDUCTION_CLOSING_TIME"])
    end
    if valuetable["LIGHT_INTENSITY_THRESHOLD"] ~= 0xFF then
        streams[keytable["KEY_LIGHT_INTENSITY_THRESHOLD"]] = int2String(valuetable["LIGHT_INTENSITY_THRESHOLD"])
    end
    if valuetable["RADAR_SENSITIVITY"] ~= 0xFF then
        streams[keytable["KEY_RADAR_SENSITIVITY"]] = int2String(valuetable["RADAR_SENSITIVITY"])
    end
    if
        (valuetable["HEATING_ENABLE"] ~= 0xFF)
        or (valuetable["BATH_ENABLE"] ~= 0xFF)
        or (valuetable["VENTILATION_ENABLE"] ~= 0xFF)
        or (valuetable["MORNING_VENTILATION_ENABLE"] ~= 0xFF)
        or (valuetable["DRYING_ENABLE"] ~= 0xFF)
        or (valuetable["BLOWING_ENABLE"] ~= 0xFF)
        or (valuetable["SOFT_WIND_ENABLE"] ~= 0xFF)
    then
        streams[keytable["KEY_MODE"]] = ""
        if valuetable["HEATING_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_HEATING
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["BATH_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_BATH
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["VENTILATION_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_VENTILATION
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["VENTILATION_ENABLE"] == 0x02 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_MORNING_VENTILATION
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["DRYING_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_DRYING
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["BLOWING_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_BLOWING
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if valuetable["SOFT_WIND_ENABLE"] == 0x01 then
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. VALUE_FUNCTION_SOFT_WIND
            streams[keytable["KEY_MODE"]] = streams[keytable["KEY_MODE"]] .. ","
        end
        if streams[keytable["KEY_MODE"]] == "" then
            streams[keytable["KEY_MODE"]] = VALUE_FUNCTION_CLOSE_ALL
        else
            streams[keytable["KEY_MODE"]] = string.sub(streams[keytable["KEY_MODE"]], 1, -2)
        end
    end
    if valuetable["HEATING_TEMPERATURE"] ~= 0xFF then
        streams[keytable["KEY_HEATING_TEMPERATURE"]] = int2String(valuetable["HEATING_TEMPERATURE"])
    end
    if valuetable["HEATING_SPEED"] ~= 0xFF then
        streams[keytable["KEY_HEATING_SPEED"]] = int2String(valuetable["HEATING_SPEED"])
    end
    if valuetable["HEATING_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_HEATING_DIRECTION"]] = int2String(valuetable["HEATING_DIRECTION"])
    end
    if valuetable["BATH_HEATING_TIME"] ~= 0xFF then
        streams[keytable["KEY_BATH_HEATING_TIME"]] = int2String(valuetable["BATH_HEATING_TIME"])
    end
    if valuetable["BATH_TEMPERATURE"] ~= 0xFF then
        streams[keytable["KEY_BATH_TEMPERATURE"]] = int2String(valuetable["BATH_TEMPERATURE"])
    end
    if valuetable["BATH_SPEED"] ~= 0xFF then
        streams[keytable["KEY_BATH_SPEED"]] = int2String(valuetable["BATH_SPEED"])
    end
    if valuetable["BATH_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_BATH_DIRECTION"]] = int2String(valuetable["BATH_DIRECTION"])
    end
    if valuetable["SOFT_WIND_HEATING_TIME"] ~= 0xFF then
        streams[keytable["KEY_SOFT_WIND_HEATING_TIME"]] = int2String(valuetable["SOFT_WIND_HEATING_TIME"])
    end
    if valuetable["SOFT_WIND_TEMPERATURE"] ~= 0xFF then
        streams[keytable["KEY_SOFT_WIND_TEMPERATURE"]] = int2String(valuetable["SOFT_WIND_TEMPERATURE"])
    end
    if valuetable["SOFT_WIND_SPEED"] ~= 0xFF then
        streams[keytable["KEY_SOFT_WIND_SPEED"]] = int2String(valuetable["SOFT_WIND_SPEED"])
    end
    if valuetable["SOFT_WIND_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_SOFT_WIND_DIRECTION"]] = int2String(valuetable["SOFT_WIND_DIRECTION"])
    end
    if valuetable["VENTILATION_SPEED"] ~= 0xFF then
        streams[keytable["KEY_VENTILATION_SPEED"]] = int2String(valuetable["VENTILATION_SPEED"])
    end
    if valuetable["VENTILATION_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_VENTILATION_DIRECTION"]] = int2String(valuetable["VENTILATION_DIRECTION"])
    end
    if valuetable["DRYING_TIME"] ~= 0xFF then
        streams[keytable["KEY_DRYING_TIME"]] = int2String(valuetable["DRYING_TIME"])
    end
    if valuetable["DRYING_TEMPERATURE"] ~= 0xFF then
        streams[keytable["KEY_DRYING_TEMPERATURE"]] = int2String(valuetable["DRYING_TEMPERATURE"])
    end
    if valuetable["DRYING_SPEED"] ~= 0xFF then
        streams[keytable["KEY_DRYING_SPEED"]] = int2String(valuetable["DRYING_SPEED"])
    end
    if valuetable["DRYING_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_DRYING_DIRECTION"]] = int2String(valuetable["DRYING_DIRECTION"])
    end
    if valuetable["BLOWING_SPEED"] ~= 0xFF then
        streams[keytable["KEY_BLOWING_SPEED"]] = int2String(valuetable["BLOWING_SPEED"])
    end
    if valuetable["BLOWING_DIRECTION"] ~= 0xFF then
        streams[keytable["KEY_BLOWING_DIRECTION"]] = int2String(valuetable["BLOWING_DIRECTION"])
    end
    ...
    if valuetable["DELAY_TIME"] ~= 0xFF then
        streams[keytable["KEY_DELAY_TIME"]] = int2String(valuetable["DELAY_TIME"])
    end
    ...
    if valuetable["SMELLY_THRESHOLD"] ~= 0xFF then
        streams[keytable["KEY_SMELLY_THRESHOLD"]] = int2String(valuetable["SMELLY_THRESHOLD"])
    end
    if valuetable["CURRENT_HUMIDITY"] ~= 0xFF then
        streams[keytable["KEY_CURRENT_HUMIDITY"]] = int2String(valuetable["CURRENT_HUMIDITY"])
    end
    if valuetable["CURRENT_RADAR_STATUS"] ~= 0xFF then
        streams[keytable["KEY_CURRENT_RADAR_STATUS"]] = int2String(valuetable["CURRENT_RADAR_STATUS"])
    end
    if valuetable["CURRENT_TEMPERATURE"] == 0xAA then valuetable["CURRENT_TEMPERATURE"] = -9 end
    if valuetable["CURRENT_TEMPERATURE"] ~= 0xFF then
        if valuetable["CURRENT_TEMPERATURE"] <= 127 then
            streams[keytable["KEY_CURRENT_TEMPERATURE"]] = int2String(valuetable["CURRENT_TEMPERATURE"])
        else
            streams[keytable["KEY_CURRENT_TEMPERATURE"]] =
                string.format("-%d", 0xFF + 1 - valuetable["CURRENT_TEMPERATURE"])
        end
    end
    ...
    if valuetable["REMOTE_POWER"] ~= 0xFF then
        streams[keytable["KEY_REMOTE_POWER"]] = int2String(valuetable["REMOTE_POWER"])
    end
    if valuetable["SMELLY_LEVEL"] ~= 0xFF then
        streams[keytable["KEY_SMELLY_LEVEL"]] = int2String(valuetable["SMELLY_LEVEL"])
    end
    ...
    if valuetable["DEHUMIDITY_THRESHOLD"] ~= 0xFF then
        streams[keytable["KEY_DEHUMIDITY_THRESHOLD"]] = int2String(valuetable["DEHUMIDITY_THRESHOLD"])
    end
    if valuetable["DEHUMIDITY_TIME"] ~= 0xFF then
        streams[keytable["KEY_DEHUMIDITY_TIME"]] = int2String(valuetable["DEHUMIDITY_TIME"])
    end
    if valuetable["DEHUMIDITY_INTERVAL_TIME"] ~= 0xFF then
        streams[keytable["KEY_DEHUMIDITY_INTERVAL_TIME"]] = int2String(valuetable["DEHUMIDITY_INTERVAL_TIME"])
    end
    ...
    elseif valuetable["TIMING1_FUNCTION"] == 0x08 then
        streams[keytable["KEY_TIMING1_FUNCTION"]] = VALUE_FUNCTION_DRYING_SAFE_POWER
    ...
    if valuetable["TIMING1_DATE_REPEAT"] ~= 0xFF then
        ...
        if bit.band(valuetable["TIMING1_DATE_REPEAT"], 0x80) ~= 0 then
            ...
            if bit.band(valuetable["TIMING1_DATE_REPEAT"], 0x01) ~= 0 then
                streams[keytable["KEY_TIMING1_DATE_REPEAT"]] = streams[keytable["KEY_TIMING1_DATE_REPEAT"]] .. "sunday,"
            end
            if bit.band(valuetable["TIMING1_DATE_REPEAT"], 0x02) ~= 0 then
                streams[keytable["KEY_TIMING1_DATE_REPEAT"]] = streams[keytable["KEY_TIMING1_DATE_REPEAT"]] .. "monday,"
            end
            if bit.band(valuetable["TIMING1_DATE_REPEAT"], 0x04) ~= 0 then
                streams[keytable["KEY_TIMING1_DATE_REPEAT"]] = streams[keytable["KEY_TIMING1_DATE_REPEAT"]]
                    .. "tuesday,"
            end
            ...
            if bit.band(valuetable["TIMING1_DATE_REPEAT"], 0x20) ~= 0 then
                streams[keytable["KEY_TIMING1_DATE_REPEAT"]] = streams[keytable["KEY_TIMING1_DATE_REPEAT"]] .. "friday,"
            end
            ...
            streams[keytable["KEY_TIMING1_DATE_REPEAT"]] =
                string.sub(streams[keytable["KEY_TIMING1_DATE_REPEAT"]], 1, -2)
        end
    end
    if valuetable["TIMING1_OPENINT_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING1_OPENINT_TIME"]] = string.format("%02d", valuetable["TIMING1_OPENINT_TIME_HOUR"])
        streams[keytable["KEY_TIMING1_OPENINT_TIME"]] = streams[keytable["KEY_TIMING1_OPENINT_TIME"]] .. ":"
        streams[keytable["KEY_TIMING1_OPENINT_TIME"]] = streams[keytable["KEY_TIMING1_OPENINT_TIME"]]
            .. string.format("%02d", valuetable["TIMING1_OPENINT_TIME_MIN"])
    end
    if valuetable["TIMING1_CLOSING_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING1_CLOSING_TIME"]] = string.format("%02d", valuetable["TIMING1_CLOSING_TIME_HOUR"])
        streams[keytable["KEY_TIMING1_CLOSING_TIME"]] = streams[keytable["KEY_TIMING1_CLOSING_TIME"]] .. ":"
        streams[keytable["KEY_TIMING1_CLOSING_TIME"]] = streams[keytable["KEY_TIMING1_CLOSING_TIME"]]
    ...
    elseif valuetable["TIMING2_FUNCTION"] == 0x08 then
        streams[keytable["KEY_TIMING2_FUNCTION"]] = VALUE_FUNCTION_DRYING_SAFE_POWER
    ...
    if valuetable["TIMING2_DATE_REPEAT"] ~= 0xFF then
        ...
        if bit.band(valuetable["TIMING2_DATE_REPEAT"], 0x80) ~= 0 then
            ...
            if bit.band(valuetable["TIMING2_DATE_REPEAT"], 0x01) ~= 0 then
                streams[keytable["KEY_TIMING2_DATE_REPEAT"]] = streams[keytable["KEY_TIMING2_DATE_REPEAT"]] .. "sunday,"
            end
            if bit.band(valuetable["TIMING2_DATE_REPEAT"], 0x02) ~= 0 then
                streams[keytable["KEY_TIMING2_DATE_REPEAT"]] = streams[keytable["KEY_TIMING2_DATE_REPEAT"]] .. "monday,"
            end
            if bit.band(valuetable["TIMING2_DATE_REPEAT"], 0x04) ~= 0 then
                streams[keytable["KEY_TIMING2_DATE_REPEAT"]] = streams[keytable["KEY_TIMING2_DATE_REPEAT"]]
                    .. "tuesday,"
            end
            ...
            if bit.band(valuetable["TIMING2_DATE_REPEAT"], 0x20) ~= 0 then
                streams[keytable["KEY_TIMING2_DATE_REPEAT"]] = streams[keytable["KEY_TIMING2_DATE_REPEAT"]] .. "friday,"
            end
            ...
            streams[keytable["KEY_TIMING2_DATE_REPEAT"]] =
                string.sub(streams[keytable["KEY_TIMING2_DATE_REPEAT"]], 1, -2)
        end
    end
    if valuetable["TIMING2_OPENINT_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING2_OPENINT_TIME"]] = string.format("%02d", valuetable["TIMING2_OPENINT_TIME_HOUR"])
        streams[keytable["KEY_TIMING2_OPENINT_TIME"]] = streams[keytable["KEY_TIMING2_OPENINT_TIME"]] .. ":"
        streams[keytable["KEY_TIMING2_OPENINT_TIME"]] = streams[keytable["KEY_TIMING2_OPENINT_TIME"]]
            .. string.format("%02d", valuetable["TIMING2_OPENINT_TIME_MIN"])
    end
    if valuetable["TIMING2_CLOSING_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING2_CLOSING_TIME"]] = string.format("%02d", valuetable["TIMING2_CLOSING_TIME_HOUR"])
        streams[keytable["KEY_TIMING2_CLOSING_TIME"]] = streams[keytable["KEY_TIMING2_CLOSING_TIME"]] .. ":"
        streams[keytable["KEY_TIMING2_CLOSING_TIME"]] = streams[keytable["KEY_TIMING2_CLOSING_TIME"]]
    ...
    elseif valuetable["TIMING3_FUNCTION"] == 0x08 then
        streams[keytable["KEY_TIMING3_FUNCTION"]] = VALUE_FUNCTION_DRYING_SAFE_POWER
    ...
    if valuetable["TIMING3_DATE_REPEAT"] ~= 0xFF then
        ...
        if bit.band(valuetable["TIMING3_DATE_REPEAT"], 0x80) ~= 0 then
            ...
            if bit.band(valuetable["TIMING3_DATE_REPEAT"], 0x01) ~= 0 then
                streams[keytable["KEY_TIMING3_DATE_REPEAT"]] = streams[keytable["KEY_TIMING3_DATE_REPEAT"]] .. "sunday,"
            end
            if bit.band(valuetable["TIMING3_DATE_REPEAT"], 0x02) ~= 0 then
                streams[keytable["KEY_TIMING3_DATE_REPEAT"]] = streams[keytable["KEY_TIMING3_DATE_REPEAT"]] .. "monday,"
            end
            if bit.band(valuetable["TIMING3_DATE_REPEAT"], 0x04) ~= 0 then
                streams[keytable["KEY_TIMING3_DATE_REPEAT"]] = streams[keytable["KEY_TIMING3_DATE_REPEAT"]]
                    .. "tuesday,"
            end
            ...
            if bit.band(valuetable["TIMING3_DATE_REPEAT"], 0x20) ~= 0 then
                streams[keytable["KEY_TIMING3_DATE_REPEAT"]] = streams[keytable["KEY_TIMING3_DATE_REPEAT"]] .. "friday,"
            end
            ...
            streams[keytable["KEY_TIMING3_DATE_REPEAT"]] =
                string.sub(streams[keytable["KEY_TIMING3_DATE_REPEAT"]], 1, -2)
        end
    end
    if valuetable["TIMING3_OPENINT_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING3_OPENINT_TIME"]] = string.format("%02d", valuetable["TIMING3_OPENINT_TIME_HOUR"])
        streams[keytable["KEY_TIMING3_OPENINT_TIME"]] = streams[keytable["KEY_TIMING3_OPENINT_TIME"]] .. ":"
        streams[keytable["KEY_TIMING3_OPENINT_TIME"]] = streams[keytable["KEY_TIMING3_OPENINT_TIME"]]
            .. string.format("%02d", valuetable["TIMING3_OPENINT_TIME_MIN"])
    end
    if valuetable["TIMING3_CLOSING_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING3_CLOSING_TIME"]] = string.format("%02d", valuetable["TIMING3_CLOSING_TIME_HOUR"])
        streams[keytable["KEY_TIMING3_CLOSING_TIME"]] = streams[keytable["KEY_TIMING3_CLOSING_TIME"]] .. ":"
        streams[keytable["KEY_TIMING3_CLOSING_TIME"]] = streams[keytable["KEY_TIMING3_CLOSING_TIME"]]
    ...
    elseif valuetable["TIMING4_FUNCTION"] == 0x08 then
        streams[keytable["KEY_TIMING4_FUNCTION"]] = VALUE_FUNCTION_DRYING_SAFE_POWER
    ...
    if valuetable["TIMING4_DATE_REPEAT"] ~= 0xFF then
        ...
        if bit.band(valuetable["TIMING4_DATE_REPEAT"], 0x80) ~= 0 then
            ...
            if bit.band(valuetable["TIMING4_DATE_REPEAT"], 0x01) ~= 0 then
                streams[keytable["KEY_TIMING4_DATE_REPEAT"]] = streams[keytable["KEY_TIMING4_DATE_REPEAT"]] .. "sunday,"
            end
            if bit.band(valuetable["TIMING4_DATE_REPEAT"], 0x02) ~= 0 then
                streams[keytable["KEY_TIMING4_DATE_REPEAT"]] = streams[keytable["KEY_TIMING4_DATE_REPEAT"]] .. "monday,"
            end
            if bit.band(valuetable["TIMING4_DATE_REPEAT"], 0x04) ~= 0 then
                streams[keytable["KEY_TIMING4_DATE_REPEAT"]] = streams[keytable["KEY_TIMING4_DATE_REPEAT"]]
                    .. "tuesday,"
            end
            ...
            if bit.band(valuetable["TIMING4_DATE_REPEAT"], 0x20) ~= 0 then
                streams[keytable["KEY_TIMING4_DATE_REPEAT"]] = streams[keytable["KEY_TIMING4_DATE_REPEAT"]] .. "friday,"
            end
            ...
            streams[keytable["KEY_TIMING4_DATE_REPEAT"]] =
                string.sub(streams[keytable["KEY_TIMING4_DATE_REPEAT"]], 1, -2)
        end
    end
    if valuetable["TIMING4_OPENINT_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING4_OPENINT_TIME"]] = string.format("%02d", valuetable["TIMING4_OPENINT_TIME_HOUR"])
        streams[keytable["KEY_TIMING4_OPENINT_TIME"]] = streams[keytable["KEY_TIMING4_OPENINT_TIME"]] .. ":"
        streams[keytable["KEY_TIMING4_OPENINT_TIME"]] = streams[keytable["KEY_TIMING4_OPENINT_TIME"]]
            .. string.format("%02d", valuetable["TIMING4_OPENINT_TIME_MIN"])
    end
    if valuetable["TIMING4_CLOSING_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING4_CLOSING_TIME"]] = string.format("%02d", valuetable["TIMING4_CLOSING_TIME_HOUR"])
        streams[keytable["KEY_TIMING4_CLOSING_TIME"]] = streams[keytable["KEY_TIMING4_CLOSING_TIME"]] .. ":"
        streams[keytable["KEY_TIMING4_CLOSING_TIME"]] = streams[keytable["KEY_TIMING4_CLOSING_TIME"]]
    ...
    elseif valuetable["TIMING5_FUNCTION"] == 0x08 then
        streams[keytable["KEY_TIMING5_FUNCTION"]] = VALUE_FUNCTION_DRYING_SAFE_POWER
    ...
    if valuetable["TIMING5_DATE_REPEAT"] ~= 0xFF then
        ...
        if bit.band(valuetable["TIMING5_DATE_REPEAT"], 0x80) ~= 0 then
            ...
            if bit.band(valuetable["TIMING5_DATE_REPEAT"], 0x01) ~= 0 then
                streams[keytable["KEY_TIMING5_DATE_REPEAT"]] = streams[keytable["KEY_TIMING5_DATE_REPEAT"]] .. "sunday,"
            end
            if bit.band(valuetable["TIMING5_DATE_REPEAT"], 0x02) ~= 0 then
                streams[keytable["KEY_TIMING5_DATE_REPEAT"]] = streams[keytable["KEY_TIMING5_DATE_REPEAT"]] .. "monday,"
            end
            if bit.band(valuetable["TIMING5_DATE_REPEAT"], 0x04) ~= 0 then
                streams[keytable["KEY_TIMING5_DATE_REPEAT"]] = streams[keytable["KEY_TIMING5_DATE_REPEAT"]]
                    .. "tuesday,"
            end
            ...
            if bit.band(valuetable["TIMING5_DATE_REPEAT"], 0x20) ~= 0 then
                streams[keytable["KEY_TIMING5_DATE_REPEAT"]] = streams[keytable["KEY_TIMING5_DATE_REPEAT"]] .. "friday,"
            end
            ...
            streams[keytable["KEY_TIMING5_DATE_REPEAT"]] =
                string.sub(streams[keytable["KEY_TIMING5_DATE_REPEAT"]], 1, -2)
        end
    end
    if valuetable["TIMING5_OPENINT_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING5_OPENINT_TIME"]] = string.format("%02d", valuetable["TIMING5_OPENINT_TIME_HOUR"])
        streams[keytable["KEY_TIMING5_OPENINT_TIME"]] = streams[keytable["KEY_TIMING5_OPENINT_TIME"]] .. ":"
        streams[keytable["KEY_TIMING5_OPENINT_TIME"]] = streams[keytable["KEY_TIMING5_OPENINT_TIME"]]
            .. string.format("%02d", valuetable["TIMING5_OPENINT_TIME_MIN"])
    end
    if valuetable["TIMING5_CLOSING_TIME_HOUR"] ~= 0xFF then
        streams[keytable["KEY_TIMING5_CLOSING_TIME"]] = string.format("%02d", valuetable["TIMING5_CLOSING_TIME_HOUR"])
        streams[keytable["KEY_TIMING5_CLOSING_TIME"]] = streams[keytable["KEY_TIMING5_CLOSING_TIME"]] .. ":"
        streams[keytable["KEY_TIMING5_CLOSING_TIME"]] = streams[keytable["KEY_TIMING5_CLOSING_TIME"]]
            .. string.format("%02d", valuetable["TIMING5_CLOSING_TIME_MIN"])
    end
    if valuetable["HARDWARE_VERSION"] ~= "" then
        streams[keytable["KEY_HARDWARE_VERSION"]] = valuetable["HARDWARE_VERSION"]
    end
    if valuetable["SOFTWARE_VERSION"] ~= "" then
        streams[keytable["KEY_SOFTWARE_VERSION"]] = valuetable["SOFTWARE_VERSION"]
    end
    return streams
end
function jsonToData(jsonCmdStr)
    ...
    elseif status then
        ...
        if status[keytable["KEY_RESULT"]] ~= VALUE_SUCCESS then bodyBytes[1] = 0x01 end
