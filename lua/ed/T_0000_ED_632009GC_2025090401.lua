...
local function statusJudge(streams, bytes)
    ...
    elseif attr == 0x012 then
        streams["hot_water_consumption"] = bit.lshift(bytes[byteCount + 6], 24)
            + bit.lshift(bytes[byteCount + 5], 16)
            + bit.lshift(bytes[byteCount + 4], 8)
            + bytes[byteCount + 3]
    ...
    elseif attr == 0x027 then
        streams["rfid_quantify"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x036 then
        streams["screenout_time"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x04C then
        streams["single_stew_time"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x04D then
        streams["single_stew_temperature"] = bytes[byteCount + 3]
    elseif attr == 0x04E then
        streams["stew_time_1"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
        streams["stew_time_2"] = bit.lshift(bytes[byteCount + 6], 8) + bytes[byteCount + 5]
        streams["stew_time_3"] = bit.lshift(bytes[byteCount + 8], 8) + bytes[byteCount + 7]
        streams["stew_time_4"] = bit.lshift(bytes[byteCount + 10], 8) + bytes[byteCount + 9]
        streams["stew_time_5"] = bit.lshift(bytes[byteCount + 12], 8) + bytes[byteCount + 11]
    ...
    elseif attr == 0x050 then
        streams["gesture_quantify"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x051 then
        streams["night_light"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x064 then
        ...
        streams["stew_water"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    ...
    elseif attr == 0x068 then
        streams["filter_left_days_1"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x070 then
        ...
        streams["relay_warm_left_time"] = bit.lshift(bytes[byteCount + 6], 8) + bytes[byteCount + 5]
    ...
    elseif attr == 0x073 then
        ...
        streams["first_custom_out_water_sec"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    elseif attr == 0x074 then
        streams["life_expire_1"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x075 then
        streams["life_expire_2"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x076 then
        streams["life_expire_3"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x077 then
        streams["life_expire_4"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x078 then
        streams["life_expire_5"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x079 then
        streams["second_custom_out_water_mode"] = bytes[byteCount + 3]
        streams["second_custom_out_water_ml"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    elseif attr == 0x07A then
        streams["third_custom_out_water_mode"] = bytes[byteCount + 3]
        streams["third_custom_out_water_ml"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    elseif attr == 0x07B then
        streams["fourth_custom_out_water_mode"] = bytes[byteCount + 3]
        streams["fourth_custom_out_water_ml"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    elseif attr == 0x07C then
        streams["fifth_custom_out_water_mode"] = bytes[byteCount + 3]
        streams["fifth_custom_out_water_ml"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    elseif attr == 0x07E then
        streams["ice_size"] = bytes[byteCount + 3]
    elseif attr == 0x200 then
        streams["input_pressure_Sensing"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif (attr == 0x202) and (bytes[byteCount + 4] ~= nil) then
        streams["water_flow"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x301 then
        streams["all_water_consumption"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x302 then
        streams["today_water_consumption"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x903 then
        streams["water_consumption_big"] = bit.lshift(bytes[byteCount + 6], 24)
            + bit.lshift(bytes[byteCount + 5], 16)
            + bit.lshift(bytes[byteCount + 4], 8)
            + bytes[byteCount + 3]
    elseif attr == 0x904 then
        streams["water_consumption_today"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    elseif attr == 0x905 then
        streams["water_consumption_average"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x90A then
        streams["water_hardness"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x90D then
        streams["regeneration_left_seconds"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x910 then
        streams["use_days"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
        streams["regeneration_count"] = bit.lshift(bytes[byteCount + 6], 8) + bytes[byteCount + 5]
    ...
    elseif attr == 0x915 then
        ...
        streams["remind_maintenance_days"] = bit.lshift(bytes[byteCount + 5], 8) + bytes[byteCount + 4]
    ...
    elseif attr == 0x919 then
        streams["salt_usage_past_7"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
        streams["salt_usage_past_30"] = bit.lshift(bytes[byteCount + 7], 16)
            + bit.lshift(bytes[byteCount + 6], 8)
            + bytes[byteCount + 5]
    elseif attr == 0x920 then
        streams["clean_available_big"] = bit.lshift(bytes[byteCount + 6], 24)
            + bit.lshift(bytes[byteCount + 5], 16)
            + bit.lshift(bytes[byteCount + 4], 8)
            + bytes[byteCount + 3]
    elseif attr == 0x921 then
        streams["cleaning_left_seconds"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
    ...
    elseif attr == 0x92A then
        streams["use_days"] = bit.lshift(bytes[byteCount + 4], 8) + bytes[byteCount + 3]
        streams["cleaning_count"] = bit.lshift(bytes[byteCount + 6], 8) + bytes[byteCount + 5]
    end
    ...
    if control then
        ...
        if control["screenout_time"] then
            setbytes(
                ...
                0x01,
                0x00,
                control["screenout_time"] % 256,
                bit.rshift(control["screenout_time"], 8),
                bodyBytes
            )
        end
        ...
        if control["plateau_pressure"] then
            setbytes(
                ...
                0x01,
                0x00,
                control["plateau_pressure"] % 256,
                bit.rshift(control["plateau_pressure"], 8),
                bodyBytes
            )
        end
        ...
        if control["leaking_protect_time"] then
            setbytes(0x12, 0x01, control["leaking_protect_time"], 0x00, 0x00, bodyBytes)
        end
        ...
        elseif
            control["gesture_disable_high_temperature"]
            and control["gesture_disable_high_temperature"] == VALUE_OFF
        then
        ...
        if control["first_custom_out_water_ml"] then
            setbytes(
                0x14,
                0x01,
                control["first_custom_out_water_mode"],
        ...
        if control["human_sensing_switch"] and control["human_sensing_switch"] == VALUE_ON then
        ...
        elseif control["human_sensing_switch"] and control["human_sensing_switch"] == VALUE_OFF then
            if control["human_sensing_distance"] then
                setbytes(
                    0x18,
                    0x01,
        ...
        if control["tank_detection_mode"] then
            setbytes(0x19, 0x01, control["tank_detection_mode"], 0x00, 0x00, bodyBytes)
        end
        if control["gesture_quantify"] then
            setbytes(
                ...
                0x01,
                0x00,
                control["gesture_quantify"] % 256,
                bit.rshift(control["gesture_quantify"], 8),
                bodyBytes
            )
        end
        ...
        elseif control["night_light"] then
            setbytes(0x2E, 0x01, 0x00, control["night_light"] % 256, bit.rshift(control["night_light"], 8), bodyBytes)
        end
        if control["hydration_setting"] then
            setbytes(0x2F, 0x01, control["hydration_setting"], 0x00, 0x00, bodyBytes)
        end
        if control["counterfeiting_detect"] and control["counterfeiting_detect"] == VALUE_ON then
            setbytes(0x30, 0x01, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["counterfeiting_detect"] and control["counterfeiting_detect"] == VALUE_OFF then
            setbytes(0x30, 0x01, 0x00, 0x00, 0x00, bodyBytes)
        end
        if control["screen_brightness"] ~= nil then
            setbytes(0x31, 0x01, control["screen_brightness"], 0x00, 0x00, bodyBytes)
        end
        if control["first_custom_out_water_sec"] then
            setbytes(
                0x32,
                0x01,
                control["first_custom_out_water_sec_mode"],
        ...
        if control["second_custom_out_water_ml"] then
            setbytes(
                0x33,
                0x01,
                control["second_custom_out_water_mode"],
        ...
        if control["third_custom_out_water_ml"] then
            setbytes(
                0x34,
                0x01,
                control["third_custom_out_water_mode"],
        ...
        if control["fourth_custom_out_water_ml"] then
            setbytes(
                0x35,
                0x01,
                control["fourth_custom_out_water_mode"],
        ...
        if control["fifth_custom_out_water_ml"] then
            setbytes(
                0x36,
                0x01,
                control["fifth_custom_out_water_mode"],
        ...
        if control["quantify_enable"] and control["quantify_enable"] == VALUE_ON then
        ...
        elseif control["quantify_enable"] and control["quantify_enable"] == VALUE_OFF then
            if control["quantify_enable_timeout"] then
                setbytes(
                    0x05,
                    0x02,
            ...
            if control["rfid_temp"] then
                if control["rfid_quantify"] then
                    setbytes(
                        ...
                        0x02,
                        control["rfid_temp"],
                        control["rfid_quantify"],
                        control["rfid_role"],
                        bodyBytes
                    )
                else
                    setbytes(0x06, 0x02, control["rfid_temp"], 0x00, control["rfid_role"], bodyBytes)
                end
            elseif control["rfid_quantify"] then
                setbytes(0x06, 0x02, 0x00, control["rfid_quantify"], control["rfid_role"], bodyBytes)
            else
        ...
        if control["set_low_code"] and control["set_high_code"] then
            setbytes(0x08, 0x02, 0x00, control["set_low_code"], control["set_high_code"], bodyBytes)
        end
        for i = 1, 5 do
            if control["quantify_tds_" .. i] then
                setbytes(0x09, 0x02, i, control["quantify_tds_" .. i], 0x00, bodyBytes)
            end
        end
        ...
        if control["no_obsolete_water"] and control["no_obsolete_water"] == VALUE_ON then
            setbytes(0x0b, 0x02, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["no_obsolete_water"] and control["no_obsolete_water"] == VALUE_OFF then
        ...
        if control["smart_no_obsolete_water"] and control["smart_no_obsolete_water"] == VALUE_ON then
            setbytes(0x0f, 0x02, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["smart_no_obsolete_water"] and control["smart_no_obsolete_water"] == VALUE_OFF then
            setbytes(0x0f, 0x02, 0x00, 0x00, 0x00, bodyBytes)
        end
        if control["quantify_calibration"] and control["quantify_calibration"] == VALUE_ON then
            if control["quantify_calibration_percent"] then
                set_calibration(0x10, 0x02, 0x01, control["quantify_calibration_percent"], 0x00, bodyBytes)
            else
                set_calibration(0x10, 0x02, 0x01, 0x00, 0x00, bodyBytes)
            end
        elseif control["quantify_calibration"] and control["quantify_calibration"] == VALUE_OFF then
            if control["quantify_calibration_percent"] then
                set_calibration(0x10, 0x02, 0x00, control["quantify_calibration_percent"], 0x00, bodyBytes)
            else
                set_calibration(0x10, 0x02, 0x00, 0x00, 0x00, bodyBytes)
            end
        end
        if control["voice_conversation_power"] and control["voice_conversation_power"] == VALUE_ON then
            setbytes(0x11, 0x02, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["voice_conversation_power"] and control["voice_conversation_power"] == VALUE_OFF then
        ...
        elseif control["auto_continue"] and control["auto_continue"] == VALUE_OFF then
        ...
        if control["auto_stop_switch"] and control["auto_stop_switch"] == VALUE_ON then
            if control["auto_stop_kind"] ~= nil then
                if control["auto_stop_percentage"] ~= nil then
                    setbytes(0x14, 0x02, 0x01, control["auto_stop_kind"], control["auto_stop_percentage"], bodyBytes)
                else
                    setbytes(0x14, 0x02, 0x01, control["auto_stop_kind"], 0x00, bodyBytes)
                end
            else
                if control["auto_stop_percentage"] ~= nil then
                    setbytes(0x14, 0x02, 0x01, 0x00, control["auto_stop_percentage"], bodyBytes)
                else
                    setbytes(0x14, 0x02, 0x01, 0x00, 0x00, bodyBytes)
                end
            end
        elseif control["auto_stop_switch"] and control["auto_stop_switch"] == VALUE_OFF then
            if control["auto_stop_kind"] ~= nil then
                if control["auto_stop_percentage"] ~= nil then
                    setbytes(0x14, 0x02, 0x00, control["auto_stop_kind"], control["auto_stop_percentage"], bodyBytes)
                else
                    setbytes(0x14, 0x02, 0x00, control["auto_stop_kind"], 0x00, bodyBytes)
                end
            else
                if control["auto_stop_percentage"] ~= nil then
                    setbytes(0x14, 0x02, 0x00, 0x00, control["auto_stop_percentage"], bodyBytes)
                else
                    setbytes(0x14, 0x02, 0x00, 0x00, 0x00, bodyBytes)
                end
            end
        end
        if control["auto_fill_water"] and control["auto_fill_water"] == VALUE_ON then
            setbytes(0x15, 0x02, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["auto_fill_water"] and control["auto_fill_water"] == VALUE_OFF then
        ...
        for i = 1, 5 do
            if control["life_" .. i] then setbytes(0x02, 0x03, i - 1, control["life_" .. i], 0x00, bodyBytes) end
        end
        ...
        for i = 1, 6 do
            if control["maxlife_" .. i] then
                setbytes(
                    ...
                    0x03,
                    i - 1,
                    control["maxlife_" .. i] % 256,
                    (control["maxlife_" .. i] - control["maxlife_" .. i] % 256) / 256,
                    bodyBytes
                )
            end
        end
        ...
        elseif control["autoclean_ctrl"] and control["autoclean_ctrl"] == VALUE_OFF then
        ...
        for i = 1, 10 do
            if control["custom_temperature_" .. i] and control["brew_tea"] == nil then
        ...
        if control["plateau_power"] and control["plateau_power"] == VALUE_ON then
            if control["plateau_boiling_point"] then
                setbytes(0x0B, 0x04, 0x01, control["plateau_boiling_point"], 0x00, bodyBytes)
            else
                setbytes(0x0B, 0x04, 0x01, 0x00, 0x00, bodyBytes)
            end
        elseif control["plateau_power"] and control["plateau_power"] == VALUE_OFF then
            if control["plateau_boiling_point"] then
                setbytes(0x0B, 0x04, 0x00, control["plateau_boiling_point"], 0x00, bodyBytes)
            else
        ...
        if control["single_keep_warm_time"] then
            setbytes(0x0D, 0x04, control["single_keep_warm_time"], 0x00, 0x00, bodyBytes)
        end
        if control["single_keep_warm_temperature"] then
            setbytes(0x0E, 0x04, control["single_keep_warm_temperature"], 0x00, 0x00, bodyBytes)
        end
        if control["single_stew_time"] then
            setbytes(
                ...
                0x04,
                0x00,
                control["single_stew_time"] % 256,
                bit.rshift(control["single_stew_time"], 8),
                bodyBytes
            )
        end
        if control["single_stew_temperature"] then
            setbytes(0x10, 0x04, control["single_stew_temperature"], 0x00, 0x00, bodyBytes)
        end
        for i = 1, 5 do
        ...
        if control["selected_stew_mode"] then
            setbytes(
                0x12,
                0x04,
                control["selected_stew_mode"],
                control["stew_water"] % 256,
                bit.rshift(control["stew_water"], 8),
                bodyBytes
            )
        end
        if control["keep_warm_temperature"] then
            setbytes(0x13, 0x04, control["keep_warm_temperature"], 0x00, 0x00, bodyBytes)
        end
        for i = 1, 9 do
            if control["stew_mode_" .. i] then
                set_tembytes(0x14, 0x04, i, control["stew_mode_" .. i], 0x00, bodyBytes)
            end
        end
            ...
            if control["relay_keep_warm"] == VALUE_ON then
                if control["relay_keep_warm_time"] then
                    setbytes(
                        ...
                        (control["relay_keep_warm_time"] - control["relay_keep_warm_time"] % 256) / 256,
                        bodyBytes
                    )
                else
            ...
            elseif control["relay_keep_warm"] == VALUE_OFF then
                ...
                if control["relay_keep_warm_time"] then
                    setbytes(
                        0x15,
                        0x04,
                        control["relay_keep_warm"],
                        control["relay_keep_warm_time"] % 256,
                        (control["relay_keep_warm_time"] - control["relay_keep_warm_time"] % 256) / 256,
                        bodyBytes
                    )
                else
                    setbytes(0x15, 0x04, control["relay_keep_warm"], 0x00, 0x00, bodyBytes)
                end
            end
        end
        ...
        if control["cold_water_reflux_switch"] and control["cold_water_reflux_switch"] == VALUE_ON then
            setbytes(0x05, 0x05, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["cold_water_reflux_switch"] and control["cold_water_reflux_switch"] == VALUE_OFF then
            setbytes(0x05, 0x05, 0x00, 0x00, 0x00, bodyBytes)
        end
        if control["cold_water_reflux_hour"] then
            set_tembytes(0x06, 0x05, control["cold_water_reflux_hour"], 0x00, 0x00, bodyBytes)
        end
        if control["ice_size"] then set_tembytes(0x07, 0x05, control["ice_size"], 0x00, 0x00, bodyBytes) end
        ...
        if control["single_max_water_consumption"] then
            setbytes(0x00, 0x06, control["single_max_water_consumption"], 0x00, 0x00, bodyBytes)
        end
        if control["single_max_water_time"] then
            setbytes(0x01, 0x06, control["single_max_water_time"], 0x00, 0x00, bodyBytes)
        end
        if control["open_close_switch"] and control["open_close_switch"] == VALUE_ON then
            setbytes(0x02, 0x06, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["open_close_switch"] and control["open_close_switch"] == VALUE_OFF then
            setbytes(0x02, 0x06, 0x00, 0x00, 0x00, bodyBytes)
        end
        if control["open_close_water_pressure"] and control["open_close_water_pressure"] == VALUE_ON then
            setbytes(0x03, 0x06, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["open_close_water_pressure"] and control["open_close_water_pressure"] == VALUE_OFF then
        ...
        if control["clean_water_consumption"] then
            setbytes(0x06, 0x06, control["clean_water_consumption"], 0x00, 0x00, bodyBytes)
        end
        if control["clean_time"] then setbytes(0x07, 0x06, control["clean_time"], 0x00, 0x00, bodyBytes) end
        if control["leak_water_protect"] and control["leak_water_protect"] == VALUE_ON then
            setbytes(0x0A, 0x06, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["leak_water_protect"] and control["leak_water_protect"] == VALUE_OFF then
        ...
        if control["water_hardness"] then
            setbytes(
                ...
                0x09,
                0x00,
                control["water_hardness"] % 256,
                bit.rshift(control["water_hardness"], 8),
                bodyBytes
            )
        end
        if control["flushing_days"] then
            setbytes(
                ...
                0x09,
                0x00,
                control["flushing_days"] % 256,
                bit.rshift(control["flushing_days"], 8),
                bodyBytes
            )
        end
        if control["timing_regeneration_hour"] then
            if control["timing_regeneration_min"] then
                setbytes(
                    ...
                    0x09,
                    0x00,
                    control["timing_regeneration_hour"],
                    control["timing_regeneration_min"],
                    bodyBytes
                )
            else
                setbytes(0x02, 0x09, 0x00, control["timing_regeneration_hour"], 0x00, bodyBytes)
            end
        end
        if control["salt_setting"] then
            setbytes(0x03, 0x09, 0x00, control["salt_setting"] % 256, bit.rshift(control["salt_setting"], 8), bodyBytes)
        end
        ...
        if control["mixed_water_gear"] then setbytes(0x08, 0x09, control["mixed_water_gear"], 0x00, 0x00, bodyBytes) end
        if control["leak_water_protection"] and control["leak_water_protection"] == VALUE_ON then
        ...
        elseif control["leak_water_protection"] and control["leak_water_protection"] == VALUE_OFF then
            if control["leak_water_protection_value"] then
                setbytes(
                    0x09,
                    0x09,
        ...
        if control["micro_leak_protection"] and control["micro_leak_protection"] == VALUE_ON then
        ...
        elseif control["micro_leak_protection"] and control["micro_leak_protection"] == VALUE_OFF then
            if control["micro_leak_protection_value"] then
                setbytes(
                    0x0A,
                    0x09,
        ...
        if control["cl_sterilization"] and control["cl_sterilization"] == VALUE_ON then
            setbytes(0x0C, 0x09, 0x01, 0x00, 0x00, bodyBytes)
        elseif control["cl_sterilization"] and control["cl_sterilization"] == VALUE_OFF then
            ...
            if control["real_date_setting_month"] ~= nil then
                if control["real_date_setting_day"] ~= nil then
                    setbytes(
                        0x0D,
                        0x09,
                        control["real_date_setting_year"],
                        control["real_date_setting_month"],
                        control["real_date_setting_day"],
                        bodyBytes
                    )
                else
                    setbytes(
                ...
                setbytes(0x0D, 0x09, control["real_date_setting_year"], 0x00, 0x00, bodyBytes)
            end
        end
        if control["real_time_setting_hour"] ~= nil then
            if control["real_time_setting_min"] ~= nil then
                setbytes(
                    ...
                    0x09,
                    0x00,
                    control["real_time_setting_hour"],
                    control["real_time_setting_min"],
                    bodyBytes
                )
            else
                setbytes(0x0E, 0x09, 0x00, control["real_time_setting_hour"], 0x00, bodyBytes)
            end
        end
        if control["maintenance_reminder_switch"] ~= nil and control["maintenance_reminder_switch"] == VALUE_ON then
        ...
        elseif control["maintenance_reminder_switch"] and control["maintenance_reminder_switch"] == VALUE_OFF then
            if control["maintenance_reminder_setting"] then
                setbytes(
                    0x0F,
                    0x09,
        ...
        if control["cleaning_days"] then
            setbytes(
                ...
                0x09,
                0x00,
                control["cleaning_days"] % 256,
                bit.rshift(control["cleaning_days"], 8),
                bodyBytes
            )
        end
        if control["timing_cleaning_hour"] then
            if control["timing_cleaning_min"] then
                setbytes(0x11, 0x09, 0x00, control["timing_cleaning_hour"], control["timing_cleaning_min"], bodyBytes)
            else
                setbytes(0x11, 0x09, 0x00, control["timing_cleaning_hour"], 0x00, bodyBytes)
            end
        end
        ...
        elseif control["filter_switch"] and control["filter_switch"] == VALUE_OFF then
        ...
        elseif control["factory_reset"] and control["factory_reset"] == VALUE_OFF then
            setbytes(0x15, 0x09, 0x00, 0x00, 0x00, bodyBytes)
        end
        if control["regenerate_drainage"] ~= nil then
            setbytes(0x16, 0x09, control["regenerate_drainage"], 0x00, 0x00, bodyBytes)
        end
