...
function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable[KEY_HEAT_TEMPERATURE] ~= nil then coolTemp = luaTable[KEY_HEAT_TEMPERATURE] end
    if luaTable[KEY_COOL_TEMPERATURE] ~= nil then coolTemp = luaTable[KEY_COOL_TEMPERATURE] end
end

function updateGlobalPropertyValueByByte(messageBytes)
    if
        (dataType == 0x03 and messageBytes[0] == 0x01)
        or (dataType == 0x02 and messageBytes[0] == 0x01)
