...
function jsonToModel(jsonCmd)
    ...
    if streams[KEY_POWER_OFF_TIME] ~= nil then closeTime = string2Int(streams[KEY_POWER_OFF_TIME]) end
    if streams[KEY_POWER_ON_TIME] ~= nil then openTime = string2Int(streams[streams[KEY_POWER_ON_TIME]]) end
end
local function binToModel(binData)
    ...
    if dataType == 0x02 or dataType == 0x03 or dataType == 0x04 or dataType == 0x05 then
        ...
        elseif cmdType == BYTE_CMD_LOCK then
            ...
            if messageBytes[5] ~= 0xFF then lockCoolTemp = messageBytes[5] end
            if messageBytes[6] ~= 0xFF then lockCoolTemp = messageBytes[6] end
