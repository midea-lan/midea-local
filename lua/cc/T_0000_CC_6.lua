...
function jsonToModel(jsonCmd)
    ...
    if (streams[KEY_SMALL_TEMPERATURE] ~= nil) then
        temperatureDecimals = checkBoundary(streams[KEY_SMALL_TEMPERATURE], 0,
                                            0.9)
    end
...
local function binToModel(binData)
    if (#binData < 23) then return nil end
    local messageBytes = binData
    if (dataType == 0x02 or dataType == 0x03 or dataType == 0x04 or dataType ==
        0x05) then
...
function jsonToData(jsonCmd)
    ...
    elseif (control) then
        ...
        for i = 0, 23 do bodyBytes[i] = 0 end
        bodyBytes[0] = 0xC3
        bodyBytes[1] = bit.bor(bit.band(powerValue, 0x80),
                               bit.band(modeValue, 0x1F))
        bodyBytes[2] = fanspeedValue
        bodyBytes[3] = temperatureValue
        if ((openTime == nil) or (openTime == 3825)) then openTime = 0 end
        bodyBytes[4] = math.floor(openTime / 15)
        if ((closeTime == nil) or (closeTime == 3825)) then closeTime = 0 end
        bodyBytes[5] = math.floor(closeTime / 15)
        bodyBytes[6] = bit.bor(bit.bor(bit.band(ecoValue, 0x01),
                                       bit.band(swingUDSwitchValue, 0x04)),
                               bit.bor(bit.band(exhaustValue, 0x08),
                                       bit.band(PTCSettingValue, 0x30)))
        bodyBytes[7] = 0xFF
        bodyBytes[8] = bit.bor(bit.bor(bit.band(sleepSwitchValue, 0x10),
                                       bit.band(digitDisplaySwitchValue, 0x08)),
...
function dataToJson(jsonCmd)
    ...
    if ((cmdType == BYTE_CMD_COMMON) or (cmdType == BYTE_CMD_QUERY_COMMON)) then
        ...
        streams[KEY_INDOOR_TEMPERATURE] = int2String(
                                              (indoorTemperature - 40) / 2)
        streams[KEY_EVAPORATOR_ENTRANCE_TEMPERATURE] = int2String(
                                                           (evaporatorEntranceTemp -
                                                               40) / 2)
        streams[KEY_EVAPORATOR_EXIT_TEMPERATURE] = int2String(
                                                       (evaporatorExitTemp - 40) /
                                                           2)
        ...
        if (errorCodeMachineStyle == 0x80) then
            streams[KEY_ERROR_FROM_MACHINE_STYLE] =
                VALUE_ERROR_FROM_OUTSIDE_MACHINE
        elseif (errorCodeMachineStyle == 0x00) then
            streams[KEY_ERROR_FROM_MACHINE_STYLE] =
                VALUE_ERROR_FROM_INSIDE_MACHINE
        end
