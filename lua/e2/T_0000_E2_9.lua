...
function jsonToModel(stateJson, controlJson)
    ...
    if controlCmd[KEY_POWER] ~= nil then
        ...
        if controlCmd[KEY_MODE] ~= nil then modeStr = controlCmd[KEY_MODE] end
        ...
        if controlCmd[KEY_TEMPERATURE] ~= nil then temperatureStr = controlCmd[KEY_TEMPERATURE] end
        temperatureValue = checkBoundary(temperatureStr, 30, 75)
        local heatStr = oldState[KEY_HEAT]
        if controlCmd[KEY_HEAT] ~= nil then heatStr = controlCmd[KEY_HEAT] end
        ...
        if controlCmd[KEY_SAFE] ~= nil then safeStr = controlCmd[KEY_SAFE] end
        ...
        if controlCmd[KEY_PROTECT] ~= nil then protectStr = controlCmd[KEY_PROTECT] end
        ...
        if controlCmd[KEY_SLEEP] ~= nil then sleepStr = controlCmd[KEY_SLEEP] end
...
function dataToJson(jsonCmd)
    ...
    if
        ((msgType == BYTE_CONTROL_REQUEST) and (msgSubType == 0x01))
        or ((msgType == BYTE_CONTROL_REQUEST) and (msgSubType == 0x02))
        or ((msgType == BYTE_CONTROL_REQUEST) and (msgSubType == 0x04))
        or ((msgType == BYTE_QUERYL_REQUEST) and (msgSubType == 0x01))
        or ((msgType == BYTE_AUTO_REPORT) and (msgSubType == 0x01))
    then
        for i = 0, bodyLength do
        ...
        if modeValue4 == 0x40 then modeStr = VALUE_MODE_STERILIZATION end
