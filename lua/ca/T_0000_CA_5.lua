...
function jsonToModel(jsonCmd)
    ...
    if streams["humidify_setting"] ~= nil then humiditySetting = checkBoundary(streams["humidify_setting"], 1, 99) end
...
function binToModel(binData)
    ...
    if
        (dataType == 0x02 and messageBytes[0] == 0x00)
        or (dataType == 0x03 and messageBytes[0] == 0x00)
        or (dataType == 0x04 and messageBytes[0] == 0x02)
    then
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        if foodSite ~= nil then bodyBytes[25] = bit.band(foodSite, 0x0F) end
        if performanceMode ~= nil then bodyBytes[27] = bit.band(performanceMode, 0x80) end
        if normalTemperatureLevel ~= nil then bodyBytes[28] = normalTemperatureLevel end
        if functionZoneLevel ~= nil then bodyBytes[29] = functionZoneLevel end
...
function dataToJson(jsonCmd)
    ...
    if
        (dataType == 0x02 and bodyBytes[0] == 0x00)
        or (dataType == 0x03 and bodyBytes[0] == 0x00)
        or (dataType == 0x04 and bodyBytes[0] == 0x02)
    then
