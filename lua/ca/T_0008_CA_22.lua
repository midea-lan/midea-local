...
local function updateGlobalPropertyValueByJson(luaTable)
    ...
    if luaTable[KEY_CHILLING_ROOM_TEMP] ~= nil then
        propertyTable["chillingRoomTemp"] = string2Int(luaTable[KEY_CHILLING_ROOM_TEMP])
    else
        propertyTable["chillingRoomTemp"] = 0xff
    end
    if luaTable[KEY_FREEZING_ROOM_TEMP] ~= nil then
        propertyTable["freezingRoomTemp"] = string2Int(luaTable[KEY_FREEZING_ROOM_TEMP])
    else
...
local function updateGlobalPropertyValueByByte(messageBytes)
    ...
    if dataType == 0x02 then
        if messageBytes[0] == 0x00 then
            ...
            propertyTable["powerSavingAuto"] = bit.rshift(bit.band(messageBytes[1], 0x010), 4)
    ...
    if dataType == 0x03 then
        if messageBytes[0] == 0x00 then
            ...
            propertyTable["chillingDoorStatus"] = bit.band(messageBytes[8], 0x01)
            propertyTable["freezingDoorStatus"] = bit.rshift(bit.band(messageBytes[8], 0x02), 1)
            propertyTable["iceDoorStatus"] = bit.rshift(bit.band(messageBytes[8], 0x04), 2)
            propertyTable["vegetableDoorStatus"] = bit.rshift(bit.band(messageBytes[8], 0x08), 3)
            propertyTable["instantaneousPower"] = bit.lshift(messageBytes[10], 8) + messageBytes[9]
            propertyTable["dailyEnergy"] = bit.lshift(messageBytes[12], 8) + messageBytes[11]
            if messageBytes[13] ~= nil and messageBytes[14] ~= nil then
                if messageBytes[13] ~= 0xFF or messageBytes[14] ~= 0xFF then
                    propertyTable["errorCode"] = bit.lshift(messageBytes[14], 8) + messageBytes[13]
                end
        ...
        if messageBytes[0] == 0xFE then
            ...
            propertyTable["errorTimes"] = bit.lshift(messageBytes[8], 8) + messageBytes[7]
            local fv = {}
            for i = 1, 7 do
                fv[i] = messageBytes[8 + i]
            end
        ...
        if messageBytes[0] == 0x22 then
            ...
            propertyTable["chillingDoorStatus"] = bit.band(messageBytes[1], 0x01)
            propertyTable["vegetableDoorStatus"] = bit.rshift(bit.band(messageBytes[1], 0x02), 1)
            propertyTable["iceDoorStatus"] = bit.rshift(bit.band(messageBytes[1], 0x04), 2)
            propertyTable["freezingUpStatus"] = bit.rshift(bit.band(messageBytes[1], 0x08), 3)
            propertyTable["freezingDownStatus"] = bit.rshift(bit.band(messageBytes[1], 0x10), 4)
            propertyTable["chillingRoomTempThan12"] = bit.band(messageBytes[2], 0x01)
            propertyTable["freezingRoomTempThan10"] = bit.rshift(bit.band(messageBytes[2], 0x02), 1)
        end
        if messageBytes[0] == 0x03 then
            propertyTable["functionType"] = 0x03
            propertyTable["rt_sensor"] = bit.lshift(messageBytes[2], 8) + messageBytes[1]
            propertyTable["r_sensor"] = bit.lshift(messageBytes[4], 8) + messageBytes[3]
            propertyTable["rd_sensor"] = bit.lshift(messageBytes[6], 8) + messageBytes[5]
            propertyTable["rd2_sensor"] = bit.lshift(messageBytes[8], 8) + messageBytes[7]
            propertyTable["f_sensor"] = bit.lshift(messageBytes[10], 8) + messageBytes[9]
            propertyTable["fd_sensor"] = bit.lshift(messageBytes[12], 8) + messageBytes[11]
            propertyTable["i_sensor"] = bit.lshift(messageBytes[14], 8) + messageBytes[13]
            propertyTable["rh_sensor"] = bit.lshift(messageBytes[16], 8) + messageBytes[15]
            ...
            if messageBytes[23] ~= nil and messageBytes[24] ~= nil then
                propertyTable["cSensor"] = bit.lshift(messageBytes[24], 8) + messageBytes[23]
            end
        end
    end
...
local function assembleJsonByGlobalProperty()
    ...
    if propertyTable["functionType"] == 0x00 then
        ...
        if propertyTable["outRoomTemp"] ~= nil then
            streams["out_room_temp"] = int2String(buma(propertyTable["outRoomTemp"]) / 10)
        end
