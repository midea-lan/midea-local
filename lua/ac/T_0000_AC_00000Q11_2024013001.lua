...
local function binToModel(binData, deviceSN8)
	...
	if ((dataType == 0x02 and messageBytes[0] == 0xB0) or (dataType == 0x03 and messageBytes[0] == 0xB1)) then
		...
		for i = 1, keyP["propertyNumber"] do
			...
			if (messageBytes[cursor + 0] == 0xE0 and messageBytes[cursor + 1] == 0x00) then
				...
				keyP["ieco_target_rate"] = bit.bor(messageBytes[cursor + 5],
					bit.bor(bit.lshift(messageBytes[cursor + 6], 8)))
				keyP["ieco_indoor_wind_speed_level"] = messageBytes[cursor + 7]
				keyP["ieco_indoor_wind_speed"] = bit.bor(messageBytes[cursor + 8],
					bit.bor(bit.lshift(messageBytes[cursor + 9], 8)))
				keyP["ieco_outdoor_wind_speed_level"] = messageBytes[cursor + 10]
				keyP["ieco_outdoor_wind_speed"] = bit.bor(messageBytes[cursor + 11],
					bit.bor(bit.lshift(messageBytes[cursor + 12], 8)))
			...
			if (messageBytes[cursor + 0] == 0x90 and messageBytes[cursor + 1] == 0x00) then
				...
				keyP["cool_strong_wind_speed"] = bit.bor(messageBytes[cursor + 11],
					bit.bor(bit.lshift(messageBytes[cursor + 12], 8)))
				keyP["cool_strong_wind_amount"] = bit.bor(messageBytes[cursor + 13],
					bit.bor(bit.lshift(messageBytes[cursor + 14], 8)))
				cursor = cursor + 15
			end
	...
	if (messageBytes[0] == 0xC1) then
		if (messageBytes[3] == 0x44) then
			keyP["real_time_power_10"] = (bcd2Int(messageBytes[16]) + bcd2Int(messageBytes[17]) / 100 + bcd2Int(messageBytes[18]) / 10000) *
			10000

			--keyP["real_time_power_10"] = tonumber((messageBytes[16] + messageBytes[17]/100 + messageBytes[18]/10000),10)

			keyP["real_time_power"] = bit.bor(bit.bor(bit.lshift(messageBytes[16], 16), bit.lshift(messageBytes[17], 8)),
				messageBytes[18])
...
function jsonToData(jsonCmd)
	...
	elseif (control) then
		...
		if (keyP["propertyNumber"] == 0) then
			...
			bodyBytes[1] = bit.bor(bit.bor(keyP["powerValue"], keyB["BYTE_CLIENT_MODE_MOBILE"]),
			...
			if (keyP["temperature"] >= 16) then
				...
				bodyBytes[2] = bit.bor(bit.bor(bit.band(keyP["modeValue"], 0xE0), 0),
					bit.lshift(bit.band(keyP["smallTemperature"], 0x01), 4))
			end
			...
			if (keyP["openTimerSwitch"] == keyB["BYTE_START_TIMER_SWITCH_ON"]) then
				bodyBytes[4] = bit.bor(bit.bor(keyP["openTimerSwitch"], bit.lshift(keyP["openHour"], 2)),
			...
			if (keyP["closeTimerSwitch"] == keyB["BYTE_CLOSE_TIMER_SWITCH_ON"]) then
				bodyBytes[5] = bit.bor(bit.bor(keyP["closeTimerSwitch"], bit.lshift(keyP["closeHour"], 2)),
			...
			elseif (keyP["comfortableSleepValue"] == keyB["BYTE_SLEEP_ON"] and comfortByte ~= nil) then
				bodyBytes[11] = bit.bor(checkBoundary(comfortByte[1], 17, 30) - 17,
					bit.lshift((checkBoundary(comfortByte[2], 17, 30) - 17), 4))
				bodyBytes[12] = bit.bor(checkBoundary(comfortByte[3], 17, 30) - 17,
