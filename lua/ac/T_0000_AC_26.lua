        ...
        if deviceSN8 == "12035" then
            ...
            if noWindSencePre == nil then
                ...
                if noWindSencePre == 2 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 3 then keyP["no_wind_sense"] = 1 end
                end
                if noWindSencePre == 3 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 2 then keyP["no_wind_sense"] = 1 end
                end
                if noWindSencePre == 1 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 5 then keyP["no_wind_sense"] = 2 end
                end
                if noWindSencePre == 1 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 4 then keyP["no_wind_sense"] = 3 end
                end
                if noWindSencePre == 2 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 4 then keyP["no_wind_sense"] = 0 end
                end
                if noWindSencePre == 3 then
                    if streams[keyT["KEY_NO_WIND_SENSE"]] == 5 then keyP["no_wind_sense"] = 0 end
                end
            end
        end
    end
...
local function binToModel(binData)
    ...
    then
        ...
        --舒省
        if #binData >= 22 then keyP["comfortPowerSave"] = bit.band(messageBytes[22], 0x01) end
    end
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if messageBytes[cursor + 0] == 0x32 and messageBytes[cursor + 1] == 0x00 then
                ...
                if deviceSN8 == "50939" or deviceSN8 == "51001" or deviceSN8 == "Z1304" then
                    if messageBytes[cursor + 4] == 2 then keyP["wind_avoid"] = 1 end
                end
                cursor = cursor + 5
            end
            ...
            if messageBytes[cursor + 0] == 0x43 and messageBytes[cursor + 1] == 0x00 then
                ...
                if messageBytes[cursor + 4] == 0x02 then keyP["prevent_straight_wind"] = 0x02 end
                if messageBytes[cursor + 4] == 0x03 then keyP["gentle_wind_sense"] = 0x03 end
                if messageBytes[cursor + 4] == 0x04 then keyP["no_wind_sense"] = 0x01 end
                cursor = cursor + 5
            end
        end
    end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        if control then
            ...
            if
                (control[keyT["KEY_SCREEN_DISPLAY"]] ~= nil)
                and (deviceSN8 ~= "12035")
                and (deviceSN8 ~= "50939")
                and (deviceSN8 ~= "51001")
                and (deviceSN8 ~= "Z1304")
        ...
        if keyP["propertyNumber"] == 0 then
            ...
            if keyP["buzzerValue"] ~= nil then
                ...
                bodyBytes[1] = bit.bor(
                    bit.bor(keyP["powerValue"], keyB["BYTE_CLIENT_MODE_MOBILE"]),
                    bit.bor(keyB["BYTE_TIMER_METHOD_REL"], 0x40)
                )
            end
            ...
            if control["ptc"] ~= nil then bodyBytes[9] = bit.bor(bodyBytes[9], 0x10) end
            ...
            if keyP["wind_avoid"] ~= nil then
                ...
                if (deviceSN8 == "50939") or (deviceSN8 == "51001") or (deviceSN8 == "Z1304") then
                    bodyBytes[cursor + 0] = 0x32
                    if keyP["wind_avoid"] == 1 then bodyBytes[cursor + 3] = 0x02 end
                end
                cursor = cursor + 4
            end
...
function dataToJson(jsonCmd)
    ...
    if keyP["propertyNumber"] == 0 then
        ...
        if
            (keyP["dryValue"] == keyB["BYTE_DRY_ON"])
            and ((keyP["modeValue"] == keyB["BYTE_MODE_COOL"]) or (keyP["modeValue"] == keyB["BYTE_MODE_DRY"]))
        then
