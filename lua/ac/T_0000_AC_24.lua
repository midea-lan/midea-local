...
local function jsonToModel(jsonCmd, jsonType)
    ...
    if streams[keyT["KEY_TEMPERATURE"]] ~= nil then
        ...
        if streams[keyT["KEY_TEMPERATURE"]] < 40 then
            ...
            keyP["temperature"] = checkBoundary(streams[keyT["KEY_TEMPERATURE"]], 60, 86)
    ...
    if jsonType == "control" and streams[keyT["KEY_PREVENT_STRAIGHT_WIND_LR"]] ~= nil then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["prevent_straight_wind_lr"] = checkBoundary(streams[keyT["KEY_PREVENT_STRAIGHT_WIND_LR"]], 0, 1)
    end
    ...
    if streams[keyT["KEY_FRESH_FILTER_RESET"]] ~= nil then
        if tonumber(streams[keyT["KEY_FRESH_FILTER_RESET"]]) == 0x01 then keyP["fresh_filter_reset"] = 0x80 end
    end
...
local function binToModel(binData, deviceSN8)
    ...
    then
        ...
        if #binData >= 23 then keyP["degree8_heat"] = bit.rshift(bit.band(messageBytes[21], 0x80), 7) end
        if #binData >= 29 then
    ...
    if dataType == 0x03 and messageBytes[0] == 0xB5 then
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if messageBytes[cursor + 0] == 0x09 and messageBytes[cursor + 1] == 0x00 then cursor = cursor + 4 end
            if messageBytes[cursor + 0] == 0x2C and messageBytes[cursor + 1] == 0x02 then cursor = cursor + 4 end
        end
    end
end
