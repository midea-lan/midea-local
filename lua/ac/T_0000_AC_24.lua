...
local function jsonToModel(jsonCmd, jsonType)
    ...
    if (streams[keyT["KEY_TEMPERATURE"]] ~= nil) then
        ...
        if (streams[keyT["KEY_TEMPERATURE"]] < 40) then
            ...
            keyP["temperature"] = checkBoundary(
                                      streams[keyT["KEY_TEMPERATURE"]], 60, 86)
...
local function binToModel(binData, deviceSN8)
    ...
    if ((dataType == 0x02 and messageBytes[0] == 0xC0) or
        ...
        keyP["fresh_filter_timeout"] = bit.rshift(
                                           bit.band(messageBytes[13], 0x40), 6)
        ...
        keyP["temperature_unit"] = bit.rshift(bit.band(messageBytes[10], 0x04),
                                              2)
        ...
        if (#binData >= 23) then
            keyP["degree8_heat"] = bit.rshift(bit.band(messageBytes[21], 0x80),
                                              7)
        end
        if (#binData >= 29) then
    ...
    if (messageBytes[0] == 0xC1) then
        if (messageBytes[3] == 0x44) then
            keyP["real_time_power"] = bit.bor(
                                          bit.bor(bit.lshift(messageBytes[16],
                                                             16), bit.lshift(
                                                      messageBytes[17], 8)),
                                          messageBytes[18])
        end
