...
local function JsonToModel(jsonCmd, jsonType)
    ...
    if (streams[keyT["KEY_NEWWIND_MODE"]] ~= nil) then
        keyP["newWindModeValue"] = checkBoundary(
                                       streams[keyT["KEY_NEWWIND_MODE"]], 0, 20)
    end
    ...
    if (streams["indoor_elec_expansion_valve_one"] ~= nil) then
        keyP["indoor_elec_expansion_valve_one"] =
            streams["indoor_elec_expansion_valve_one"]
    end
    if (streams["indoor_elec_expansion_valve_two"] ~= nil) then
        keyP["indoor_elec_expansion_valve_two"] =
            streams["indoor_elec_expansion_valve_two"]
    end
    ...
    if (streams["force_wind_machine_one_speed"] ~= nil) then
        keyP["force_wind_machine_one_speed"] =
            streams["force_wind_machine_one_speed"]
    end
    if (streams["force_wind_machine_two_speed"] ~= nil) then
        keyP["force_wind_machine_two_speed"] =
            streams["force_wind_machine_two_speed"]
    end
    if (streams["force_expansion_valve_one_speed"] ~= nil) then
        keyP["force_expansion_valve_one_speed"] =
            streams["force_expansion_valve_one_speed"]
    end
    if (streams["force_expansion_valve_two_speed"] ~= nil) then
        keyP["force_expansion_valve_two_speed"] =
            streams["force_expansion_valve_two_speed"]
    end
    ...
    if (streams["fengguan_indoor_one_speed_level"] ~= nil) then
        keyP["fengguan_indoor_one_speed_level"] =
            streams["fengguan_indoor_one_speed_level"]
    end
    if (streams["fengguan_indoor_two_speed_level"] ~= nil) then
        keyP["fengguan_indoor_two_speed_level"] =
            streams["fengguan_indoor_two_speed_level"]
    end
    ...
    if (streams["water_temp_linkage_support"] ~= nil) then
        keyP["water_temp_linkage_support"] =
            streams["water_temp_linkage_support"]
    end
    ...
    if (streams["save_energy_mode_effective_countdown"] ~= nil) then
        keyP["save_energy_mode_effective_countdown"] =
            streams["save_energy_mode_effective_countdown"]
    ...
    if (streams["fresh_air_exhaust_fan_speed"] ~= nil) then
        keyP["fresh_air_exhaust_fan_speed"] =
            streams["fresh_air_exhaust_fan_speed"]
    end
    if (streams["fresh_air_intake_fan_speed"] ~= nil) then
        keyP["fresh_air_intake_fan_speed"] =
            streams["fresh_air_intake_fan_speed"]
    end
end
local function binToModel(binData)
    ...
    if (dataType == 0x20 or dataType == 0x11 or dataType == 0xF1) then
        ...
        if (#binData > 44) then
            ...
            keyP["female_care"] =
                bit.rshift(bit.band(messageBytes[45], 0x80), 7)
        end
        if (#binData > 45) then
        ...
        if (#binData > 51) then
            ...
            keyP["has_shunt_valve"] = bit.rshift(
                                          bit.band(messageBytes[52], 0x02), 1)
            keyP["heater_heating_switch"] =
                bit.rshift(bit.band(messageBytes[52], 0x04), 2)
            keyP["stop_warm_support"] = bit.rshift(
                                            bit.band(messageBytes[52], 0x08), 3)
            keyP["water_temp_linkage_support"] =
                bit.rshift(bit.band(messageBytes[52], 0x10), 4)
            keyP["water_temp_linkage_switch"] =
                bit.rshift(bit.band(messageBytes[52], 0x20), 5)
            keyP["water_model_dry"] = bit.rshift(
                                          bit.band(messageBytes[52], 0x40), 6)
            keyP["airoptimization_unsupport"] =
                bit.rshift(bit.band(messageBytes[52], 0x80), 7)
        end
        if (#binData > 52) then
            keyP["water_model_force_temp"] = bit.band(messageBytes[53], 0x7F)
            keyP["smart_water_model"] = bit.rshift(
                                            bit.band(messageBytes[53], 0x80), 7)
        end
        if (#binData > 53) then
            keyP["single_control"] = bit.band(messageBytes[54], 0x01)
            keyP["single_select"] = bit.rshift(bit.band(messageBytes[54], 0x02),
                                               1)
            keyP["single_status"] = bit.rshift(bit.band(messageBytes[54], 0x04),
                                               2)
            keyP["new_wind_single_control"] =
                bit.rshift(bit.band(messageBytes[54], 0x08), 3)
            keyP["has_quick_fry"] = bit.rshift(bit.band(messageBytes[54], 0x10),
                                               4)
            keyP["quick_fry"] = bit.rshift(bit.band(messageBytes[54], 0x20), 5)
            keyP["has_prepare_food"] = bit.rshift(
                                           bit.band(messageBytes[54], 0x40), 6)
            keyP["prepare_food"] = bit.rshift(bit.band(messageBytes[54], 0x80),
                                              7)
        end
        if (#binData > 54) then
            keyP["new_wind_machine_intake_switch"] =
                bit.rshift(bit.band(messageBytes[55], 0x20), 5)
            keyP["new_wind_machine_exhaust_switch"] =
                bit.rshift(bit.band(messageBytes[55], 0x40), 6)
            keyP["has_follow_body_sense"] =
                bit.rshift(bit.band(messageBytes[55], 0x80), 7)
        end
        if (#binData > 56) then
            keyP["save_energy_mode_effective_countdown"] = bit.bor(bit.lshift(
                                                                       bit.band(
                                                                           messageBytes[56],
                                                                           0x7F),
                                                                       4),
                                                                   bit.band(
                                                                       messageBytes[55],
                                                                       0x0F))
            keyP["nobody_off_switch"] = bit.rshift(
                                            bit.band(messageBytes[55], 0x10), 4)
            keyP["nobody_off_time"] = bit.rshift(
                                          bit.band(messageBytes[57], 0XFC), 2)
        end
        ...
        if (#binData > 61) then
            keyP["fresh_air_intake_fan_speed"] =
                bit.band(messageBytes[62], 0x7F)
            keyP["has_new_wind_machine_intake_switch"] =
                bit.rshift(bit.band(messageBytes[62], 0x80), 7)
        end
        if (#binData > 62) then
            keyP["fresh_air_exhaust_fan_speed"] =
                bit.band(messageBytes[63], 0x7F)
            keyP["has_new_wind_machine_exhaust_switch"] =
                bit.rshift(bit.band(messageBytes[63], 0x80), 7)
        end
        if (#binData > 63) then
            keyP["quick_prepare_food_angle"] = bit.band(messageBytes[64], 0x7F)
            keyP["has_quick_prepare_food_angle"] =
                bit.rshift(bit.band(messageBytes[64], 0x80), 7)
        end
        if (#binData > 64) then
            keyP["double_engine_heat"] = bit.band(messageBytes[65], 0x01)
            keyP["has_double_engine_heat"] =
                bit.rshift(bit.band(messageBytes[65], 0x02), 1)
            keyP["smart_hybird"] = bit.rshift(bit.band(messageBytes[65], 0x04),
                                              2)
            keyP["has_smart_hybird"] = bit.rshift(
                                           bit.band(messageBytes[65], 0x08), 3)
            keyP["smart_hybird_pause"] = bit.rshift(
                                             bit.band(messageBytes[65], 0x10), 4)
            keyP["c003_protocol_support"] =
                bit.rshift(bit.band(messageBytes[65], 0x20), 5)
            keyP["c003_protocol_inner_support"] =
                bit.rshift(bit.band(messageBytes[65], 0x40), 6)
            keyP["no_wind_swing_ud_support"] =
                bit.rshift(bit.band(messageBytes[65], 0x80), 7)
        end
        if (#binData > 65) then
            keyP["independent_strong_support"] =
                bit.band(messageBytes[66], 0x01)
        end
    end
    if (dataType == 0x10) then
        ...
        if (#binData > 57) then
            keyP["new_wind_machine_operating_mode"] =
                bit.band(messageBytes[58], 0x0F)
            keyP["special_machine_type"] = bit.rshift(
                                               bit.band(messageBytes[58], 0x7F),
                                               4)
        end
        if (#binData > 89) then
        ...
        if (#binData > 95) then
            ...
            keyP["radar_install_status"] = bit.rshift(
                                               bit.band(messageBytes[96], 0x02),
                                               1)
            keyP["radar_monitor_people_status_report"] =
                bit.rshift(bit.band(messageBytes[96], 0x04), 2)
            keyP["nobody_off_status"] = bit.rshift(
                                            bit.band(messageBytes[96], 0x08), 3)
            keyP["double_engine_heat_status"] =
                bit.rshift(bit.band(messageBytes[96], 0x20), 5)
            keyP["smart_hybird_status"] = bit.rshift(
                                              bit.band(messageBytes[96], 0x40),
                                              6)
            keyP["humidity_switch_fault"] =
                bit.rshift(bit.band(messageBytes[96], 0x80), 7)
        end
        if (#binData > 106) then
            ...
            if (keyP["target_machine_info"] == 0x01) then
                if (#binData > 107) then
                    ...
                    keyP["has_fengguan_mini_new_wind"] =
                        bit.rshift(bit.band(messageBytes[108], 0x02), 1)
                end
            end
            if (keyP["target_machine_info"] == 0x02) then
                keyP["defrosting_display"] =
                    bit.rshift(bit.band(messageBytes[108], 0x04), 2)
    ...
    if (dataType == 0x12) then
        ...
        if (bit.band(messageBytes[7], 0x80) == 0x80) then
            ...
            keyP["tw1_in_water_temp"] =
                (messageBytes[6] + messageBytes[7] * 256) / 100
        end
        if (bit.band(messageBytes[9], 0x80) == 0x80) then
            ...
            keyP["tw1_out_water_temp"] =
                (messageBytes[8] + messageBytes[9] * 256) / 100
        end
        if (bit.band(messageBytes[19], 0x80) == 0x80) then
            ...
            keyP["tw_out_water_temp"] = (messageBytes[18] + messageBytes[19] *
                                            256) / 100
        end
        if (#binData > 25) then
    ...
    if (dataType == 0xC0) then
        ...
        if (messageBytes[3] == 0x02) then
            ...
            keyP["has_strong_wind"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 1],
                                                     0x02), 1)
            keyP["strongWindValue"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 2],
                                                     0x02), 1)
            keyP["no_wind_sense"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 2],
                                                   0x04), 2)
            keyP["dryValue"] = bit.rshift(
                                   bit.band(messageBytes[cursor + 2], 0x08), 3)
            keyP["follow_body_sense"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 2],
                                                       0x10), 4)
            keyP["cool_hot_sense"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 2],
                                                    0x20), 5)
            keyP["swingLeftUDValue"] = bit.rshift(bit.band(
                                                      messageBytes[cursor + 2],
                                                      0x40), 6)
            keyP["has_wind_swing_ud"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 1],
                                                       0x40), 6)
            keyP["swingUpLRValue"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 2],
                                                    0x80), 7)
            keyP["has_wind_swing_lr"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 1],
                                                       0x80), 7)
            keyP["has_ptc"] = bit.band(messageBytes[cursor + 3], 0x01)
            keyP["PTCValue"] = bit.band(messageBytes[cursor + 4], 0x01)
            keyP["PTCDependT4Value"] = bit.rshift(bit.band(
                                                      messageBytes[cursor + 4],
                                                      0x02), 1)
            keyP["wind_straight"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 4],
                                                   0x04), 2)
            keyP["wind_avoid"] = bit.rshift(
                                     bit.band(messageBytes[cursor + 4], 0x08), 3)
            keyP["fengguan_remove_odor"] =
                bit.rshift(bit.band(messageBytes[cursor + 4], 0x10), 4)
            keyP["has_common_remove_odor"] =
                bit.rshift(bit.band(messageBytes[cursor + 3], 0x10), 4)
            keyP["self_clean"] = bit.rshift(
                                     bit.band(messageBytes[cursor + 4], 0x20), 5)
            keyP["energySaveValue"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 4],
                                                     0x40), 6)
            keyP["nobody_energy_save"] =
                bit.rshift(bit.band(messageBytes[cursor + 4], 0x80), 7)
            keyP["manuPurify"] = bit.band(messageBytes[cursor + 6], 0x01)
            keyP["has_manu_purify"] = bit.band(messageBytes[cursor + 5], 0x01)
            keyP["autoPurify"] = bit.rshift(
                                     bit.band(messageBytes[cursor + 6], 0x02), 1)
            keyP["has_auto_purify"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 5],
                                                     0x02), 1)
            keyP["autoHumi"] = bit.rshift(
                                   bit.band(messageBytes[cursor + 6], 0x04), 2)
            keyP["has_auto_humi"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 5],
                                                   0x04), 2)
            keyP["manuHumi"] = bit.rshift(
                                   bit.band(messageBytes[cursor + 6], 0x08), 3)
            keyP["has_manu_humi"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 5],
                                                   0x04), 2)
            keyP["new_wind_machine"] = bit.rshift(bit.band(
                                                      messageBytes[cursor + 6],
                                                      0x10), 4)
            keyP["new_wind_machine_link"] =
                bit.rshift(bit.band(messageBytes[cursor + 6], 0x20), 5)
            keyP["stop_warm"] = bit.rshift(
                                    bit.band(messageBytes[cursor + 6], 0x80), 7)
            keyP["eco"] = bit.band(messageBytes[cursor + 8], 0x01)
            keyP["has_eco"] = bit.band(messageBytes[cursor + 7], 0x01)
            keyP["prevent_super_cool"] =
                bit.rshift(bit.band(messageBytes[cursor + 8], 0x02), 1)
            keyP["has_prevent_super_cool"] =
                bit.rshift(bit.band(messageBytes[cursor + 7], 0x02), 1)
            keyP["new_wind_model_intake_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 8], 0x04), 2)
            keyP["new_wind_model_exhaust_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 8], 0x08), 3)
            keyP["new_wind_model_mute"] =
                bit.rshift(bit.band(messageBytes[cursor + 8], 0x10), 4)
            keyP["tube_protect"] = bit.rshift(
                                       bit.band(messageBytes[cursor + 8], 0x20),
                                       5)
            keyP["prepare_food"] = bit.rshift(
                                       bit.band(messageBytes[cursor + 8], 0x40),
                                       6)
            keyP["has_prepare_food"] = bit.rshift(bit.band(
                                                      messageBytes[cursor + 7],
                                                      0x40), 6)
            keyP["quick_fry"] = bit.rshift(
                                    bit.band(messageBytes[cursor + 8], 0x80), 7)
            keyP["has_quick_fry"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 7],
                                                   0x80), 7)
            keyP["power_on_timer"] = bit.band(messageBytes[cursor + 15], 0x01)
            keyP["has_power_on_timer"] =
                bit.band(messageBytes[cursor + 14], 0x01)
            keyP["power_off_timer"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 15],
                                                     0x02), 1)
            keyP["has_power_off_timer"] =
                bit.rshift(bit.band(messageBytes[cursor + 14], 0x02), 1)
            keyP["new_wind_model_on_timer"] =
                bit.rshift(bit.band(messageBytes[cursor + 15], 0x04), 2)
            keyP["has_new_wind_model_on_timer"] =
                bit.rshift(bit.band(messageBytes[cursor + 14], 0x04), 2)
            keyP["new_wind_model_off_timer"] =
                bit.rshift(bit.band(messageBytes[cursor + 15], 0x08), 3)
            keyP["has_new_wind_model_off_timer"] =
                bit.rshift(bit.band(messageBytes[cursor + 14], 0x08), 3)
            keyP["comfortableSleepValue"] =
                bit.rshift(bit.band(messageBytes[cursor + 15], 0x10), 4)
            if (keyP["comfortableSleepValue"] == 1) then
                keyP["comfortableSleepValue"] = 0x30
            end
            keyP["has_comfort_sleep"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 14],
                                                       0x10), 4)
            keyP["buzzerValue"] = bit.rshift(
                                      bit.band(messageBytes[cursor + 15], 0x20),
                                      5)
            keyP["fresh_air_remove_odor"] =
                bit.rshift(bit.band(messageBytes[cursor + 15], 0x40), 6)
            keyP["has_remove_odor"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 14],
                                                     0x40), 6)
            keyP["voice_control"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 15],
                                                   0x80), 7)
            keyP["water_model_power"] =
                bit.band(messageBytes[cursor + 29], 0x01)
            keyP["water_model_power_save"] =
                bit.rshift(bit.band(messageBytes[cursor + 29], 0x02), 1)
            keyP["water_model_clean"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 29],
                                                       0x04), 2)
            keyP["water_model_temperature_auto"] =
                bit.rshift(bit.band(messageBytes[cursor + 29], 0x08), 3)
            keyP["water_model_ptc"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 29],
                                                     0x10), 4)
            keyP["water_model_go_out"] =
                bit.rshift(bit.band(messageBytes[cursor + 29], 0x20), 5)
            keyP["has_water_model_temperature_set"] =
                bit.rshift(bit.band(messageBytes[cursor + 30], 0x40), 6)
            keyP["has_water_mode"] = bit.rshift(bit.band(
            ...
            keyP["water_model_temperature_set"] =
                (messageBytes[cursor + 31] - 50) / 2
            keyP["up_down_wind_direction"] =
                bit.band(messageBytes[cursor + 32], 0x0F)
            keyP["left_right_wind_direction"] =
                bit.rshift(bit.band(messageBytes[cursor + 32], 0xF0), 4)
            keyP["fresh_air_mode"] = bit.band(messageBytes[cursor + 33], 0x1F)
            keyP["has_fresh_air_mode"] =
                bit.rshift(bit.band(messageBytes[cursor + 33], 0x80), 7)
            keyP["wind_strength"] = bit.rshift(bit.band(
                                                   messageBytes[cursor + 33],
                                                   0x20), 5)
            keyP["exhaust_strength"] = bit.rshift(bit.band(
                                                      messageBytes[cursor + 33],
                                                      0x40), 6)
            keyP["fresh_air_fan_speed"] =
                bit.band(messageBytes[cursor + 34], 0x7F)
            keyP["has_fresh_air_fan_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 34], 0x80), 7)
            keyP["quick_prepare_food_angle"] = bit.band(
                                                   messageBytes[cursor + 35],
                                                   0x7F)
            keyP["has_quick_prepare_food_angle"] =
                bit.rshift(bit.band(messageBytes[cursor + 35], 0x80), 7)
            ...
            keyP["has_new_wind_machine_intake_switch"] = bit.band(
                                                             messageBytes[cursor +
                                                                 46], 0x01)
            keyP["new_wind_machine_intake_switch"] = bit.band(
                                                         messageBytes[cursor +
                                                             47], 0x01)
            keyP["has_new_wind_machine_exhaust_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 46], 0x02), 1)
            keyP["new_wind_machine_exhaust_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 47], 0x02), 1)
            keyP["has_heater_heating_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 46], 0x80), 7)
            keyP["heater_heating_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 47], 0x80), 7)
            keyP["has_airoptimization_or_total_time"] =
                bit.rshift(bit.band(messageBytes[cursor + 46], 0x40), 6)
            keyP["airoptimization_or_total_time"] =
                bit.rshift(bit.band(messageBytes[cursor + 47], 0x40), 6)
            keyP["fresh_air_intake_fan_speed"] = bit.band(
                                                     messageBytes[cursor + 48],
                                                     0x7F)
            keyP["has_fresh_air_intake_fan_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 48], 0x80), 7)
            keyP["fresh_air_exhaust_fan_speed"] = bit.band(
                                                      messageBytes[cursor + 49],
                                                      0x7F)
            keyP["has_fresh_air_exhaust_fan_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 49], 0x80), 7)
            keyP["water_model_force_temp"] = messageBytes[cursor + 54]
            keyP["new_wind_model_intake_wind"] = bit.band(
                                                     messageBytes[cursor + 55],
                                                     0x7F)
            if (#binData > 55) then
                keyP["new_wind_model_exhaust_wind"] = bit.band(
                                                          messageBytes[cursor +
                                                              56], 0x7F)
            end
        end
        if (messageBytes[3] == 0x03) then
            ...
            keyP["has_new_wind_machine"] =
                bit.band(messageBytes[cursor + 3], 0x01)
            keyP["new_wind_machine"] = bit.band(messageBytes[cursor + 4], 0x01)
            keyP["has_fresh_air_remove_odor"] =
                bit.rshift(bit.band(messageBytes[cursor + 3], 0x02), 1)
            keyP["fresh_air_remove_odor"] =
                bit.rshift(bit.band(messageBytes[cursor + 4], 0x02), 1)
            keyP["has_new_wind_machine_intake_switch"] =
                bit.rshift(bit.band(messageBytes[cursor + 3], 0x04), 2)
            keyP["new_wind_machine_intake_switch"] =
            ...
            keyP["power_on_timer"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 4],
                                                    0x10), 4)
            keyP["power_off_timer"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 4],
            ...
            keyP["exhaust_strength"] = bit.rshift(bit.band(
                                                      ...
                                                      0x40), 6)
            keyP["has_fresh_air_mode"] =
                bit.rshift(bit.band(messageBytes[cursor + 5], 0x80), 7)
            keyP["fresh_air_intake_fan_speed"] = bit.band(
                                                     messageBytes[cursor + 6],
                                                     0x7F)
            keyP["has_fresh_air_intake_fan_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 6], 0x80), 7)
            keyP["fresh_air_exhaust_fan_speed"] = bit.band(
                                                      messageBytes[cursor + 7],
                                                      0x7F)
            keyP["has_fresh_air_exhaust_fan_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 7], 0x80), 7)
            keyP["power_on_time_value"] = bit.bor(
                                              bit.lshift(bit.band(
                                                             messageBytes[cursor +
                                                                 10], 0x0F), 8),
        ...
        if (messageBytes[2] == 0x06) then
            local cursor = 4
            keyP["indoor_machine_one_speed"] = bit.band(
                                                   messageBytes[cursor + 1],
                                                   0x01)
            keyP["indoor_machine_two_speed"] =
                bit.rshift(bit.band(messageBytes[cursor + 1], 0x02), 1)
            keyP["indoor_elec_expansion_valve_one"] =
                bit.rshift(bit.band(messageBytes[cursor + 1], 0x04), 2)
            keyP["indoor_elec_expansion_valve_two"] =
                bit.rshift(bit.band(messageBytes[cursor + 1], 0x08), 3)
            keyP["in_water_pump_force"] =
                bit.rshift(bit.band(messageBytes[cursor + 1], 0x10), 4)
            keyP["out_water_pump_force"] =
                bit.rshift(bit.band(messageBytes[cursor + 1], 0x20), 5)
            keyP["ptc_support_force"] = bit.rshift(bit.band(
                                                       messageBytes[cursor + 1],
                                                       0x40), 6)
            keyP["energy_need_c0"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 1],
                                                    0x80), 7)
            keyP["indoor_lr_wind_angle"] =
                bit.band(messageBytes[cursor + 2], 0x01)
            keyP["indoor_ud_wind_angle"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x02), 1)
            keyP["fengguan_one_speed_enable"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x04), 2)
            keyP["fengguan_two_speed_enable"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x08), 3)
            keyP["new_wind_two_speed_enable"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x10), 4)
            keyP["new_wind_one_speed_enable"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x20), 5)
            keyP["new_wind_model_gate"] =
                bit.rshift(bit.band(messageBytes[cursor + 2], 0x40), 6)
            keyP["remove_odor_c0"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 2],
                                                    0x80), 7)
            keyP["force_wind_machine_one_speed"] =
            ...
            keyP["force_expansion_valve_two_speed"] =
                messageBytes[cursor + 11] * 256 + messageBytes[cursor + 10]
            keyP["in_water_pump"] = bit.band(messageBytes[cursor + 12], 0x01)
            keyP["out_water_pump"] = bit.rshift(bit.band(
                                                    messageBytes[cursor + 12],
                                                    0x02), 1)
            keyP["ptc_support"] = bit.rshift(
                                      bit.band(messageBytes[cursor + 12], 0x04),
                                      2)
    ...
    if (dataType == 0x14) then
        ...
        keyP["fg_timer1_week1_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x02), 1)
        keyP["fg_timer1_week2_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x04), 2)
        keyP["fg_timer1_week3_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x08), 3)
        keyP["fg_timer1_week4_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x10), 4)
        keyP["fg_timer1_week5_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x20), 5)
        keyP["fg_timer1_week6_effect"] = bit.rshift(
                                             bit.band(messageBytes[1], 0x40), 6)
        keyP["fg_timer1_select"] =
            bit.rshift(bit.band(messageBytes[1], 0x80), 7)
        ...
        keyP["fg_timer2_week1_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x02), 1)
        keyP["fg_timer2_week2_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x04), 2)
        keyP["fg_timer2_week3_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x08), 3)
        keyP["fg_timer2_week4_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x10), 4)
        keyP["fg_timer2_week5_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x20), 5)
        keyP["fg_timer2_week6_effect"] = bit.rshift(
                                             bit.band(messageBytes[7], 0x40), 6)
        keyP["fg_timer2_select"] =
            bit.rshift(bit.band(messageBytes[7], 0x80), 7)
...
local function getAcMsg(bodyData, cType)
    ...
    if (cType == keyB["BYTE_QUERYL_REQUEST"] or cType ==
        ...
        keyB["BYTE_QUERY_WATER_RUN_REQUEST"] or cType == 0x13 or cType == 0x14) then
        msgBytes[9] = 0x03
    else
        msgBytes[9] = 0x02
    end
...
local function c003_jsonToData(control, status)
    ...
    if (control["fresh_air_mode_enable"] ~= nil and control["fresh_air_mode"] ~=
        nil) then
        bodyBytes[cursor + 4] = bit.bor(control["fresh_air_mode"],
                                        bodyBytes[cursor + 4])
        bodyBytes[cursor + 4] = bit.bor(bit.lshift(
                                            control["fresh_air_mode_enable"], 7),
                                        bodyBytes[cursor + 4])
    end
    if (control["wind_strength"] ~= nil) then
        bodyBytes[cursor + 4] = bit.bor(bit.lshift(control["wind_strength"], 5),
                                        bodyBytes[cursor + 4])
        bodyBytes[cursor + 4] = bit.bor(bit.lshift(
                                            control["wind_strength_enable"], 7),
                                        bodyBytes[cursor + 4])
    end
    if (control["exhaust_strength"] ~= nil) then
        bodyBytes[cursor + 4] = bit.bor(bit.lshift(control["exhaust_strength"],
                                                   6), bodyBytes[cursor + 4])
        bodyBytes[cursor + 4] = bit.bor(bit.lshift(
                                            control["exhaust_strength_enable"],
                                            7), bodyBytes[cursor + 4])
    end
    if (control["fresh_air_intake_fan_speed_enable"] ~= nil and
        control["fresh_air_intake_fan_speed"] ~= nil) then
        bodyBytes[cursor + 5] = bit.bor(control["fresh_air_intake_fan_speed"],
                                        bodyBytes[cursor + 5])
        bodyBytes[cursor + 5] = bit.bor(bit.lshift(
                                            control["fresh_air_intake_fan_speed_enable"],
                                            7), bodyBytes[cursor + 5])
    end
    if (control["fresh_air_exhaust_fan_speed_enable"] ~= nil and
        control["fresh_air_exhaust_fan_speed"] ~= nil) then
        bodyBytes[cursor + 6] = bit.bor(control["fresh_air_exhaust_fan_speed"],
                                        bodyBytes[cursor + 6])
        bodyBytes[cursor + 6] = bit.bor(bit.lshift(
                                            control["fresh_air_exhaust_fan_speed_enable"],
                                            7), bodyBytes[cursor + 6])
    end
    if (control["power_on_timer_enable"] ~= nil and
        control["power_on_time_value"] ~= nil) then
        bodyBytes[cursor + 7] = bit.band(control["power_on_time_value"], 0xFF)
        bodyBytes[cursor + 9] = bit.bor(bit.rshift(
                                            control["power_on_time_value"], 8),
                                        bodyBytes[cursor + 9])
    end
    if (control["power_off_timer_enable"] ~= nil and
        control["power_off_time_value"] ~= nil) then
        bodyBytes[cursor + 8] = bit.band(control["power_off_time_value"], 0xFF)
        bodyBytes[cursor + 9] = bit.bor(bit.lshift(bit.rshift(
                                                       control["power_off_time_value"],
                                                       8), 4), bodyBytes[9])
    end
    if (control["fresh_air_fan_speed_enable"] ~= nil and
        control["fresh_air_fan_speed"] ~= nil) then
        bodyBytes[cursor + 1] = bit.band(control["fresh_air_fan_speed"], 0x7F)
        bodyBytes[cursor + 1] = bit.bor(bit.lshift(
                                            control["fresh_air_fan_speed_enable"],
                                            7), bodyBytes[cursor + 1])
    end
...
local function c002_jsonToDataTemp(control, status)
    ...
    if (control["mode_enable"] ~= nil and control["mode"] ~= nil) then
        bodyBytes[cursor + 8] =
            bit.bor(keyP["modeValue"], bodyBytes[cursor + 8])
        bodyBytes[cursor + 8] = bit.bor(bit.lshift(control["mode_enable"], 7),
                                        bodyBytes[cursor + 8])
    end
    if (control["temperature_enable"] ~= nil and control["temperature"] ~= nil) then
        bodyBytes[cursor + 9] = (control["temperature"] +
                                    control["small_temperature"]) * 2 + 30
        bodyBytes[cursor + 9] = bit.bor(bit.lshift(
                                            control["temperature_enable"], 7),
                                        bodyBytes[cursor + 9])
    end
    if (control["wind_speed_enable"] ~= nil and control["wind_speed"] ~= nil) then
        bodyBytes[cursor + 10] = bit.bor(control["wind_speed"],
                                         bodyBytes[cursor + 10])
        bodyBytes[cursor + 10] = bit.bor(bit.lshift(
                                             control["wind_speed_enable"], 7),
                                         bodyBytes[cursor + 10])
    end
    if (control["dehumidity_enable"] ~= nil and control["dehumidity"] ~= nil) then
        bodyBytes[cursor + 11] = bit.bor(control["dehumidity"],
                                         bodyBytes[cursor + 11])
        bodyBytes[cursor + 11] = bit.bor(bit.lshift(
                                             control["dehumidity_enable"], 7),
                                         bodyBytes[cursor + 11])
    end
    if (control["humidity_enable"] ~= nil and control["humidity"] ~= nil) then
        bodyBytes[cursor + 12] = bit.bor(control["humidity"],
                                         bodyBytes[cursor + 12])
        bodyBytes[cursor + 12] = bit.bor(
                                     bit.lshift(control["humidity_enable"], 7),
                                     bodyBytes[cursor + 12])
    end
    if (control["power_on_timer_enable"] ~= nil and
        control["power_on_time_value"] ~= nil) then
        bodyBytes[cursor + 15] = bit.band(control["power_on_time_value"], 0xFF)
        bodyBytes[cursor + 17] = bit.bor(bit.rshift(
                                             control["power_on_time_value"], 8),
                                         bodyBytes[cursor + 17])
    end
    if (control["power_off_timer_enable"] ~= nil and
        ...
        bodyBytes[cursor + 17] = bit.bor(
                                     bit.lshift(bit.rshift(
                                                    control["power_off_time_value"],
                                                    8), 4),
                                     bodyBytes[cursor + 17])
    end
    if (control["new_wind_model_on_timer_enable"] ~= nil and
        control["new_wind_model_on_timer_value"] ~= nil) then
        bodyBytes[cursor + 18] = bit.band(
                                     control["new_wind_model_on_timer_value"],
                                     0xFF)
        bodyBytes[cursor + 20] = bit.bor(bit.rshift(
                                             control["new_wind_model_on_timer_value"],
                                             8), bodyBytes[cursor + 20])
    end
    if (control["new_wind_model_off_timer_enable"] ~= nil and
        control["new_wind_model_off_timer_value"] ~= nil) then
        bodyBytes[cursor + 19] = bit.band(
                                     control["new_wind_model_off_timer_value"],
                                     0xFF)
        bodyBytes[cursor + 20] = bit.bor(
                                     bit.lshift(bit.rshift(
                                                    control["new_wind_model_off_timer_value"],
                                                    8), 4),
                                     bodyBytes[cursor + 20])
    ...
    if (keyP["comfortableSleepValue"] == 0x30 and comfortByte ~= nil) then
        bodyBytes[cursor + 21] = bit.bor(
                                     ...
                                     bit.lshift(
                                         (checkBoundary(comfortByte[2], 16, 30) -
                                             16), 4))
        bodyBytes[cursor + 22] = bit.bor(
                                     checkBoundary(comfortByte[3], 16, 30) - 16,
                                     bit.lshift(
                                         (checkBoundary(comfortByte[4], 16, 30) -
                                             16), 4))
        bodyBytes[cursor + 23] = bit.bor(
                                     checkBoundary(comfortByte[5], 16, 30) - 16,
                                     bit.lshift((comfortByte[6] - 16), 4))
        bodyBytes[cursor + 24] = bit.bor(
                                     checkBoundary(comfortByte[7], 16, 30) - 16,
                                     bit.lshift((comfortByte[8] - 16), 4))
        bodyBytes[cursor + 25] = bit.bor(
                                     checkBoundary(comfortByte[9], 16, 30) - 16,
                                     bit.lshift((comfortByte[10] - 16), 4))
    end
    if (control["water_model_temperature_set_enable"] ~= nil and
        control["water_model_temperature_set"] ~= nil) then
        bodyBytes[cursor + 30] = bit.bor(
                                     (control["water_model_temperature_set"] * 2 +
                                         50), bodyBytes[cursor + 30])
        bodyBytes[cursor + 29] = bit.bor(bit.lshift(
                                             control["water_model_temperature_set_enable"],
                                             6), bodyBytes[cursor + 29])
    end
    if (control["water_mode_enable"] ~= nil and control["water_mode"] ~= nil) then
        bodyBytes[cursor + 29] = bit.bor(control["water_mode"],
                                         bodyBytes[cursor + 29])
        bodyBytes[cursor + 29] = bit.bor(bit.lshift(
                                             control["water_mode_enable"], 7),
                                         bodyBytes[cursor + 29])
    end
    if (control["left_right_wind_direction_enable"] ~= nil and
        control["left_right_wind_direction"] ~= nil) then
        bodyBytes[cursor + 31] = bit.bor(bit.lshift(
                                             control["left_right_wind_direction"],
                                             4), bodyBytes[cursor + 31])
        bodyBytes[cursor + 27] = bit.bor(bit.lshift(
                                             control["left_right_wind_direction_enable"],
                                             7), bodyBytes[cursor + 27])
    end
    if (control["up_down_wind_direction_enable"] ~= nil and
        control["up_down_wind_direction"] ~= nil) then
        bodyBytes[cursor + 31] = bit.bor(control["up_down_wind_direction"],
                                         bodyBytes[cursor + 31])
        bodyBytes[cursor + 27] = bit.bor(bit.lshift(
                                             control["up_down_wind_direction_enable"],
                                             6), bodyBytes[cursor + 27])
    end
    if (control["fresh_air_mode_enable"] ~= nil and control["fresh_air_mode"] ~=
        nil) then
        bodyBytes[cursor + 32] = bit.bor(control["fresh_air_mode"],
                                         bodyBytes[cursor + 32])
        bodyBytes[cursor + 32] = bit.bor(bit.lshift(
                                             control["fresh_air_mode_enable"], 7),
                                         bodyBytes[cursor + 32])
        bodyBytes[cursor + 32] = bit.bor(
                                     bit.lshift(keyP["exhaust_strength"], 6),
                                     bodyBytes[cursor + 32])
        bodyBytes[cursor + 32] = bit.bor(bit.lshift(keyP["wind_strength"], 5),
                                         bodyBytes[cursor + 32])
    end
    if (control["wind_strength"] ~= nil) then
        bodyBytes[cursor + 32] = bit.bor(
                                     bit.lshift(control["wind_strength"], 5),
                                     bodyBytes[cursor + 32])
        bodyBytes[cursor + 32] = bit.bor(bit.lshift(
                                             control["wind_strength_enable"], 7),
                                         bodyBytes[cursor + 32])
    ...
    if (control["exhaust_strength"] ~= nil) then
        bodyBytes[cursor + 32] = bit.bor(
                                     bit.lshift(control["exhaust_strength"], 6),
                                     bodyBytes[cursor + 32])
        bodyBytes[cursor + 32] = bit.bor(bit.lshift(
                                             control["exhaust_strength_enable"],
                                             7), bodyBytes[cursor + 32])
    end
    if (control["fresh_air_fan_speed_enable"] ~= nil and
        control["fresh_air_fan_speed"] ~= nil) then
        bodyBytes[cursor + 33] = bit.bor(control["fresh_air_fan_speed"],
                                         bodyBytes[cursor + 33])
        bodyBytes[cursor + 33] = bit.bor(bit.lshift(
                                             control["fresh_air_fan_speed_enable"],
                                             7), bodyBytes[cursor + 33])
    end
    if (control["quick_prepare_food_angle_enable"] ~= nil and
        control["quick_prepare_food_angle"] ~= nil) then
        bodyBytes[cursor + 34] = bit.bor(control["quick_prepare_food_angle"],
                                         bodyBytes[cursor + 34])
        bodyBytes[cursor + 34] = bit.bor(bit.lshift(
                                             control["quick_prepare_food_angle_enable"],
                                             7), bodyBytes[cursor + 34])
    end
    ...
    if (control["new_home_remove_arofene"] ~= nil and
        control["new_home_remove_arofene_enable"] ~= nil) then
        bodyBytes[cursor + 38] = bit.bor(control["new_home_remove_arofene"],
                                         bodyBytes[cursor + 38])
        bodyBytes[cursor + 38] = bit.bor(bit.lshift(
                                             control["new_home_remove_arofene_enable"],
                                             3), bodyBytes[cursor + 38])
    end
    if (control["shortest_remove_odor_time_enable"] ~= nil and
        control["shortest_remove_odor_time"] ~= nil) then
        bodyBytes[cursor + 39] = bit.band(control["shortest_remove_odor_time"],
                                          0xFF)
        bodyBytes[cursor + 40] = bit.band(bit.rshift(
                                              control["shortest_remove_odor_time"],
                                              8), 0xFF)
        bodyBytes[cursor + 45] = bit.bor(bit.lshift(
                                             control["shortest_remove_odor_time_enable"],
                                             2), bodyBytes[cursor + 45])
    end
    if (control["longest_remove_odor_time_enable"] ~= nil and
        control["longest_remove_odor_time"] ~= nil) then
        bodyBytes[cursor + 41] = bit.band(control["longest_remove_odor_time"],
                                          0xFF)
        bodyBytes[cursor + 42] = bit.band(bit.rshift(
                                              control["longest_remove_odor_time"],
                                              8), 0xFF)
        bodyBytes[cursor + 45] = bit.bor(bit.lshift(
                                             control["longest_remove_odor_time_enable"],
                                             3), bodyBytes[cursor + 45])
    end
    if (control["fresh_air_intake_wind_enable"] ~= nil and
        control["fresh_air_intake_wind"] ~= nil) then
        bodyBytes[cursor + 43] = bit.bor(control["fresh_air_intake_wind"],
                                         bodyBytes[cursor + 43])
        bodyBytes[cursor + 45] = bit.bor(bit.lshift(
                                             control["fresh_air_intake_wind_enable"],
                                             4), bodyBytes[cursor + 45])
    end
    if (control["fresh_air_exhaust_wind_enable"] ~= nil and
        control["fresh_air_exhaust_wind"] ~= nil) then
        bodyBytes[cursor + 44] = bit.bor(control["fresh_air_exhaust_wind"],
                                         bodyBytes[cursor + 44])
        bodyBytes[cursor + 45] = bit.bor(bit.lshift(
                                             control["fresh_air_exhaust_wind_enable"],
                                             5), bodyBytes[cursor + 45])
    end
    if (control["fresh_air_intake_fan_speed_enable"] ~= nil and
        control["fresh_air_intake_fan_speed"] ~= nil) then
        bodyBytes[cursor + 47] = bit.bor(control["fresh_air_intake_fan_speed"],
                                         bodyBytes[cursor + 47])
        bodyBytes[cursor + 47] = bit.bor(bit.lshift(
                                             control["fresh_air_intake_fan_speed_enable"],
                                             7), bodyBytes[cursor + 47])
    end
    if (control["fresh_air_exhaust_fan_speed_enable"] ~= nil and
        control["fresh_air_exhaust_fan_speed"] ~= nil) then
        bodyBytes[cursor + 48] = bit.bor(control["fresh_air_exhaust_fan_speed"],
                                         bodyBytes[cursor + 48])
        bodyBytes[cursor + 48] = bit.bor(bit.lshift(
                                             control["fresh_air_exhaust_fan_speed_enable"],
                                             7), bodyBytes[cursor + 48])
    end
    if (control["water_model_force_temp_enable"] ~= nil and
        control["water_model_force_temp"] ~= nil) then
        bodyBytes[cursor + 54] = bit.bor(control["water_model_force_temp"],
                                         bodyBytes[cursor + 54])
        bodyBytes[cursor + 49] = bit.bor(bit.lshift(
                                             control["water_model_force_temp_enable"],
                                             6), bodyBytes[cursor + 49])
    end
    if (control["new_wind_model_intake_wind_enable"] ~= nil and
        control["new_wind_model_intake_wind"] ~= nil) then
        bodyBytes[cursor + 55] = bit.bor(control["new_wind_model_intake_wind"],
                                         bodyBytes[cursor + 55])
        bodyBytes[cursor + 55] = bit.bor(bit.lshift(
                                             control["new_wind_model_intake_wind_enable"],
                                             7), bodyBytes[cursor + 55])
    end
    if (control["new_wind_model_exhaust_wind_enable"] ~= nil and
        control["new_wind_model_exhaust_wind"] ~= nil) then
        bodyBytes[cursor + 56] = bit.bor(control["new_wind_model_exhaust_wind"],
                                         bodyBytes[cursor + 56])
        bodyBytes[cursor + 56] = bit.bor(bit.lshift(
                                             control["new_wind_model_exhaust_wind_enable"],
                                             7), bodyBytes[cursor + 56])
    end
    if (control["inner_purifier_on_pm_enable"] ~= nil and
        control["inner_purifier_on_pm"] ~= nil) then
        bodyBytes[cursor + 57] = bit.bor(math.floor(
                                             control["inner_purifier_on_pm"] /
                                                 256), bodyBytes[cursor + 57])
        bodyBytes[cursor + 58] = bit.bor(math.floor(
                                             control["inner_purifier_on_pm"] %
                                                 256), bodyBytes[cursor + 58])
        bodyBytes[cursor + 57] = bit.bor(bit.lshift(
                                             control["inner_purifier_on_pm_enable"],
                                             7), bodyBytes[cursor + 57])
    end
    if (control["fresh_air_on_co2_enable"] ~= nil and
        control["fresh_air_on_co2"] ~= nil) then
        bodyBytes[cursor + 59] = bit.bor(math.floor(
                                             control["fresh_air_on_co2"] / 256),
                                         bodyBytes[cursor + 59])
        bodyBytes[cursor + 60] = bit.bor(math.floor(
                                             control["fresh_air_on_co2"] % 256),
                                         bodyBytes[cursor + 60])
        bodyBytes[cursor + 59] = bit.bor(bit.lshift(
                                             control["fresh_air_on_co2_enable"],
                                             7), bodyBytes[cursor + 59])
    end
...
local gC002OnOffConfig = {
    ...
    ["cool_hot_sense_enable"] = {
        pbyIndex = 0,
        pbitIndex = 5,
        fbyIndex = 1,
        fbitIndex = 5
    },
    ["wind_swing_ud_enable"] = {
        pbyIndex = 0,
        pbitIndex = 6,
        fbyIndex = 1,
        fbitIndex = 6
    },
    ["wind_swing_lr_enable"] = {
        pbyIndex = 0,
        pbitIndex = 7,
        fbyIndex = 1,
        fbitIndex = 7
    ...
    ["nobody_energy_save_enable"] = {
        pbyIndex = 2,
        pbitIndex = 7,
        fbyIndex = 3,
        fbitIndex = 7
    },
    ["inner_purifier_enable"] = {
        pbyIndex = 4,
        pbitIndex = 0,
        fbyIndex = 5,
        fbitIndex = 0
    },
    ["manu_inner_purifier_enable"] = {
        pbyIndex = 4,
        pbitIndex = 1,
        fbyIndex = 5,
        fbitIndex = 1
    ...
    ["new_wind_machine_enable"] = {
        pbyIndex = 4,
        pbitIndex = 4,
        fbyIndex = 5,
        fbitIndex = 4
    },
    ["new_wind_machine_link_enable"] = {
        pbyIndex = 4,
        pbitIndex = 5,
        fbyIndex = 5,
        fbitIndex = 5
    },
    ["fresh_air_remove_odor_enable"] = {
        pbyIndex = 4,
        pbitIndex = 6,
        fbyIndex = 5,
        fbitIndex = 6
    ...
    ["prevent_super_cool_enable"] = {
        pbyIndex = 6,
        pbitIndex = 1,
        fbyIndex = 7,
        fbitIndex = 1
    },
    ["new_wind_model_intake_switch_enable"] = {
        pbyIndex = 6,
        pbitIndex = 2,
        fbyIndex = 7,
        fbitIndex = 2
    },
    ["new_wind_model_exhaust_switch_enable"] = {
        pbyIndex = 6,
        pbitIndex = 3,
        fbyIndex = 7,
        fbitIndex = 3
    },
    ["new_wind_model_mute_enable"] = {
        pbyIndex = 6,
        pbitIndex = 4,
        fbyIndex = 7,
        fbitIndex = 4
    ...
    ["power_off_timer_enable"] = {
        pbyIndex = 13,
        pbitIndex = 1,
        fbyIndex = 14,
        fbitIndex = 1
    },
    ["new_wind_model_on_timer_enable"] = {
        pbyIndex = 13,
        pbitIndex = 2,
        fbyIndex = 14,
        fbitIndex = 2
    },
    ["new_wind_model_off_timer_enable"] = {
        pbyIndex = 13,
        pbitIndex = 3,
        fbyIndex = 14,
        fbitIndex = 3
    },
    ["comfort_sleep_enable"] = {
        pbyIndex = 13,
        pbitIndex = 4,
        fbyIndex = 14,
        fbitIndex = 4
    ...
    ["water_model_power_enable"] = {
        pbyIndex = 27,
        pbitIndex = 0,
        fbyIndex = 28,
        fbitIndex = 0
    },
    ["water_model_power_save_enable"] = {
        pbyIndex = 27,
        pbitIndex = 1,
        fbyIndex = 28,
        fbitIndex = 1
    },
    ["water_model_clean_enable"] = {
        pbyIndex = 27,
        pbitIndex = 2,
        fbyIndex = 28,
        fbitIndex = 2
    },
    ["water_model_temperature_auto_enable"] = {
        pbyIndex = 27,
        pbitIndex = 3,
        fbyIndex = 28,
        fbitIndex = 3
    },
    ["water_model_ptc_enable"] = {
        pbyIndex = 27,
        pbitIndex = 4,
        fbyIndex = 28,
        fbitIndex = 4
    },
    ["water_model_go_out_enable"] = {
        pbyIndex = 27,
        pbitIndex = 5,
        fbyIndex = 28,
        fbitIndex = 5
...
local function c002_jsonToData(control, status)
    ...
    if (control["water_model_temperature_set_enable"] ~= nil and
        control["water_model_temperature_set"] ~= nil) then
        bodyBytes[cursor + 30] = bit.bor(control["dehumidity"],
                                         bodyBytes[cursor + 30])
        bodyBytes[cursor + 29] = bit.bor(bit.lshift(
                                             control["water_model_temperature_set_enable"],
                                             7), bodyBytes[cursor + 29])
    end
    if (control["water_mode_enable"] ~= nil and control["water_mode"] ~= nil) then
        bodyBytes[cursor + 29] = bit.bor(control["water_mode"],
                                         bodyBytes[cursor + 29])
        bodyBytes[cursor + 29] = bit.bor(bit.lshift(
                                             control["water_mode_enable"], 6),
                                         bodyBytes[cursor + 29])
    end
    if (control["left_right_wind_direction_enable"] ~= nil and
        control["left_right_wind_direction"] ~= nil) then
...
function set_bit(bodyBytes, byteIndex, bitIndex, keyValue)
    bodyBytes[byteIndex] = bit.bor(bit.lshift(keyValue, bitIndex),
                                   bodyBytes[byteIndex])
end
function jsonToData(jsonCmd)
    ...
    if (query) then
        ...
        elseif (queryType == "c002_query" or queryType == "c006_query" or
            queryType == "c003_query") then
            local bodyBytes = {}
    ...
    elseif (control) then
        ...
        elseif (keyP["control_c006"] == 1) then
            local bodyBytes = {}
            for i = 0, 33 do bodyBytes[i] = 0 end
            bodyBytes[0] = keyP["pack_id"]
            bodyBytes[1] = 1
            bodyBytes[2] = 1
            bodyBytes[3] = 0
            ...
            bodyBytes[14] = 0x16
            bodyBytes[15] = 1
            bodyBytes[16] = bit.bor(keyP["indoor_machine_one_speed"],
                                    bodyBytes[5])
            bodyBytes[16] = bit.bor(
                                bit.lshift(keyP["indoor_machine_two_speed"], 1),
                                bodyBytes[5])
            bodyBytes[16] = bit.bor(bit.lshift(
                                        keyP["indoor_elec_expansion_valve_one"],
                                        2), bodyBytes[16])
            bodyBytes[16] = bit.bor(bit.lshift(
                                        keyP["indoor_elec_expansion_valve_two"],
                                        3), bodyBytes[16])
            bodyBytes[16] = bit.bor(bit.lshift(keyP["in_water_pump_force"], 4),
                                    bodyBytes[16])
            bodyBytes[16] = bit.bor(bit.lshift(keyP["out_water_pump_force"], 5),
                                    bodyBytes[16])
            bodyBytes[16] = bit.bor(bit.lshift(keyP["ptc_support_force"], 6),
                                    bodyBytes[16])
            bodyBytes[16] = bit.bor(bit.lshift(keyP["energy_need_c0"], 7),
                                    bodyBytes[16])
            bodyBytes[17] = bit.bor(keyP["indoor_lr_wind_angle"], bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(keyP["indoor_ud_wind_angle"], 1),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(
                                        keyP["fengguan_one_speed_enable"], 2),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(
                                        keyP["fengguan_two_speed_enable"], 3),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(
                                        keyP["new_wind_two_speed_enable"], 4),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(
                                        keyP["new_wind_one_speed_enable"], 5),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(keyP["new_wind_model_gate"], 6),
                                    bodyBytes[17])
            bodyBytes[17] = bit.bor(bit.lshift(keyP["remove_odor_c0"], 7),
                                    bodyBytes[17])
            bodyBytes[19] = bit.band(keyP["force_wind_machine_one_speed"], 0xFF)
            bodyBytes[20] = bit.band(bit.rshift(
                                         keyP["force_wind_machine_one_speed"], 8),
                                     0xFF)
            bodyBytes[21] = bit.band(keyP["force_wind_machine_two_speed"], 0xFF)
            bodyBytes[22] = bit.band(bit.rshift(
                                         keyP["force_wind_machine_two_speed"], 8),
                                     0xFF)
            bodyBytes[23] = bit.band(keyP["force_expansion_valve_one_speed"],
                                     0xFF)
            bodyBytes[24] = bit.band(bit.rshift(
                                         keyP["force_expansion_valve_one_speed"],
                                         8), 0xFF)
            bodyBytes[25] = bit.band(keyP["force_expansion_valve_two_speed"],
                                     0xFF)
            bodyBytes[26] = bit.band(bit.rshift(
                                         keyP["force_expansion_valve_two_speed"],
                                         8), 0xFF)
            bodyBytes[27] = bit.bor(keyP["in_water_pump"], bodyBytes[27])
            bodyBytes[27] = bit.bor(bit.lshift(keyP["out_water_pump"], 1),
                                    bodyBytes[27])
            bodyBytes[27] = bit.bor(bit.lshift(keyP["ptc_support"], 2),
                                    bodyBytes[27])
        ...
        elseif (keyP["fg_timer_number"] ~= 0) then
            ...
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week1_effect"], 1),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week2_effect"], 2),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week3_effect"], 3),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week4_effect"], 4),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week5_effect"], 5),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(
                               bit.lshift(keyP["fg_timer1_week6_effect"], 6),
                               bodyBytes[1])
            bodyBytes[1] = bit.bor(bit.lshift(keyP["fg_timer1_select"], 7),
                                   bodyBytes[1])
            bodyBytes[2] = bit.bor(keyP["fg_timer1_wind_speed"], bodyBytes[2])
            bodyBytes[2] = bit.bor(bit.lshift(keyP["fg_timer1_power"], 7),
                                   bodyBytes[2])
            bodyBytes[3] = bit.bor(keyP["fg_timer1_mode"], bodyBytes[3])
            bodyBytes[4] = bit.bor((keyP["fg_timer1_temperature"] * 2),
                                   bodyBytes[4])
            ...
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week1_effect"], 1),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week2_effect"], 2),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week3_effect"], 3),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week4_effect"], 4),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week5_effect"], 5),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(
                               bit.lshift(keyP["fg_timer2_week6_effect"], 6),
                               bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["fg_timer2_select"], 7),
                                   bodyBytes[7])
            bodyBytes[8] = bit.bor(keyP["fg_timer2_wind_speed"], bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["fg_timer2_power"], 7),
                                   bodyBytes[8])
            bodyBytes[9] = bit.bor(keyP["fg_timer2_mode"], bodyBytes[9])
            bodyBytes[10] = bit.bor((keyP["fg_timer2_temperature"] * 2),
                                    bodyBytes[10])
        ...
        elseif (keyP["new_wind_single_control"] == 1) then
            ...
            bodyBytes[45] = bit.bor(bit.lshift(
                                    ...
                                    bodyBytes[45])
            bodyBytes[45] = bit.bor(bit.lshift(keyP["female_care"], 7),
                                    bodyBytes[45])
            ...
            bodyBytes[52] = bit.bor(bit.lshift(keyP["has_shunt_valve"], 1),
                                    bodyBytes[52])
            bodyBytes[52] = bit.bor(
                                bit.lshift(keyP["heater_heating_switch"], 2),
                                bodyBytes[52])
            bodyBytes[52] = bit.bor(bit.lshift(keyP["stop_warm_support"], 3),
                                    bodyBytes[52])
            bodyBytes[52] = bit.bor(bit.lshift(
                                        keyP["water_temp_linkage_support"], 4),
                                    bodyBytes[52])
            bodyBytes[52] = bit.bor(bit.lshift(
                                        keyP["water_temp_linkage_switch"], 5),
                                    bodyBytes[52])
            bodyBytes[52] = bit.bor(bit.lshift(keyP["water_model_dry"], 6),
                                    bodyBytes[52])
            bodyBytes[53] = bit.bor(keyP["water_model_force_temp"],
                                    bodyBytes[53])
            bodyBytes[53] = bit.bor(bit.lshift(keyP["smart_water_model"], 7),
                                    bodyBytes[53])
            bodyBytes[55] = bit.band(
                                keyP["save_energy_mode_effective_countdown"],
                                0x0F)
            bodyBytes[55] = bit.bor(bit.lshift(keyP["nobody_off_switch"], 4),
                                    bodyBytes[55])
            bodyBytes[56] = bit.band(bit.rshift(
                                         keyP["save_energy_mode_effective_countdown"],
                                         4), 0x7F)
            bodyBytes[56] = bit.bor(bit.lshift(
                                        keyP["save_energy_mode_effective_countdown_setting_enable"],
                                        7), bodyBytes[56])
            bodyBytes[57] = bit.bor(keyP["nobody_off_switch_setting_enable"],
                                    bodyBytes[57])
            bodyBytes[57] = bit.bor(bit.lshift(
                                        keyP["nobody_off_time_setting_enable"],
                                        1), bodyBytes[57])
            bodyBytes[57] = bit.bor(bit.lshift(keyP["nobody_off_time"], 2),
                                    bodyBytes[57])
            bodyBytes[55] = bit.bor(
                                bit.lshift(keyP["follow_body_sense_enable"], 7),
                                bodyBytes[55])
            bodyBytes[62] = bit.bor(keyP["fresh_air_intake_fan_speed"],
                                    bodyBytes[62])
            bodyBytes[63] = bit.bor(keyP["fresh_air_exhaust_fan_speed"],
                                    bodyBytes[63])
            bodyBytes[65] = bit.bor(keyP["double_engine_heat"], bodyBytes[65])
            bodyBytes[65] = bit.bor(bit.lshift(
                                        keyP["double_engine_heat_enable"], 1),
                                    bodyBytes[65])
            bodyBytes[65] = bit.bor(bit.lshift(keyP["smart_hybird"], 2),
                                    bodyBytes[65])
            bodyBytes[65] = bit.bor(bit.lshift(keyP["smart_hybird_enable"], 3),
                                    bodyBytes[65])
            bodyBytes[65] = bit.bor(bit.lshift(keyP["smart_hybird_pause"], 4),
                                    bodyBytes[65])
...
function dataToJson(jsonCmd)
    ...
    if (dataType == 0x20 or dataType == 0x11 or dataType == 0xF1 or
        (dataType == 0xC0 and (bodyBytes[3] == 0x02 or bodyBytes[3] == 0x03))) then
        ...
        streams["has_quick_prepare_food_angle"] =
            keyP["has_quick_prepare_food_angle"]
        ...
        streams["new_wind_machine_exhaust_switch"] =
            keyP["new_wind_machine_exhaust_switch"]
        streams["new_wind_machine_intake_switch"] =
            keyP["new_wind_machine_intake_switch"]
        streams["fresh_air_exhaust_fan_speed"] =
            keyP["fresh_air_exhaust_fan_speed"]
        streams["fresh_air_intake_fan_speed"] =
            keyP["fresh_air_intake_fan_speed"]
        streams["has_new_wind_machine_exhaust_switch"] =
            keyP["has_new_wind_machine_exhaust_switch"]
        streams["has_new_wind_machine_intake_switch"] =
            keyP["has_new_wind_machine_intake_switch"]
        ...
        streams["c003_protocol_inner_support"] =
            keyP["c003_protocol_inner_support"]
        streams["no_wind_swing_ud_support"] = keyP["no_wind_swing_ud_support"]
        streams["independent_strong_support"] =
            keyP["independent_strong_support"]
        streams["mute"] = keyP["mute"]
        if (dataType == 0xC0) then
            streams["prepare_food"] = keyP["prepare_food"]
            streams["quick_fry"] = keyP["quick_fry"]
            streams["quick_prepare_food_angle"] =
                keyP["quick_prepare_food_angle"]
            streams["new_wind_machine_exhaust_switch"] =
                keyP["new_wind_machine_exhaust_switch"]
            streams["new_wind_machine_intake_switch"] =
                keyP["new_wind_machine_intake_switch"]
            ...
            streams["water_model_force_temp"] = keyP["water_model_force_temp"]
            streams["has_water_model_temperature_set"] =
                keyP["has_water_model_temperature_set"]
    ...
    if (dataType == 0x10) then
        ...
        streams["radar_monitor_people_status_report"] =
            keyP["radar_monitor_people_status_report"]
        ...
        if (keyP["target_machine_info"] ~= nil) then
            if (keyP["target_machine_info"] == 0x01) then
                ...
                streams["has_fengguan_mini_new_wind"] =
                    keyP["has_fengguan_mini_new_wind"]
            end
            if (keyP["target_machine_info"] == 0x02) then
        ...
        streams["new_wind_machine_operating_mode"] =
            keyP["new_wind_machine_operating_mode"]
    ...
    if (dataType == 0x12) then
        ...
        streams["water_model_temperature_set_12"] =
            keyP["water_model_temperature_set_12"]
    end
    if (dataType == 0x30) then
