...
local function binToModel(binData, deviceSN8)
    ...
    then
        ...
        if dataType == 0x05 then
            keyP["analysis_value"] = keyP["analysis_value"]
                .. ",wind_swing_lr_angle,wind_swing_ud_angle,gentle_wind_sense,prevent_straight_wind,no_wind_sense,fresh_air,linkage_sync,linkage,moisturizing,no_wind_sense_right,no_wind_sense_left,ptc_default_rule,light_sensitive,self_clean"
            ...
            if #binData >= 33 then
                ...
                keyP["analysis_value"] = keyP["analysis_value"]
                    .. ",fresh_air_mode,fresh_air_mode_two,inner_purifier_mode,moisturizing_fan_speed,fresh_air_fan_speed,inner_purifier_fan_speed,indoor_humidity,five_dimension_mode,total_status_switch,wind_no_linkage"
            end
            if #binData >= 38 then
                ...
                keyP["analysis_value"] = keyP["analysis_value"]
                    .. ",screen_display_time,linkage_fan_speed,indoor_temperature"
            end
        end
    end
    ...
    if (dataType == 0x02 and messageBytes[0] == 0xB0) or (dataType == 0x03 and messageBytes[0] == 0xB1) then
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if messageBytes[cursor + 0] == 0x7E and messageBytes[cursor + 1] == 0x00 then
                if #binData < 21 then return nil end
                keyP["analysis_value"] =
                    "power,mode,temperature,wind_speed,power_on_timer,smart_dry_value,power_off_timer,power_off_time_value,power_on_time_value,ptc,eco,dry,wind_swing_lr,wind_swing_lr_under,wind_swing_ud,kick_quilt,prevent_cold,small_temperature,purifier,dust_full_time,power_saving,fault_tag,wind_swing_ud_left,wind_swing_ud_right,wind_swing_lr_right,wind_swing_lr_left"
    ...
    if query then
        ...
        if queryType == "group_data_zero" or queryType == "group_data_four" or queryType == "group_data_one" then
            ...
            for v in values(queryList) do
                ...
                if queryType == "total_status_switch" then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x88
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
            end
