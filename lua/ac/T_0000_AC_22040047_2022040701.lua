...
local function binToModel(binData, deviceSN8)
    ...
    if messageBytes[0] == 0xC1 then
        ...
        if messageBytes[3] == 0x44 then
            ...
            keyP["total_power_consumption"] = bit.bor(
                bit.bor(
                    bit.bor(bit.lshift(messageBytes[4], 32), bit.lshift(messageBytes[5], 16)),
                    bit.lshift(messageBytes[6], 8)
                ),
                messageBytes[7]
            )
            keyP["total_operating_consumption"] = bit.bor(
                bit.bor(
                    bit.bor(bit.lshift(messageBytes[8], 32), bit.lshift(messageBytes[9], 16)),
                    bit.lshift(messageBytes[10], 8)
                ),
                messageBytes[11]
            )
            keyP["current_operating_consumption"] = bit.bor(
                bit.bor(
                    bit.bor(bit.lshift(messageBytes[12], 32), bit.lshift(messageBytes[13], 16)),
