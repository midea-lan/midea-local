...
local function JsonToModel(jsonCmd, jsonType)
    ...
    if streams["fresh_air_exhaust_wind"] ~= nil and jsonType == "control" then
        keyP["fresh_air_exhaust_wind"] = streams["fresh_air_exhaust_wind"]
    end
    if streams["fresh_air_intake_wind"] ~= nil and jsonType == "control" then
...
local function binToModel(binData)
    ...
    if dataType == 0x20 or dataType == 0x11 or dataType == 0xF1 then
        ...
        if #binData > 43 then keyP["fresh_air_intake_wind"] = messageBytes[43] end
        if #binData > 44 then keyP["fresh_air_exhaust_wind"] = messageBytes[44] end
    end
