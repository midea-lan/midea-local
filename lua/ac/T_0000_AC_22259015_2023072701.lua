...
local function JsonToModel(jsonCmd, jsonType)
    ...
    if streams[keyT["KEY_STANDBY_CLEAN"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        keyP["standby_clean"] = keyB["BYTE_COMMON_ON"]
        keyP["power"] = 0
        keyP["strongWindValue"] = 0
        keyP["vertical_swing_left"] = 0
        ...
        keyP["water_washing"] = 0
        keyP["scene_id"] = 0
    elseif streams[keyT["KEY_STANDBY_CLEAN"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
        keyP["standby_clean"] = keyB["BYTE_COMMON_OFF"]
    end
    ...
    if streams[keyT["KEY_DISINFECT"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        ...
        keyP["remove_peculiar_smell"] = 1
        keyP["standby_clean"] = 0
    elseif streams[keyT["KEY_DISINFECT"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
    ...
    if streams[keyT["KEY_AUTO_PURIFY"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        ...
        keyP["remove_peculiar_smell"] = 1
        keyP["standby_clean"] = 0
    elseif streams[keyT["KEY_AUTO_PURIFY"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
    ...
    if streams[keyT["KEY_MANUL_PURIFY"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        ...
        keyP["remove_peculiar_smell"] = 1
        keyP["standby_clean"] = 0
    elseif streams[keyT["KEY_MANUL_PURIFY"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
    ...
    if streams[keyT["KEY_AUTO_HUMI"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        ...
        keyP["remove_peculiar_smell"] = 1
        keyP["standby_clean"] = 0
    elseif streams[keyT["KEY_AUTO_HUMI"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
    ...
    if streams[keyT["KEY_MANUL_HUMI"]] == keyV["VALUE_FUNCTION_ON"] and jsonType == "control" then
        ...
        keyP["remove_peculiar_smell"] = 1
        keyP["standby_clean"] = 0
    elseif streams[keyT["KEY_MANUL_HUMI"]] == keyV["VALUE_FUNCTION_OFF"] and jsonType == "control" then
    ...
    if streams[keyT["KEY_MODE"]] == keyV["VALUE_MODE_HEAT"] and jsonType == "control" then
        keyP["modeValue"] = keyB["BYTE_MODE_HEAT"]
        keyP["PTCValue"] = keyB["BYTE_COMMON_ON"]
    elseif streams[keyT["KEY_MODE"]] == keyV["VALUE_MODE_COOL"] and jsonType == "control" then
        keyP["modeValue"] = keyB["BYTE_MODE_COOL"]
    elseif streams[keyT["KEY_MODE"]] == keyV["VALUE_MODE_AUTO"] and jsonType == "control" then
        keyP["modeValue"] = keyB["BYTE_MODE_AUTO"]
        keyP["PTCValue"] = keyB["BYTE_COMMON_ON"]
    elseif streams[keyT["KEY_MODE"]] == keyV["VALUE_MODE_DRY"] and jsonType == "control" then
        keyP["modeValue"] = keyB["BYTE_MODE_DRY"]
        keyP["humi_flag"] = keyP["humi_flag"] + 3
    elseif streams[keyT["KEY_MODE"]] == keyV["VALUE_MODE_FAN"] and jsonType == "control" then
        keyP["modeValue"] = keyB["BYTE_MODE_FAN"]
    end
    ...
    if streams[keyT["KEY_DEHUMIDITY"]] ~= nil then
        keyP["dehumidityValue"] = checkBoundary(streams[keyT["KEY_DEHUMIDITY"]], 1, 101)
    end
    if streams[keyT["KEY_HUMIDITY"]] ~= nil then keyP["humidityValue"] = streams[keyT["KEY_HUMIDITY"]] end
    ...
    if streams[keyT["KEY_NEWWIND_MODE"]] ~= nil then
        keyP["newWindModeValue"] = checkBoundary(streams[keyT["KEY_NEWWIND_MODE"]], 0, 4)
    end
    ...
    if streams["remove_dust_full"] ~= nil then keyP["remove_dust_full"] = streams["remove_dust_full"] end
    ...
    if streams["remove_peculiar_smell"] ~= nil then keyP["remove_peculiar_smell"] = streams["remove_peculiar_smell"] end
    ...
    if streams["wind_swing_right"] ~= nil then keyP["wind_swing_right"] = streams["wind_swing_right"] end
    if streams["wind_swing_left"] ~= nil then keyP["wind_swing_left"] = streams["wind_swing_left"] end
    ...
    if streams["air_exhaust"] ~= nil then keyP["air_exhaust"] = streams["air_exhaust"] end
    ...
    if streams["comfort_fresh_air"] ~= nil then keyP["comfort_fresh_air"] = streams["comfort_fresh_air"] end
    ...
    if streams["huanqi_function"] ~= nil then keyP["huanqi_function"] = streams["huanqi_function"] end
    ...
    if streams["horizontal_swing_left"] ~= nil then
        ...
        if streams["horizontal_swing_left"] == 1 then keyP["up_down_wind_direction"] = 0 end
    end
    if streams["horizontal_swing_right"] ~= nil then
        keyP["horizontal_swing_right"] = streams["horizontal_swing_right"]
        if streams["horizontal_swing_right"] == 1 then keyP["up_down_wind_direction"] = 0 end
    end
    if streams["down_wind_speed_level"] ~= nil then keyP["down_wind_speed_level"] = streams["down_wind_speed_level"] end
    ...
    if streams["dry_clean"] ~= nil then keyP["dry_clean"] = streams["dry_clean"] end
    if streams["disinfect_setting_time"] ~= nil then
        keyP["disinfect_setting_time"] = streams["disinfect_setting_time"]
    end
    if streams["auto_purifier_on_pm"] ~= nil then keyP["auto_purifier_on_pm"] = streams["auto_purifier_on_pm"] end
    if streams["auto_purifier_off_pm"] ~= nil then keyP["auto_purifier_off_pm"] = streams["auto_purifier_off_pm"] end
    if streams["auto_fresh_on_co2"] ~= nil then keyP["auto_fresh_on_co2"] = streams["auto_fresh_on_co2"] end
    if streams["auto_fresh_off_co2"] ~= nil then keyP["auto_fresh_off_co2"] = streams["auto_fresh_off_co2"] end
    if streams["humi_on_value"] ~= nil then keyP["humi_on_value"] = streams["humi_on_value"] end
    if streams["humi_off_value"] ~= nil then keyP["humi_off_value"] = streams["humi_off_value"] end
    if streams["outdoor_pm"] ~= nil then keyP["outdoor_pm"] = streams["outdoor_pm"] end
    if streams["manul_humi_value"] ~= nil then keyP["manul_humi_value"] = streams["manul_humi_value"] end
    if streams["right_wind_speed"] ~= nil then keyP["right_wind_speed"] = streams["right_wind_speed"] end
    if streams["left_wind_speed_target_value"] ~= nil then
        keyP["left_wind_speed_target_value"] = streams["left_wind_speed_target_value"]
    end
    if streams["right_wind_speed_target_value"] ~= nil then
        keyP["right_wind_speed_target_value"] = streams["right_wind_speed_target_value"]
    end
    if streams["voice_control"] ~= nil then keyP["voice_control"] = streams["voice_control"] end
    if streams["prevent_straight_wind_lr"] ~= nil then
        keyP["prevent_straight_wind_lr"] = streams["prevent_straight_wind_lr"]
    end
    if streams["down_wind_left_switch"] ~= nil then keyP["down_wind_left_switch"] = streams["down_wind_left_switch"] end
    ...
    if streams["solar_eco"] ~= nil then keyP["solar_eco"] = streams["solar_eco"] end
    ...
    if streams["scene_id"] ~= nil then keyP["scene_id"] = streams["scene_id"] end
    ...
    if streams["eight_degree_heat"] ~= nil then keyP["eight_degree_heat"] = streams["eight_degree_heat"] end
    if streams["eight_degree_heat_remember"] ~= nil then
        keyP["eight_degree_heat_remember"] = streams["eight_degree_heat_remember"]
    end
    ...
    if streams["pm2_5_algorithm"] ~= nil then keyP["pm2_5_algorithm"] = streams["pm2_5_algorithm"] end
    if streams["co2_algorithm"] ~= nil then keyP["co2_algorithm"] = streams["co2_algorithm"] end
    if streams["temp_algorithm"] ~= nil then keyP["temp_algorithm"] = streams["temp_algorithm"] end
    if streams["humidity_algorithm"] ~= nil then keyP["humidity_algorithm"] = streams["humidity_algorithm"] end
    if streams["recommend_mode_switch"] ~= nil then keyP["recommend_mode_switch"] = streams["recommend_mode_switch"] end
    if streams["current_wind_speed"] ~= nil then keyP["current_wind_speed"] = streams["current_wind_speed"] end
    if streams["current_down_wind_speed"] ~= nil then
        keyP["current_down_wind_speed"] = streams["current_down_wind_speed"]
    end
    if
        (
            streams["total_time_switch"] ~= nil
            or streams["pm2_5_algorithm"] ~= nil
            or streams["co2_algorithm"] ~= nil
            or streams["temp_algorithm"] ~= nil
            or streams["humidity_algorithm"] ~= nil
            or streams["recommend_mode_switch"] ~= nil
        ) and jsonType == "control"
    then
        keyP["control_flag"] = 2
    end
    if streams["limit_dry_switch_set"] ~= nil then keyP["limit_dry_switch_set"] = streams["limit_dry_switch_set"] end
    if streams["limit_dry_switch_set"] ~= nil and jsonType == "control" then keyP["control_flag"] = 3 end
    if streams["prevent_super_cool"] ~= nil then keyP["prevent_super_cool"] = streams["prevent_super_cool"] end
    if streams["last_humidity"] ~= nil then keyP["last_humidity"] = streams["last_humidity"] end
    if streams["rewarming_dry"] ~= nil then keyP["rewarming_dry"] = streams["rewarming_dry"] end
end
local function binToModel(binData)
    ...
    if dataType == 0xFF then
        ...
        keyP["communication_fault"] = bit.rshift(bit.band(messageBytes[5], 0x80), 7)
        keyP["smart_eye"] = bit.band(messageBytes[6], 0x03)
        keyP["horizontal_swing_left"] = bit.rshift(bit.band(messageBytes[6], 0x04), 2)
        keyP["horizontal_swing_right"] = bit.rshift(bit.band(messageBytes[6], 0x08), 3)
        keyP["vertical_swing_left"] = bit.rshift(bit.band(messageBytes[6], 0x10), 4)
        keyP["vertical_swing_right"] = bit.rshift(bit.band(messageBytes[6], 0x20), 5)
        ...
        keyP["remove_dust_full"] = bit.rshift(bit.band(messageBytes[7], 0x40), 6)
        keyP["clear_filter_change_time"] = bit.rshift(bit.band(messageBytes[7], 0x80), 7)
        keyP["self_wash"] = bit.band(messageBytes[8], 0x01)
        keyP["energy_save"] = bit.rshift(bit.band(messageBytes[8], 0x02), 1)
        keyP["advance_cold_hot"] = bit.rshift(bit.band(messageBytes[8], 0x04), 2)
        ...
        keyP["reset_electricity"] = bit.rshift(bit.band(messageBytes[8], 0x40), 6)
        ...
        keyP["total_fast_check"] = bit.rshift(bit.band(messageBytes[9], 0x04), 2)
        keyP["elec_dust_collect"] = bit.rshift(bit.band(messageBytes[9], 0x08), 3)
        ...
        keyP["child_comfort_sleep"] = bit.rshift(bit.band(messageBytes[10], 0x04), 2)
        keyP["energy_save_xiaotiane"] = bit.rshift(bit.band(messageBytes[10], 0x08), 3)
        keyP["no_wind_sense"] = bit.rshift(bit.band(messageBytes[10], 0x10), 4)
        keyP["try_run"] = bit.rshift(bit.band(messageBytes[10], 0x20), 5)
        keyP["prevent_water_enable"] = bit.rshift(bit.band(messageBytes[10], 0x40), 6)
        keyP["display_receive_control_button"] = bit.rshift(bit.band(messageBytes[10], 0x80), 7)
        ...
        keyP["eight_degree_heat"] = bit.rshift(bit.band(messageBytes[12], 0xF0), 4)
        ...
        keyP["left_right_wind_direction"] = bit.rshift(bit.band(messageBytes[18], 0xF0), 4)
        keyP["mosquito_repellent"] = bit.rshift(bit.band(messageBytes[19], 0x02), 1)
        ...
        keyP["prevent_straight_wind"] = bit.rshift(bit.band(messageBytes[19], 0x10), 4)
        keyP["gentle_wind_sense"] = bit.rshift(bit.band(messageBytes[19], 0x20), 5)
        keyP["child_prevent_cold_wind_child"] = bit.rshift(bit.band(messageBytes[19], 0x40), 6)
        keyP["child_prevent_cold_wind_parent"] = bit.rshift(bit.band(messageBytes[19], 0x80), 7)
        keyP["no_wind_sense_up"] = bit.band(messageBytes[20], 0x01)
        keyP["no_wind_sense_down"] = bit.rshift(bit.band(messageBytes[20], 0x02), 1)
        keyP["permanent_wind"] = bit.rshift(bit.band(messageBytes[20], 0x04), 2)
        keyP["temperature_unit"] = bit.rshift(bit.band(messageBytes[20], 0x08), 3)
        ...
        keyP["manul_water_washing"] = bit.rshift(bit.band(messageBytes[23], 0x02), 1)
        keyP["water_washing_control"] = bit.rshift(bit.band(messageBytes[23], 0x0C), 2)
        keyP["ud_wind_accelerate"] = bit.rshift(bit.band(messageBytes[23], 0x10), 4)
        keyP["lr_wind_accelerate"] = bit.rshift(bit.band(messageBytes[23], 0x20), 5)
        keyP["prevent_trip"] = bit.rshift(bit.band(messageBytes[23], 0x40), 6)
        keyP["smart_temperature_control"] = bit.rshift(bit.band(messageBytes[23], 0x80), 7)
        ...
        keyP["child_prevent_cold_wind"] = bit.rshift(bit.band(messageBytes[24], 0x08), 3)
        keyP["double_water_washing"] = bit.rshift(bit.band(messageBytes[24], 0x10), 4)
        ...
        keyP["remove_peculiar_smell"] = bit.rshift(bit.band(messageBytes[32], 0x10), 4)
    ...
    if dataType == 0x11 or dataType == 0xF1 then
        ...
        keyP["horizontal_swing_right"] = bit.rshift(bit.band(messageBytes[1], 0x02), 1)
        keyP["vertical_swing_left"] = bit.rshift(bit.band(messageBytes[1], 0x04), 2)
        keyP["vertical_swing_right"] = bit.rshift(bit.band(messageBytes[1], 0x08), 3)
        ...
        keyP["down_wind_left_switch"] = bit.rshift(bit.band(messageBytes[29], 0x40), 6)
        keyP["down_wind_right_switch"] = bit.rshift(bit.band(messageBytes[29], 0x80), 7)
        ...
        if #binData > 57 then
            ...
            keyP["pm2_5_algorithm"] = bit.rshift(bit.band(messageBytes[57], 0x02), 1)
            keyP["co2_algorithm"] = bit.rshift(bit.band(messageBytes[57], 0x04), 2)
            keyP["temp_algorithm"] = bit.rshift(bit.band(messageBytes[57], 0x08), 3)
            keyP["total_time_switch"] = bit.rshift(bit.band(messageBytes[57], 0x10), 4)
            keyP["eight_degree_heat"] = messageBytes[58]
            keyP["eight_degree_heat_remember"] = messageBytes[59]
            keyP["limit_dry_switch_set"] = bit.rshift(bit.band(messageBytes[57], 0x20), 5)
            keyP["prevent_super_cool"] = bit.rshift(bit.band(messageBytes[57], 0x40), 6)
        else
            keyP["eight_degree_heat"] = 0
            keyP["eight_degree_heat_remember"] = 0
        end
        if #binData > 59 then
            keyP["main_control_software_version"] = bit.band(bit.lshift(messageBytes[61], 8), messageBytes[60])
            keyP["main_control_parm_version"] = bit.band(bit.lshift(messageBytes[63], 8), messageBytes[62])
            keyP["out_machine_parm_version"] = bit.band(bit.lshift(messageBytes[65], 8), messageBytes[64])
            if #binData > 68 then
        ...
        if #binData > 67 then
            if bit.band(messageBytes[67], 0x80) == 0x80 then
                keyP["down_humidity"] = (0 - bit.band(bit.bnot(messageBytes[67] * 256 + messageBytes[66]) + 1, 0xffff))
                    / 100
            else
                keyP["down_humidity"] = (messageBytes[66] + messageBytes[67] * 256) / 100
            end
        end
        if #binData > 69 then
            keyP["full_time_hour"] = bit.bor(
                bit.bor(bit.bor(messageBytes[70], bit.lshift(messageBytes[71], 8)), bit.lshift(messageBytes[72], 16)),
        ...
        if #binData > 76 then
            ...
            keyP["has_rewarming_dry"] = bit.rshift(bit.band(messageBytes[77], 0x02), 1)
        end
    ...
    if dataType == 0x10 then
        ...
        keyP["outdoor_fan_lose_speed"] = bit.rshift(bit.band(messageBytes[22], 0x02), 1)
    ...
    if dataType == 0x12 then
        ...
        keyP["standby_anti_freezing_protection"] = bit.rshift(bit.band(messageBytes[1], 0x10), 4)
        ...
        keyP["indoor_e_parameter"] = bit.rshift(bit.band(messageBytes[2], 0x80), 7)
    ...
    if dataType == 0x40 then
        ...
        keyP["voltage_protect"] = bit.rshift(bit.band(messageBytes[19], 0x20), 5)
    ...
    if dataType == 0x30 then
        if bit.band(messageBytes[1], 0x80) == 0x80 then
            keyP["t1_temp"] = (0 - bit.band(bit.bnot(messageBytes[1] * 256 + messageBytes[0]) + 1, 0xffff)) / 100
        else
            keyP["t1_temp"] = (messageBytes[0] + messageBytes[1] * 256) / 100
        end
        if bit.band(messageBytes[3], 0x80) == 0x80 then
            keyP["t2_temp"] = (0 - bit.band(bit.bnot(messageBytes[3] * 256 + messageBytes[2]) + 1, 0xffff)) / 100
        else
            keyP["t2_temp"] = (messageBytes[2] + messageBytes[3] * 256) / 100
        end
        if bit.band(messageBytes[5], 0x80) == 0x80 then
            keyP["t3_temp"] = (0 - bit.band(bit.bnot(messageBytes[5] * 256 + messageBytes[4]) + 1, 0xffff)) / 100
        else
            keyP["t3_temp"] = (messageBytes[4] + messageBytes[5] * 256) / 100
        end
        if bit.band(messageBytes[7], 0x80) == 0x80 then
            keyP["t4_temp"] = (0 - bit.band(bit.bnot(messageBytes[7] * 256 + messageBytes[6]) + 1, 0xffff)) / 100
        else
            keyP["t4_temp"] = (messageBytes[6] + messageBytes[7] * 256) / 100
        end
        keyP["dust_co2"] = bit.bor(bit.lshift(messageBytes[47], 8), messageBytes[46])
        keyP["co2_concentration"] = bit.bor(bit.lshift(messageBytes[58], 8), messageBytes[57])
        keyP["wind_speed_real_ac"] = messageBytes[16]
        keyP["indoor_e0_fault"] = bit.rshift(bit.band(messageBytes[17], 0x04), 2)
        keyP["indoor_ea_fault"] = bit.rshift(bit.band(messageBytes[21], 0x20), 5)
        keyP["indoor_e3_fault"] = bit.rshift(bit.band(messageBytes[17], 0x08), 3)
        keyP["indoor_fe_fault"] = bit.rshift(bit.band(messageBytes[18], 0x20), 5)
        keyP["outdoor_e51_fault"] = bit.band(messageBytes[26], 0x01)
        keyP["outdoor_e52_fault"] = bit.rshift(bit.band(messageBytes[26], 0x02), 1)
        keyP["outdoor_e53_fault"] = bit.rshift(bit.band(messageBytes[26], 0x04), 2)
        keyP["outdoor_e54_fault"] = bit.rshift(bit.band(messageBytes[26], 0x08), 3)
        keyP["indoor_e60_fault"] = bit.band(messageBytes[17], 0x01)
        keyP["indoor_e61_fault"] = bit.rshift(bit.band(messageBytes[17], 0x02), 1)
        keyP["outdoor_e7_fault"] = bit.rshift(bit.band(messageBytes[26], 0x40), 6)
        keyP["rfid_fault"] = bit.rshift(bit.band(messageBytes[22], 0x20), 5)
        keyP["indoor_eb1_fault"] = bit.rshift(bit.band(messageBytes[22], 0x10), 4)
    ...
    if dataType == 0x20 then
        if bit.band(messageBytes[13], 0x80) == 0x80 then
            keyP["down_humidity"] = (0 - bit.band(bit.bnot(messageBytes[13] * 256 + messageBytes[12]) + 1, 0xffff))
                / 100
        else
            keyP["down_humidity"] = (messageBytes[12] + messageBytes[13] * 256) / 100
        end
    end
    if dataType == 0x32 then
        keyP["wind_speed_real"] = messageBytes[12]
        keyP["disinfect_time"] = bit.bor(bit.lshift(messageBytes[15], 8), messageBytes[14])
        keyP["wet_film_time"] = bit.bor(bit.lshift(messageBytes[17], 8), messageBytes[16])
        keyP["filter_time"] = bit.bor(bit.lshift(messageBytes[19], 8), messageBytes[18])
        keyP["ac_filter_time"] = messageBytes[33]
        keyP["water_tank_water"] = bit.rshift(bit.band(messageBytes[22], 0x04), 2)
        keyP["water_tank_board"] = bit.rshift(bit.band(messageBytes[22], 0x08), 3)
        keyP["indoor_e3_1_fault"] = bit.rshift(bit.band(messageBytes[6], 0x02), 1)
    ...
    if dataType == 0xC0 then
        ...
        keyP["category_tag"] = bit.bor(bit.lshift(messageBytes[7], 8), messageBytes[6])
        keyP["filter_available_time"] = string.char(messageBytes[15], messageBytes[16], messageBytes[17])
        keyP["filter_available_time"] = tonumber(keyP["filter_available_time"]) * 100
        keyP["filter_used_time"] = bit.bor(bit.lshift(messageBytes[35], 8), messageBytes[34])
    end
end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        elseif keyP["control_flag"] == 2 then
            ...
            bodyBytes[4] = bit.bor(bit.lshift(keyP["pm2_5_algorithm"], 1), bodyBytes[4])
            bodyBytes[4] = bit.bor(bit.lshift(keyP["co2_algorithm"], 2), bodyBytes[4])
            bodyBytes[4] = bit.bor(bit.lshift(keyP["temp_algorithm"], 3), bodyBytes[4])
            bodyBytes[4] = bit.bor(bit.lshift(keyP["total_time_switch"], 4), bodyBytes[4])
            bodyBytes[4] = bit.bor(bit.lshift(keyP["recommend_mode_switch"], 5), bodyBytes[4])
            infoM = getTotalMsg(bodyBytes, 0xF1)
        elseif keyP["control_flag"] == 3 then
            ...
            bodyBytes[1] = (keyP["temperature"] + keyP["small_temperature"]) * 2 + 30
            ...
            bodyBytes[5] = bit.bor(bit.lshift(keyP["display_status"], 1), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["mute_voice"], 2), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["cool_hot_sense"], 3), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["low_power_cost"], 4), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["dry_clean"], 5), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["strongWindValue"], 6), bodyBytes[5])
            bodyBytes[5] = bit.bor(bit.lshift(keyP["communication_fault"], 7), bodyBytes[5])
            bodyBytes[6] = bit.bor(keyP["smart_eye"], bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["horizontal_swing_left"], 2), bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["horizontal_swing_right"], 3), bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["vertical_swing_left"], 4), bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["vertical_swing_right"], 5), bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["power"], 6), bodyBytes[6])
            bodyBytes[6] = bit.bor(bit.lshift(keyP["solar_eco"], 7), bodyBytes[6])
            bodyBytes[7] = bit.bor(keyP["forceCoolMode"], bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["forceAutoMode"], 1), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["force_defrost"], 2), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["PTCDependT4Value"], 3), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["PTCValue"], 4), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["do_not_disturb"], 5), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["remove_dust_full"], 6), bodyBytes[7])
            bodyBytes[7] = bit.bor(bit.lshift(keyP["clear_filter_change_time"], 7), bodyBytes[7])
            bodyBytes[8] = bit.bor(keyP["self_wash"], bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["energySaveValue"], 1), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["advance_cold_hot"], 2), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["rui_wind"], 3), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["set_i_mode"], 4), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["save_i_mode"], 5), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["reset_electricity"], 6), bodyBytes[8])
            bodyBytes[8] = bit.bor(bit.lshift(keyP["high_save_power"], 7), bodyBytes[8])
            bodyBytes[9] = bit.bor(keyP["fast_check"], bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["elec_fast_check"], 1), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["total_fast_check"], 2), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["elec_dust_collect"], 3), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["huanqi_function"], 4), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["light_level"], 5), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["natural_wind"], 6), bodyBytes[9])
            bodyBytes[9] = bit.bor(bit.lshift(keyP["leave_home"], 7), bodyBytes[9])
            bodyBytes[10] = bit.bor(keyP["prevent_cold_child"], bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["preventCold"], 1), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["child_comfort_sleep"], 2), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["energy_save_xiaotiane"], 3), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["no_wind_sense"], 4), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["try_run"], 5), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["prevent_water_enable"], 6), bodyBytes[10])
            bodyBytes[10] = bit.bor(bit.lshift(keyP["display_receive_control_button"], 7), bodyBytes[10])
            ...
            bodyBytes[12] = bit.bor(bit.lshift(keyP["eight_degree_heat"], 4), bodyBytes[12])
            bodyBytes[13] = bit.bor(keyP["buzzerValue"], bodyBytes[13])
            bodyBytes[13] = bit.bor(bit.lshift(keyP["buzzer_param"], 4), bodyBytes[13])
            ...
            bodyBytes[17] = bit.bor(keyP["second_temperature_setting"], bodyBytes[17])
            bodyBytes[18] = bit.bor(keyP["up_down_wind_direction"], bodyBytes[18])
            bodyBytes[18] = bit.bor(bit.lshift(keyP["left_right_wind_direction"], 4), bodyBytes[18])
            bodyBytes[19] = bit.bor(keyP["water_washing"], bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["mosquito_repellent"], 1), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["comfort_save"], 2), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["x_fan_control"], 3), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["prevent_straight_wind"], 4), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["gentle_wind_sense"], 5), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["child_prevent_cold_wind_child"], 6), bodyBytes[19])
            bodyBytes[19] = bit.bor(bit.lshift(keyP["child_prevent_cold_wind_parent"], 7), bodyBytes[19])
            bodyBytes[20] = bit.bor(keyP["no_wind_sense_up"], bodyBytes[20])
            bodyBytes[20] = bit.bor(bit.lshift(keyP["no_wind_sense_down"], 1), bodyBytes[20])
            bodyBytes[20] = bit.bor(bit.lshift(keyP["permanent_wind"], 2), bodyBytes[20])
            bodyBytes[20] = bit.bor(bit.lshift(keyP["temperature_unit"], 3), bodyBytes[20])
            ...
            bodyBytes[23] = bit.bor(bit.lshift(keyP["manul_water_washing"], 1), bodyBytes[23])
            bodyBytes[23] = bit.bor(bit.lshift(keyP["water_washing_control"], 2), bodyBytes[23])
            bodyBytes[23] = bit.bor(bit.lshift(keyP["ud_wind_accelerate"], 4), bodyBytes[23])
            bodyBytes[23] = bit.bor(bit.lshift(keyP["lr_wind_accelerate"], 5), bodyBytes[23])
            bodyBytes[23] = bit.bor(bit.lshift(keyP["prevent_trip"], 6), bodyBytes[23])
            bodyBytes[23] = bit.bor(bit.lshift(keyP["prevent_super_cool"], 7), bodyBytes[23])
            bodyBytes[24] = bit.bor(keyP["timer_message"], bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["lock"], 1), bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["remote_control"], 2), bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["child_prevent_cold_wind"], 3), bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["double_water_washing"], 4), bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["deep_clean"], 5), bodyBytes[24])
            bodyBytes[24] = bit.bor(bit.lshift(keyP["extreme_wind"], 6), bodyBytes[24])
            bodyBytes[25] = keyP["fn_no_wind_sense"]
            bodyBytes[26] = keyP["fresh_air_wind_precent"]
            bodyBytes[27] = keyP["auto_wash_time"]
            bodyBytes[28] = bit.bor(bit.lshift(keyP["autoNewWind"], 1), bodyBytes[28])
            bodyBytes[28] = bit.bor(bit.lshift(keyP["disinfect"], 5), bodyBytes[28])
            bodyBytes[33] = bit.bor(keyP["right_ud_wind"], bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["wind_swing_left"], 1), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["wind_swing_right"], 2), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["no_wind_sense_left"], 3), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["no_wind_sense_right"], 4), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["autoHumi"], 5), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["air_exhaust"], 6), bodyBytes[33])
            bodyBytes[33] = bit.bor(bit.lshift(keyP["manuHumi"], 7), bodyBytes[33])
            bodyBytes[34] = keyP["down_wind_speed_level"]
            bodyBytes[35] = bit.bor(keyP["comfort_fresh_air"], bodyBytes[35])
            bodyBytes[35] = bit.bor(bit.lshift(keyP["fresh_air_setting_mode"], 1), bodyBytes[35])
            bodyBytes[35] = bit.bor(bit.lshift(keyP["autoPurify"], 2), bodyBytes[35])
            bodyBytes[35] = bit.bor(bit.lshift(keyP["manuPurify"], 3), bodyBytes[35])
            bodyBytes[32] = bit.bor(bit.lshift(keyP["remove_arofene"], 2), bodyBytes[32])
            bodyBytes[32] = bit.bor(bit.lshift(keyP["remove_peculiar_smell"], 3), bodyBytes[32])
            bodyBytes[32] = bit.bor(bit.lshift(keyP["auto_comfort_fresh_air"], 7), bodyBytes[32])
            bodyBytes[36] = bit.band(keyP["disinfect_setting_time"], 0xff)
            bodyBytes[37] = bit.band(bit.rshift(keyP["disinfect_setting_time"], 8), 0xff)
            bodyBytes[38] = bit.band(keyP["auto_purifier_on_pm"], 0xff)
            bodyBytes[39] = bit.band(bit.rshift(keyP["auto_purifier_on_pm"], 8), 0xff)
            bodyBytes[40] = bit.band(keyP["auto_purifier_off_pm"], 0xff)
            bodyBytes[41] = bit.band(bit.rshift(keyP["auto_purifier_off_pm"], 8), 0xff)
            bodyBytes[42] = bit.band(keyP["auto_fresh_on_co2"], 0xff)
            bodyBytes[43] = bit.band(bit.rshift(keyP["auto_fresh_on_co2"], 8), 0xff)
            bodyBytes[44] = bit.band(keyP["auto_fresh_off_co2"], 0xff)
            bodyBytes[45] = bit.band(bit.rshift(keyP["auto_fresh_off_co2"], 8), 0xff)
            bodyBytes[46] = bit.band(keyP["humi_on_value"], 0xff)
            bodyBytes[47] = bit.band(bit.rshift(keyP["humi_off_value"], 8), 0xff)
            ...
            bodyBytes[53] = bit.band(keyP["right_wind_speed_target_value"], 0xff)
            bodyBytes[54] = bit.band(keyP["scene_id"], 0xff)
            bodyBytes[30] = bit.bor(keyP["voice_control"], bodyBytes[30])
            bodyBytes[30] = bit.bor(bit.lshift(keyP["prevent_straight_wind_lr"], 1), bodyBytes[30])
            keyP["closeHour"] = math.floor(keyP["power_off_time_value"] / 60)
            keyP["closeStepMintues"] = math.floor((keyP["power_off_time_value"] % 60) / 15)
            keyP["closeMin"] = math.floor(((keyP["power_off_time_value"] % 60) % 15))
            keyP["openHour"] = math.floor(keyP["power_on_time_value"] / 60)
            keyP["openStepMintues"] = math.floor((keyP["power_on_time_value"] % 60) / 15)
            keyP["openMin"] = math.floor(((keyP["power_on_time_value"] % 60) % 15))
            if keyP["power_on_timer"] == 0x01 then
                bodyBytes[55] =
                    bit.bor(bit.bor(keyP["power_on_timer"], bit.lshift(keyP["openHour"], 2)), keyP["openStepMintues"])
            elseif keyP["power_on_timer"] == 0x00 then
                bodyBytes[55] = 0x7F
            end
            if keyP["power_off_timer"] == 0x01 then
                bodyBytes[56] = bit.bor(
                    bit.bor(keyP["power_off_timer"], bit.lshift(keyP["closeHour"], 2)),
                    keyP["closeStepMintues"]
                )
            elseif keyP["power_off_timer"] == 0x00 then
                bodyBytes[56] = 0x7F
            end
            bodyBytes[59] = bit.bor(keyP["down_wind_left_switch"], bodyBytes[59])
            bodyBytes[59] = bit.bor(bit.lshift(keyP["down_wind_right_switch"], 1), bodyBytes[59])
            bodyBytes[59] = bit.bor(bit.lshift(keyP["rewarming_dry"], 5), bodyBytes[59])
...
function dataToJson(jsonCmd)
    ...
    if dataType == 0x11 or dataType == 0xF1 then
        ...
        if keyP["dryValue"] == keyB["BYTE_COMMON_ON"] or (keyP["modeValue"] == keyB["BYTE_MODE_DRY"]) then
        ...
        streams["child_prevent_cold_wind_parent"] = keyP["child_prevent_cold_wind_parent"]
        streams["child_prevent_cold_wind_child"] = keyP["child_prevent_cold_wind_child"]
        ...
        streams["second_temperature_setting"] = keyP["second_temperature_setting"]
        ...
        streams["eight_degree_heat_remember"] = keyP["eight_degree_heat_remember"]
        ...
        streams["left_wind_speed_target_value"] = keyP["left_wind_speed_target_value"]
        streams["right_wind_speed_target_value"] = keyP["right_wind_speed_target_value"]
        ...
        streams["main_control_software_version"] = keyP["main_control_software_version"]
    ...
    if dataType == 0x31 then streams["humidity_value"] = keyP["humidity_value"] end
    if dataType == 0x20 then streams["down_humidity"] = keyP["down_humidity"] end
    if dataType == 0x32 then
