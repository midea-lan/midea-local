...
local function binToModel(binData)
    ...
    if (dataType == 0x03 and messageBytes[0] == 0xB5) then
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if (messageBytes[cursor + 0] == 0x1D and messageBytes[cursor + 1] ==
                0x02) then
                keyP["b5_water_pump"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x2D and messageBytes[cursor + 1] ==
                0x02) then
    ...
    elseif (messageBytes[0] == 0xB1 or messageBytes[0] == 0xB0) then
        ...
        keyP["waterPumpEnableValue"] = bit.rshift(
                                           bit.band(messageBytes[9], 0x10), 4)
        ...
        keyP["waterTimeValue"] = bit.bor(bit.lshift(messageBytes[14], 8),
                                         messageBytes[13])
...
function dataToJson(jsonCmd)
    ...
    dataType = info[10]
    if ((dataType ~= 0x02) and (dataType ~= 0x03) and (dataType ~= 0x04) and
        (dataType ~= 0x05)) then return nil end
    ...
    if (keyP["propertyNumber"] == 0) then
        ...
        if (keyP["waterPumpEnableValue"] == 0x10 or keyP["waterPumpEnableValue"] ==
            0x01) then
        ...
        if (keyP["propertyNumber"] > 0) then
            ...
            if (keyP["openTimerSwitch"] == keyB["BYTE_START_TIMER_SWITCH_ON"]) then
                bodyBytes[4] = bit.bor(bit.bor(keyP["openTimerSwitch"],
                                       ...
                                       openStepMintues)
                bodyBytes[6] = bit.bor(bodyBytes[6],
                                       bit.lshift((15 - openMin), 4))
            ...
            bodyBytes[10] = bit.bor(bit.lshift(keyP["swingUDValue"], 3),
                                    bodyBytes[10])
