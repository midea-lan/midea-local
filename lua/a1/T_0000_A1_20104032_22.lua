...
local function jsonToModel(stateJson, controlJson)
    ...
    if temValue == keyV["VALUE_FUNCTION_ON"] then
        keyP["childLockValue"] = keyB["BYTE_CHILD_LOCK_ON"]
    else
        keyP["childLockValue"] = keyB["BYTE_CHILD_LOCK_OFF"]
    end
    ...
    elseif temValue == keyV["VALUE_MODE_MUTE"] then
        keyP["modeValue"] = keyB["BYTE_MODE_MUTE"]
    elseif temValue == keyV["VALUE_MODE_AUTO_2"] then
        keyP["modeValue"] = keyB["BYTE_MODE_AUTO_2"]
    elseif temValue == "sleep" then
        keyP["modeValue"] = 0x0C
    end
    ...
    if controlCmd["self_clean"] ~= nil then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        if controlCmd["self_clean"] == keyV["VALUE_FUNCTION_ON"] then
            keyP["self_clean"] = 1
        else
            keyP["self_clean"] = 0
        end
...
function dataToJson(jsonCmd)
    ...
    if keyP["propertyNumber"] == 0 then
        ...
        elseif keyP["modeValue"] == keyB["BYTE_MODE_MUTE"] then
            streams[keyT["KEY_MODE"]] = keyV["VALUE_MODE_MUTE"]
        elseif keyP["modeValue"] == keyB["BYTE_MODE_AUTO_2"] then
            streams[keyT["KEY_MODE"]] = keyV["VALUE_MODE_AUTO_2"]
        elseif keyP["modeValue"] == 0x0C then
            streams[keyT["KEY_MODE"]] = "sleep"
        end
        ...
        if keyP["dry"] == 0x02 then
            streams[keyT["KEY_DRY"]] = "on"
        else
            streams[keyT["KEY_DRY"]] = "off"
        end
        ...
        if keyP["childLockValue"] == 0x80 then
            streams["child_lock"] = "on"
        elseif keyP["childLockValue"] == 0x00 then
            streams["child_lock"] = "off"
        else
            streams["child_lock"] = "off"
        end
...
function jsonToData(jsonCmd)
    ...
    elseif control then
        ...
        if keyP["propertyNumber"] > 0 then
            ...
            bodyBytes[9] = bit.bor(
                bit.bor(bit.bor(keyP["anionValue"], keyP["filterValue"]), keyP["waterPumpValue"]),
