...
local function general_bin_section_without_key_map(key_map)
    ...
    elseif
        ...
        or key_map.value_type == "int16_t"
        or key_map.value_type == "int16_t_10"
        or key_map.value_type == "int16_t_1/10"
...
local function get_json_in_spec_type(data, key_map)
    ...
    if type(data) == "table" then
        ...
        elseif key_map.value_type == "int16_t_10" then
            if bit.rshift(data[1], 15) == 1 then
                ret = string.format("-%.1f", (bit.band(bit.bnot(bit.band(data[1], 0x7FFF)), 0x7FFF) + 1) / 10)
            else
                ret = string.format("%.1f", data[1] / 10)
            end
        elseif key_map.value_type == "int16_t_1/10" then
            if bit.rshift(data[1], 15) == 1 then
                ret = string.format("-%d", (bit.band(bit.bnot(bit.band(data[1], 0x7FFF)), 0x7FFF) + 1) * 10)
            else
...
local function check_lua_header(binData)
    ...
    if
        ...
        or data[3] ~= 0xC3 -- or data[len-1] ~= 0xFF
    ...
    if _hmiVersionNum_patch ~= nil then
        if hmiVersionNum ~= nil then
            ...
            if _hmiVersionNum_patch2 ~= nil then
                json_data["status"]["hmiVersionNum"] =
                    string.format("%d.%d.%d.%02d|[%d]", v1, _hmiVersionNum_patch, v2, v3, _hmiVersionNum_patch2)
            else
